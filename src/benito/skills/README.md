...
- `src/Benito/config/` — workspace path resolution and trust-marker location.
- `src/Benito/agent/` — injection consumers in `agent/prompts/` and `agent/harness/session/turn.rs`.
- `src/Benito/workspace/` — workspace-relative skill paths.
- `src/core/event_bus/` — emits `DomainEvent::Skill(*)` on install / uninstall.

## Called by

- `src/Benito/tools/traits.rs` — `ToolResult` / `ToolContent` shape shared with the tool registry.
- `src/Benito/workspace/ops.rs` — workspace bootstrap touches the skill directory layout.
- `src/Benito/agent/agents/integrations_agent/prompt.rs` — integrations agent reads the skill catalog.
- `src/Benito/agent/harness/fork_context.rs` — fork context propagates injected skills.
- `src/Benito/agent/harness/session/turn.rs` — per-turn injection point.
- `src/Benito/agent/prompts/{mod,types}.rs` — render `## Available Skills` catalog section.
...
- Cross-cutting agent + skill behavior is covered indirectly by `src/Benito/agent/harness/session/{turn,runtime}_tests.rs`.
