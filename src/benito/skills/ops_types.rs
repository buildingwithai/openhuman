...
pub enum SkillScope {
    /// Skill shipped with the user's global config (`~/.Benito/skills/...`).
    User,
    /// Skill shipped with the current workspace (`<ws>/.Benito/skills/...`).
...
pub struct Skill {
    ...
    pub name: String,
    /// On-disk slug — the directory name under `~/.Benito/skills/` (user
