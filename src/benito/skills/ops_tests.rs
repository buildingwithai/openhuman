...
fn load_skills_parses_skill_md_frontmatter() {
    ...
    write(&ws.join(".Benito").join("trust"), "");
    let skill_dir = ws.join(".Benito").join("skills").join("hello-world");
    write(
...
fn deprecated_top_level_fields_load_with_migration_warning() {
    ...
    let ws = dir.path();
    write(&ws.join(".Benito").join("trust"), "");
    let skill_dir = ws.join(".Benito").join("skills").join("legacy-fm");
    write(
        &skill_dir.join("SKILL.md"),
...
fn project_skills_skipped_when_not_trusted() {
    ...
    let ws = dir.path();
    // No trust marker.
    let skill_dir = ws.join(".Benito").join("skills").join("unsafe");
    write(
        &skill_dir.join("SKILL.md"),
...
fn frontmatter_missing_name_warns_and_falls_back() {
    ...
    let ws = dir.path();
    write(&ws.join(".Benito").join("trust"), "");
    let skill_dir = ws.join(".Benito").join("skills").join("mystery");
    write(
        &skill_dir.join("SKILL.md"),
...
fn frontmatter_missing_description_uses_first_body_line() {
    ...
    let ws = dir.path();
    write(&ws.join(".Benito").join("trust"), "");
    let skill_dir = ws.join(".Benito").join("skills").join("s");
    write(
        &skill_dir.join("SKILL.md"),
        "---\nname: s\n---\n\n# Heading\n\nActual first line.\n",
    );
...
fn directory_name_mismatch_warns_but_loads() {
    ...
    let ws = dir.path();
    write(&ws.join(".Benito").join("trust"), "");
    let skill_dir = ws.join(".Benito").join("skills").join("dir-name");
    write(
        &skill_dir.join("SKILL.md"),
...
fn project_scope_shadows_user_scope_on_collision() {
    ...
    write(&ws_dir.path().join(".Benito").join("trust"), "");
...
fn symlinked_skill_dirs_are_skipped() {
    ...
    // Symlink <ws>/.Benito/skills/evil -> external/evil
    let skills_root = ws.join(".Benito").join("skills");
...
fn hidden_dirs_are_skipped() {
    ...
    let ws = dir.path();
    write(&ws.join(".Benito").join("trust"), "");
    let hidden = ws.join(".Benito").join("skills").join(".hidden");
    write(
...
fn create_skill_project_scope_writes_under_workspace_when_trusted() {
    let home = tempfile::tempdir().unwrap();
    let ws = tempfile::tempdir().unwrap();
    write(&ws.path().join(".Benito").join(TRUST_MARKER), "");
...
fn uninstall_skill_rejects_path_traversal_names() {
    let home = tempfile::tempdir().unwrap();
    std::fs::create_dir_all(home.path().join(".Benito").join("skills")).unwrap();
...
fn uninstall_skill_refuses_dir_without_skill_md() {
    let home = tempfile::tempdir().unwrap();
    let bogus = home.path().join(".Benito").join("skills").join("bogus");
...
fn uninstall_skill_rejects_symlink_escape() {
    let home = tempfile::tempdir().unwrap();
    let skills_root = home.path().join(".Benito").join("skills");
    std::fs::create_dir_all(&skills_root).unwrap();
...
/// A symlinked skills *root* (`~/.Benito/skills -> elsewhere`) must
...
fn uninstall_skill_rejects_symlinked_skills_root() {
    ...
    std::fs::create_dir_all(home.path().join(".Benito")).unwrap();
    std::os::unix::fs::symlink(&real_skills, home.path().join(".Benito").join("skills"))
        .unwrap();
    let err = uninstall_skill(
        UninstallSkillParams {
            name: "real".into(),
