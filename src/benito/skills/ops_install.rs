...
/// it to `<workspace>/.Benito/skills/<slug>/SKILL.md`.
///
/// Design rationale: Benito's skill discovery scans
/// `<workspace>/.Benito/skills/` (plus `~/.Benito/skills/` and legacy
...
/// agent ecosystem is incompatible with Benito's skill layout, so we fetch
...
pub async fn install_skill_from_url(
    ...
    // Install to user scope (`~/.Benito/skills/<slug>`), which `discover_skills`
    // scans unconditionally. Project scope (`<ws>/.Benito/skills/`) is gated on
    // a `<ws>/.Benito/trust` marker and would render the install invisible to the
...
pub struct UninstallSkillParams {
    ...
    /// `~/.Benito/skills/<slug>/`. Retained as `name` for wire-format
...
/// Remove an installed user-scope SKILL.md skill from `~/.Benito/skills/`.
///
...
pub fn uninstall_skill(
    ...
    let skills_root = home.join(".Benito").join("skills");
