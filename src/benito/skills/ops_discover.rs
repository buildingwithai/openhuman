...
/// `<workspace>/.Benito/skills/` instead, but this directory is kept for
...
/// so user-scope skills (`~/.Benito/skills/`, `~/.agents/skills/`) are
...
///   `~/.Benito/skills/` and `~/.agents/skills/`.
...
///
/// Looks for `<workspace>/.Benito/trust`. The marker file's contents are
/// ignored — presence is sufficient.
pub fn is_workspace_trusted(workspace_dir: &Path) -> bool {
    workspace_dir.join(".Benito").join(TRUST_MARKER).exists()
}
...
fn project_roots(workspace: &Path) -> Vec<PathBuf> {
    vec![
        workspace.join(".Benito").join("skills"),
...
/// `.Benito/trust` marker) and locating the matching entry; this keeps the
