...
///
/// Scope resolution:
/// * [`SkillScope::User`] → `~/.Benito/skills/`
/// * [`SkillScope::Project`] → `<workspace>/.Benito/skills/`. Requires the
///   trust marker at `<workspace>/.Benito/trust` to be present; otherwise
    ...
    let scope_root = match params.scope {
        ...
        SkillScope::Project => {
            if !is_workspace_trusted(workspace_dir) {
                return Err(format!(
                    "workspace {} is not trusted; create {}/.Benito/trust to enable project-scope skills",
                    workspace_dir.display(),
                    workspace_dir.display(),
                ));
            }
            workspace_dir.join(".Benito").join("skills")
        }
        SkillScope::Legacy => {
            return Err(
