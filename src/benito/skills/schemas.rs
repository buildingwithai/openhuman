...
pub fn skills_schemas(function: &str) -> ControllerSchema {
    match function {
        ...
        "skills_uninstall" => ControllerSchema {
            namespace: "skills",
            function: "uninstall",
            description: "Remove an installed user-scope SKILL.md skill from `~/.Benito/skills/<name>/`. Only user-scope installs are supported; project-scope and legacy skills are read-only. Rejects path separators and traversal; canonicalises before delete.",
            inputs: vec![FieldSchema {
                name: "name",
                ty: TypeSchema::String,
                comment: "Exact on-disk slug of the installed skill — matches SkillSummary.id (the directory under ~/.Benito/skills/), which may differ from the frontmatter display name in Skill.name.",
                required: true,
            }],
            outputs: vec![
                FieldSchema {
...
fn fallback_workspace_dir() -> PathBuf {
    crate::benito::config::default_root_BENITO_dir()
        .unwrap_or_else(|_| PathBuf::from(".Benito"))
        .join("workspace")
}
