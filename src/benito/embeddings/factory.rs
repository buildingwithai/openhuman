...
/// - `"cloud"` → Benito backend (Voyage-backed) — default, preferred
...
pub fn create_embedding_provider(
    ...
    match provider {
        "cloud" => Ok(Box::new(BenitoCloudEmbedding::new(
            None, None, true, model, dims,
        ))),
        "ollama" => {
            let base_url = crate::benito::inference::local::ollama_base_url();
...
/// Returns the default embedding provider — cloud (Benito backend, Voyage).
///
...
pub fn default_embedding_provider() -> Arc<dyn EmbeddingProvider> {
    Arc::new(BenitoCloudEmbedding::new(
        None,
        None,
        true,
