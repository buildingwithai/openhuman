...
use crate::benito::autocomplete::ops::AutocompleteHistoryParams;
use crate::benito::autocomplete::{
...
fn handle_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async { to_json(crate::benito::autocomplete::rpc::autocomplete_status().await?) })
}

fn handle_start(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let payload = deserialize_params::<AutocompleteStartParams>(params)?;
        to_json(crate::benito::autocomplete::rpc::autocomplete_start(payload).await?)
    })
}
...
fn handle_stop(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::autocomplete::rpc::autocomplete_stop(payload).await?)
    })
}
...
fn handle_current(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::autocomplete::rpc::autocomplete_current(payload).await?)
    })
}

fn handle_debug_focus(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async {
        to_json(crate::benito::autocomplete::rpc::autocomplete_debug_focus().await?)
    })
}
...
fn handle_accept(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::autocomplete::rpc::autocomplete_accept(payload).await?)
    })
}
...
fn handle_set_style(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::autocomplete::rpc::autocomplete_set_style(payload).await?)
    })
}
...
fn handle_history(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::autocomplete::rpc::autocomplete_history(payload).await?)
    })
}

fn handle_clear_history(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async {
        to_json(crate::benito::autocomplete::rpc::autocomplete_clear_history().await?)
    })
}
