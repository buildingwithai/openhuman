...
use crate::benito::inference::local as local_ai;
use chrono::Utc;
...
impl AutocompleteEngine {
    ...
    pub async fn start(
        ...
        PRECOMPILE_ONCE.call_once(|| {
            crate::benito::accessibility::precompile_helper_background();
        });
                ...
                if last_refresh.elapsed() >= Duration::from_millis(current_debounce_ms) {
                    ...
                    match refresh_result {
                        Ok(Ok(Err(err))) => {
                            ...
                            if is_perm_denied {
                                crate::benito::accessibility::mark_system_events_denied();
                            }
    ...
    pub async fn accept(
        ...
        // Dual-write: KV (UI list) + local docs (semantic search).
        {
            ...
            tokio::spawn(async move {
                crate::benito::autocomplete::history::save_accepted_completion(
                    &ctx,
                    &sug,
                    app.as_deref(),
                )
                .await;
                crate::benito::autocomplete::history::save_completion_to_local_docs(
                    &ctx,
                    &sug,
                    app.as_deref(),
                )
    ...
    async fn refresh(&self, context_override: Option<String>) -> Result<(), String> {
        ...
        let focused = if let Some(context) = context_override {
            FocusedTextContext {
                app_name: Some("Benito".to_string()),
                role: None,
                text: context,
                selected_text: None,
                raw_error: None,
        ...
        // When Benito itself is focused AND this is the background engine loop,
        ...
        if !is_in_app && app_lower.contains("Benito") {
            let mut state = self.inner.lock().await;
            state.app_name = focused.app_name;
            state.phase = "idle".to_string();
            state.last_error = None;
        ...
        let (relevant_examples, recent_examples) = if is_in_app {
            ...
            let relevant_future =
                crate::benito::autocomplete::history::query_relevant_examples(&context, 4);
            let recent_future = crate::benito::autocomplete::history::load_recent_examples(4);
...
pub async fn start_if_enabled(app_config: &Config) {
    ...
    crate::benito::accessibility::clear_automation_denial();
