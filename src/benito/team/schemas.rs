...
fn handle_team_get_usage(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::team::get_usage(&config).await?)
    })
}
...
fn handle_team_list_members(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::team::list_members(&config, &payload.team_id).await?)
    })
}
...
fn handle_team_list_teams(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::benito::team::list_teams(&config).await?)
    })
}
...
fn handle_team_get_team(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let payload = deserialize_params::<TeamIdParams>(params)?;
        to_json(crate::benito::team::get_team(&config, &payload.team_id).await?)
    })
}
...
fn handle_team_create_team(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::team::create_team(&config, &payload.name).await?)
    })
}
...
fn handle_team_update_team(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::team::update_team(&config, &payload.team_id, payload.name.as_deref())
                .await?,
        )
    })
}
...
fn handle_team_delete_team(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let payload = deserialize_params::<TeamIdParams>(params)?;
        to_json(crate::benito::team::delete_team(&config, &payload.team_id).await?)
    })
}
...
fn handle_team_switch_team(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let payload = deserialize_params::<TeamIdParams>(params)?;
        to_json(crate::benito::team::switch_team(&config, &payload.team_id).await?)
    })
}
...
fn handle_team_leave_team(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let payload = deserialize_params::<TeamIdParams>(params)?;
        to_json(crate::benito::team::leave_team(&config, &payload.team_id).await?)
    })
}
...
fn handle_team_join_team(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::team::join_team(&config, &payload.code).await?)
    })
}
...
fn handle_team_create_invite(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::team::create_invite(
                &config,
                &payload.team_id,
                payload.max_uses,
...
fn handle_team_remove_member(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::team::remove_member(&config, &payload.team_id, &payload.user_id)
                .await?,
        )
    })
}
...
fn handle_team_change_member_role(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::team::change_member_role(
                &config,
                &payload.team_id,
                &payload.user_id,
                &payload.role,
...
fn handle_team_list_invites(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let payload = deserialize_params::<TeamIdParams>(params)?;
        to_json(crate::benito::team::list_invites(&config, &payload.team_id).await?)
    })
}
...
fn handle_team_revoke_invite(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::team::revoke_invite(&config, &payload.team_id, &payload.invite_id)
                .await?,
        )
    })
}
