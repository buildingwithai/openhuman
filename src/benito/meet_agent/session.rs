...
impl MeetAgentSession {
    ...
    /// substring on `"hey Benito"` (and `"hey open human"` to
    ...
    pub fn note_caption(&mut self, speaker: &str, text: &str, ts_ms: u64) -> bool {
        ...
        // phrase ("hey, Benito"), capitalizes mid-sentence, and
        ...
        if let Some(idx) = wake_idx {
            let after = if normalized[idx..].starts_with("hey Benito") {
                idx + "hey Benito".len()
            } else {
...
/// it ("Hey, Benito", "hey open-human", etc).
...
mod tests {
    ...
    fn note_caption_handles_punctuated_wake() {
        ...
        let fired = s.note_caption("Alice", "Hey, Benito remember the launch", 1);
    ...
    fn note_caption_does_not_double_fire_on_growing_caption() {
        let mut s = MeetAgentSession::new("p".into(), 16_000);
        let first = s.note_caption("Alice", "hey Benito take notes", 1);
        assert!(first);
        let second = s.note_caption("Alice", "hey Benito take notes about the launch", 2);
        assert!(!second, "second caption while wake_active must not refire");
        let prompt = s.take_pending_prompt().expect("prompt drained");
        // First wake stripped "hey Benito"; the continuation
        // appended the WHOLE growing caption (still containing "hey
        // Benito" because we don't re-strip), separated by a
