...
//!    to [`crate::benito::voice::cloud_transcribe`]. Returns the
...
//!    [`crate::benito::voice::reply_speech`] requesting
...
/// wake word fires so multi-caption utterances ("hey Benito …
...
pub async fn run_caption_turn(request_id: &str) -> Result<bool, String> {
    // Wait briefly so a multi-fragment wake utterance ("hey Benito
...
async fn stt(samples: &[i16]) -> Result<String, String> {
    use crate::benito::voice::cloud_transcribe::{transcribe_cloud, CloudTranscribeOptions};

    let config = crate::benito::config::ops::load_config_with_timeout().await?;
...
You are Benito, an AI assistant joining a live Google Meet call as a participant. \
...
the speaker addresses you by name (\"Benito\", \"hey Benito\"), asks a direct \
...
async fn tts(text: &str) -> Result<Vec<i16>, String> {
    use crate::benito::voice::reply_speech::{synthesize_reply, ReplySpeechOptions};
...
mod tests {
    use super::*;
    use crate::benito::meet_agent::session::registry;
