...
fn doctor_model_targets() -> Vec<String> {
    crate::benito::inference::provider::list_providers()
        .into_iter()
...
fn check_daemon_state(config: &Config, items: &mut Vec<DiagnosticItem>) {
    let cat = "daemon";
    let state_file = crate::benito::service::daemon::state_file_path(config);
...
fn check_memory_tree_db(config: &Config, items: &mut Vec<DiagnosticItem>) {
    ...
    // ── Probe connection ─────────────────────────────────────────────
    match crate::benito::memory::tree::store::with_connection(config, |conn| {
