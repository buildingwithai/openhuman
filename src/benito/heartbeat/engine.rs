use crate::benito::config::{Config, HeartbeatConfig};
use crate::benito::subconscious::global::get_or_init_engine;
use anyhow::Result;
use std::path::Path;
...
impl HeartbeatEngine {
    ...
    async fn run_event_planner_tick_for_config(&self, config: &Config) {
        ...
        let summary =
            crate::benito::heartbeat::planner::evaluate_and_dispatch(config, chrono::Utc::now())
                .await;
        tracing::debug!(
...
mod tests {
    ...
    async fn ensure_heartbeat_file_creates_file_with_defaults() {
        let dir = std::env::temp_dir().join("BENITO_test_heartbeat_defaults");
    ...
    async fn ensure_heartbeat_file_does_not_overwrite() {
        let dir = std::env::temp_dir().join("BENITO_test_heartbeat_no_overwrite");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
