...
async fn handles_trigger_event_without_panic() {
    ...
    // process-global `BENITO_WORKSPACE`, so without isolation it
    ...
    // point `BENITO_WORKSPACE` at an isolated, persisted config.
    use crate::benito::config::{Config, TEST_ENV_LOCK};
...
fn direct_mode_constant_matches_trigger_drop_check() {
    ...
    use crate::benito::config::ComposioConfig;
    assert_eq!(
...
async fn trigger_subscriber_skips_triage_when_env_disabled() {
    ...
    // this test must isolate `BENITO_WORKSPACE` under `TEST_ENV_LOCK`
...
async fn handles_connection_created_event_without_panic() {
    ...
    // again). Those reads hit the process-global `BENITO_WORKSPACE`.
    ...
    // with a default. Hold `TEST_ENV_LOCK`, point `BENITO_WORKSPACE`
    ...
    // `BENITO_WORKSPACE` points at the isolated dir, so it can never
...
async fn connection_subscriber_skips_when_no_provider_registered() {
    ...
    // hold `TEST_ENV_LOCK`, point `BENITO_WORKSPACE` at a leaked
