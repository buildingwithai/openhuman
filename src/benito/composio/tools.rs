//! Agent-facing tools that proxy through the Benito backend's
...
//! `Tool` trait plumbing in [`crate::benito::tools`].
//!
...
pub struct ComposioListToolkitsTool {
    ...
    /// [`crate::benito::config::ComposioConfig::mode`] toggle is
...
pub struct ComposioListToolsTool {
    ...
    /// [`crate::benito::composio::ops::composio_execute`] and avoids
...
pub struct ComposioExecuteTool {
    ...
    /// [`crate::benito::composio::ops::composio_execute`]. See
    /// issue #1710.
    config: Arc<Config>,
}
...
pub fn all_composio_agent_tools(config: &crate::benito::config::Config) -> Vec<Box<dyn Tool>> {
    ...
    if !crate::benito::agent::harness::subagent_runner::user_is_signed_in_to_composio(config) {
        tracing::debug!(
