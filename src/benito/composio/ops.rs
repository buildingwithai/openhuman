...
///
/// The shared [`crate::benito::integrations::IntegrationClient`]
...
/// shape in the leak set (Benito-TAURI-35 / -2H: backend 502 from
...
/// signal to drop the dominant 5xx leak shape (Benito-TAURI-35 / -2H)
...
pub async fn composio_list_tools(
    ...
    match kind {
        ...
        ComposioClientKind::Direct(direct) => {
            ...
            // Benito's curation policy isn't, and direct-mode users
...
/// Apply Benito's curated-whitelist + user-scope visibility filter to
...
/// `Benito.composio_get_user_profile` — fetch a normalized user
...
/// `Benito.composio_refresh_all_identities` — re-fetch the user
...
/// `Benito.composio_sync` — run a sync pass for a connected account
...
use crate::benito::context::prompt::{ConnectedIntegration, ConnectedIntegrationTool};
...
/// resolves to the per-user Benito dir).
...
pub async fn composio_get_mode(config: &Config) -> OpResult<RpcOutcome<serde_json::Value>> {
    ...
    let key_present = crate::benito::credentials::get_composio_api_key(config)
...
pub async fn composio_set_api_key(
    ...
    crate::benito::credentials::store_composio_api_key(config, trimmed)
        .await
    ...
    let mode_log = if activate_direct {
        ...
        // all observe it.
        let mut cfg_mut = crate::benito::config::rpc::load_config_with_timeout()
            .await
            .map_err(|e| format!("[composio-direct] reload config failed: {e}"))?;
        cfg_mut.composio.mode = crate::benito::config::schema::COMPOSIO_MODE_DIRECT.into();
        cfg_mut
            .save()
            .await
...
pub async fn composio_clear_api_key(config: &Config) -> OpResult<RpcOutcome<serde_json::Value>> {
    ...
    crate::benito::credentials::clear_composio_api_key(config)
        .await
    ...
    let mut cfg_mut = crate::benito::config::rpc::load_config_with_timeout()
        .await
        .map_err(|e| format!("[composio-direct] reload config failed: {e}"))?;
    cfg_mut.composio.mode = crate::benito::config::schema::COMPOSIO_MODE_BACKEND.into();
    cfg_mut
        .save()
        .await
        .map_err(|e| format!("[composio-direct] save config failed: {e}"))?;
