...
async fn authorize_rejects_empty_toolkit() {
    let inner = Arc::new(crate::benito::integrations::IntegrationClient::new(
...
fn config_with_session_token(tmp: &tempfile::TempDir) -> crate::benito::config::Config {
...
fn store_get_clear_composio_api_key_roundtrip() {
    use crate::benito::credentials::{get_composio_api_key, COMPOSIO_DIRECT_PROVIDER};
...
fn direct_tool_for_test() -> std::sync::Arc<crate::benito::tools::ComposioTool> {
    std::sync::Arc::new(crate::benito::tools::ComposioTool::new(
        "ck_test_direct",
        Some("default"),
        std::sync::Arc::new(crate::benito::security::SecurityPolicy::default()),
    ))
}
...
async fn pricing_for_config_short_circuits_in_direct_mode() {
    ...
    let client = crate::benito::integrations::IntegrationClient::new(
        "http://127.0.0.1:0".into(),
        "test".into(),
    );
    let mut config = crate::benito::config::Config::default();
    ...
    let pricing = crate::benito::integrations::pricing_for_config(&client, &config).await;
