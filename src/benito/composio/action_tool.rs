...
//! [`crate::benito::config::ComposioConfig::mode`] toggle — see
...
use crate::benito::agent::harness::current_sandbox_mode;
use crate::benito::agent::harness::definition::SandboxMode;
use crate::benito::config::rpc as config_rpc;
use crate::benito::config::Config;
use crate::benito::tools::traits::{PermissionLevel, Tool, ToolCategory, ToolResult};
...
pub struct ComposioActionTool {
    ...
    /// the [`crate::benito::config::ComposioConfig::mode`] toggle is
...
mod tests {
    use super::*;
    use crate::benito::agent::harness::with_current_sandbox_mode;
    use std::path::Path;
    ...
    async fn sandbox_unset_leaves_per_action_execute_to_downstream() {
        ...
        // `TEST_ENV_LOCK` and point `BENITO_WORKSPACE` at an
    ...
    // config via `load_config_with_timeout()` (reads `BENITO_WORKSPACE`)
    // and was therefore subject to a parallel-test env-var race: another
    // non-`TEST_ENV_LOCK` test mutating `BENITO_WORKSPACE` in the await
    ...
    fn factory_routes_through_backend_when_mode_is_backend() {
        ...
        let msg = match crate::benito::composio::client::create_composio_client(&config) {
    ...
    fn factory_routes_through_direct_when_mode_is_direct() {
        ...
        assert!(
            matches!(
                kind,
                crate::benito::composio::client::ComposioClientKind::Direct(_)
            ),
    ...
    async fn mode_toggle_between_calls_is_observed() {
        ...
        // rewriting `BENITO_WORKSPACE/config.toml` between the two
        ...
        direct_config.composio.mode =
            crate::benito::config::schema::COMPOSIO_MODE_DIRECT.to_string();
