...
//! `Benito.composio_*`:
//!   - `composio.list_toolkits`       → `Benito.composio_list_toolkits`
//!   - `composio.list_capabilities`   → `Benito.composio_list_capabilities`
//!   - `composio.list_connections`    → `Benito.composio_list_connections`
//!   - `composio.authorize`           → `Benito.composio_authorize`
//!   - `composio.delete_connection`   → `Benito.composio_delete_connection`
//!   - `composio.list_tools`          → `Benito.composio_list_tools`
//!   - `composio.execute`             → `Benito.composio_execute`
//!   - `composio.list_github_repos`   → `Benito.composio_list_github_repos`
//!   - `composio.create_trigger`      → `Benito.composio_create_trigger`
//!   - `composio.get_user_profile`    → `Benito.composio_get_user_profile`
//!   - `composio.refresh_all_identities` → `Benito.composio_refresh_all_identities`
//!   - `composio.sync`                → `Benito.composio_sync`
...
pub fn schemas(function: &str) -> ControllerSchema {
    match function {
        ...
        "list_capabilities" => ControllerSchema {
            ...
            description: "List Benito's built-in Composio capability matrix without requiring a signed-in Composio session.",
            inputs: vec![],
            outputs: vec![FieldSchema {
                name: "capabilities",
                ty: TypeSchema::Json,
...
fn handle_set_user_scopes(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let memory = match crate::benito::memory::global::client_if_ready() {
            Some(m) => m,
            None => {
                tracing::error!(
                    method = "composio.set_user_scopes",
