...
impl Drop for WorkspaceEnvGuard {
    fn drop(&mut self) {
        match self.previous.take() {
            Some(prev) => unsafe {
                std::env::set_var("BENITO_WORKSPACE", prev);
            },
            None => unsafe {
...
fn config_with_backend(tmp: &tempfile::TempDir, base: String) -> Config {
    ...
    c.config_path = tmp.path().join("config.toml");
    c.api_url = Some(base);
    crate::benito::credentials::AuthService::from_config(&c)
...
async fn composio_get_user_profile_via_mock_returns_provider_profile() {
    ...
    crate::benito::composio::providers::init_default_providers();
...
async fn composio_sync_gmail_via_mock_archives_raw_email_and_updates_outcome() {
    use crate::benito::config::TEST_ENV_LOCK;
    use crate::benito::memory::tree::content_store::raw::{raw_rel_path, RawKind};
    use crate::benito::memory::tree::rpc::{list_chunks_rpc, ListChunksRequest};
    let _cache_guard = CACHE_TEST_GUARD.lock().unwrap_or_else(|e| e.into_inner());
    let _env_guard = TEST_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    ...
    let _workspace_env_guard = WorkspaceEnvGuard::set(tmp.path());
    config.save().await.unwrap();
    let _ = crate::benito::memory::global::init(config.workspace_dir.clone()).unwrap();
...
fn direct_mode_config(tmp: &tempfile::TempDir) -> Config {
    ...
    c.workspace_dir = tmp.path().join("workspace");
    c.config_path = tmp.path().join("config.toml");
    c.composio.mode = crate::benito::config::schema::COMPOSIO_MODE_DIRECT.into();
    crate::benito::credentials::AuthService::from_config(&c)
        .store_provider_token(
            crate::benito::credentials::ops::COMPOSIO_DIRECT_PROVIDER,
            crate::benito::credentials::DEFAULT_AUTH_PROFILE_NAME,
            "ck_test_direct_key",
            std::collections::HashMap::new(),
            true,
...
fn composio_failure_tag_is_non_2xx_for_backend_returned_502() {
    // Benito-TAURI-35 / -2H wire shape — the dominant leak. The
...
fn composio_failure_tag_is_transport_for_operation_timed_out() {
    // Benito-TAURI-18 / -G shape — `composio/execute` reqwest chain
...
// from a 401, and the dominant leak shape (Benito-TAURI-35 / -2H)
// re-opens.
