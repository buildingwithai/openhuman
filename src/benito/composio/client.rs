//! Thin HTTP wrapper over the Benito backend's
//! `/agent-integrations/composio/*` routes.
//!
//! All calls go through the shared
//! [`crate::benito::integrations::IntegrationClient`] so they inherit
...
use crate::benito::integrations::IntegrationClient;
...
impl ComposioClient {
    ...
    async fn raw_delete<T: serde::de::DeserializeOwned>(&self, path: &str) -> Result<T> {
        ...
        if !status.is_success() {
            ...
            let detail = crate::benito::integrations::client::extract_error_detail(
                &body_text,
                crate::benito::integrations::client::MAX_ERROR_BODY_LEN,
            );
            ...
            let logged_body =
                crate::benito::integrations::client::extract_error_detail(&body_text, 300);
            tracing::debug!(
            ...
            // Benito-TAURI-BC. 4xx user-input / auth-state shapes
        ...
        if !envelope.success {
            ...
            // not found" / "Missing required fields: …" — Benito-TAURI-3R
...
/// [`crate::benito::agent::harness::subagent_runner::user_is_signed_in_to_composio`]
/// (probe) instead.
///
...
/// [`crate::benito::integrations::build_client`]. The only reason
...
pub(super) fn build_composio_client(
    ...
    let inner = crate::benito::integrations::build_client(config)?;
    Some(ComposioClient::new(inner))
}

// ── Direct-mode factory ─────────────────────────────────────────────
//
// Mirrors `src/Benito/embeddings/factory.rs` so anyone reading both
...
use crate::benito::config::schema::{COMPOSIO_MODE_BACKEND, COMPOSIO_MODE_DIRECT};
...
pub enum ComposioClientKind {
    ...
    Direct(Arc<crate::benito::tools::ComposioTool>),
}
...
///   [`crate::benito::credentials::COMPOSIO_DIRECT_PROVIDER`]
...
pub fn create_composio_client(
    ...
    match mode {
        ...
        MODE_DIRECT_PAT => {
            ...
            let stored = crate::benito::credentials::get_composio_api_key(config)
            ...
            // context.
            let security = Arc::new(crate::benito::security::SecurityPolicy::default());
            let tool = crate::benito::tools::ComposioTool::new(
                &api_key,
...
/// [`crate::benito::tools::ComposioTool::get_connection_url`] and
...
/// [`crate::benito::tools::ComposioTool::list_connected_accounts`])
...
/// [`crate::benito::tools::ComposioTool::list_tool_schemas_v3`] and
