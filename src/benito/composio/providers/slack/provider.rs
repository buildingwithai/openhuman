...
use crate::benito::composio::providers::{
...
/// `BENITO_SLACK_BACKFILL_DAYS` env var if set and parseable as a
/// positive integer; falls back to [`BACKFILL_DAYS`] otherwise.
fn backfill_days() -> i64 {
    match std::env::var("BENITO_SLACK_BACKFILL_DAYS") {
        Ok(s) => match s.trim().parse::<i64>() {
            Ok(n) if n >= 1 => n,
            _ => {
                log::warn!(
                    "[composio:slack] BENITO_SLACK_BACKFILL_DAYS={s:?} not a positive integer; \
...
/// Resolve the JSON dump directory from `BENITO_SLACK_DUMP_DIR`.
fn dump_dir() -> Option<PathBuf> {
    std::env::var_os("BENITO_SLACK_DUMP_DIR").map(PathBuf::from)
}
...
impl ComposioProvider for SlackProvider {
    ...
    fn curated_tools(&self) -> Option<&'static [CuratedTool]> {
        Some(crate::benito::composio::providers::catalogs::SLACK_CURATED)
    }
