...
/// `crate::benito::memory::tree::score::extract::EntityKind` so the
...
pub fn persist_provider_profile(profile: &ProviderUserProfile) -> usize {
    let Some(client) = crate::benito::memory::global::client_if_ready() else {
        tracing::debug!(
...
mod tests {
    use super::*;
    use crate::benito::memory::store::profile::{profile_load_all, PROFILE_INIT_SQL};
