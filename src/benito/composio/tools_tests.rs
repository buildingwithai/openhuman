...
impl WorkspaceEnvGuard {
    fn set(path: &Path) -> Self {
        let previous = std::env::var_os("BENITO_WORKSPACE");
        unsafe {
            std::env::set_var("BENITO_WORKSPACE", path);
        }
        Self { previous }
    }
}
    ...
    fn drop(&mut self) {
        unsafe {
            match self.previous.take() {
                Some(value) => std::env::set_var("BENITO_WORKSPACE", value),
                None => std::env::remove_var("BENITO_WORKSPACE"),
            }
        }
    }
}
...
fn fake_config_arc() -> Arc<crate::benito::config::Config> {
...
async fn authorize_tool_execute_rejects_missing_toolkit() {
    ...
    let txt = result
        ...
        .filter_map(|c| match c {
            crate::benito::tools::traits::ToolContent::Text { text } => Some(text.clone()),
            _ => None,
        })
        .collect::<Vec<_>>()
        .join(" ");
...
fn agent_tools_register_when_backend_signed_in() {
    ...
    let mut config = crate::benito::config::Config::default();
    config.config_path = tmp.path().join("config.toml");
    crate::benito::credentials::AuthService::from_config(&config)
        .store_provider_token(
            crate::benito::credentials::APP_SESSION_PROVIDER,
            crate::benito::credentials::DEFAULT_AUTH_PROFILE_NAME,
            "test-token",
...
async fn sandbox_read_only_blocks_write_scope_action() {
    let t = ComposioExecuteTool::new(fake_config_arc());
    let result =
        crate::benito::agent::harness::with_current_sandbox_mode(SandboxMode::ReadOnly, async {
...
async fn sandbox_read_only_passes_through_read_scope_actions_to_downstream_gates() {
    ...
    // `TEST_ENV_LOCK` and point `BENITO_WORKSPACE` at an isolated,
    ...
    use crate::benito::config::TEST_ENV_LOCK;
...
async fn sandbox_unset_leaves_all_scopes_to_downstream_gates() {
    ...
    // and point `BENITO_WORKSPACE` at an isolated, persisted config
...
async fn sandbox_sandboxed_mode_does_not_trigger_readonly_gate() {
    ...
    let t = ComposioExecuteTool::new(Arc::new(config));
    let result = crate::benito::agent::harness::with_current_sandbox_mode(
...
fn render_tools_markdown_groups_by_toolkit_and_drops_schemas() {
    use crate::benito::composio::types::{
...
fn render_tools_markdown_handles_empty_response() {
    use crate::benito::composio::types::ComposioToolsResponse;
...
fn direct_mode_config() -> crate::benito::config::Config {
...
fn execute_tool_resolves_to_direct_kind_when_mode_is_direct() {
    ...
    let kind = crate::benito::composio::client::create_composio_client(&config)
        .expect("direct mode with inline api_key must resolve");
    assert_eq!(
        kind.mode(),
        crate::benito::config::schema::COMPOSIO_MODE_DIRECT,
...
fn execute_tool_resolves_to_backend_kind_when_mode_is_backend() {
    ...
    assert_eq!(
        kind.mode(),
        crate::benito::config::schema::COMPOSIO_MODE_BACKEND,
...
async fn execute_tool_per_call_factory_means_no_baked_client() {
    ...
    // per call which reads from `~/.Benito/config.toml` (or the
    // workspace pointed at by `BENITO_WORKSPACE`). To isolate the
    // test from the dev's real config we hold `TEST_ENV_LOCK`, point
    // `BENITO_WORKSPACE` at a tempdir, and persist the test's
