//! `Benito text-input` — standalone CLI for text input intelligence.
//!
...
//!
//! Usage:
//!   Benito text-input run       [--port <u16>] [-v]
//!   Benito text-input read      [-v] [--bounds]
//!   Benito text-input insert    <text> [-v]
//!   Benito text-input ghost     <text> [--ttl <ms>] [-v]
//!   Benito text-input dismiss   [-v]

use anyhow::Result;

/// Entry point for `Benito text-input <subcommand>`.
pub(crate) fn run_text_input_command(args: &[String]) -> Result<()> {
    ...
    match args[0].as_str() {
        ...
        other => Err(anyhow::anyhow!(
            "unknown text-input subcommand '{other}'. Run `Benito text-input --help`."
        )),
    }
}
...
/// `Benito text-input run` — start a minimal JSON-RPC server.
fn run_server(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) {
        println!("Usage: Benito text-input run [--port <u16>] [-v]");
        println!();
...
/// `Benito text-input read` — one-shot read of the focused field.
fn run_read(args: &[String]) -> Result<()> {
    if args.iter().any(|a| is_help(a)) {
        println!("Usage: Benito text-input read [--bounds] [-v]");
        println!();
    ...
    rt.block_on(async {
        let params = crate::benito::text_input::ReadFieldParams {
            include_bounds: Some(opts.include_bounds),
        };
        let outcome = crate::benito::text_input::rpc::read_field(params)
            .await
...
/// `Benito text-input insert <text>` — insert text into the focused field.
fn run_insert(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) || rest.is_empty() {
        println!("Usage: Benito text-input insert <text> [-v]");
        println!();
    ...
    rt.block_on(async {
        let params = crate::benito::text_input::InsertTextParams {
            text,
            validate_focus: None,
            expected_app: None,
            expected_role: None,
        };
        let outcome = crate::benito::text_input::rpc::insert_text(params)
            .await
            .map_err(|e| anyhow::anyhow!(e))?;
...
/// `Benito text-input ghost <text>` — show ghost text overlay.
fn run_ghost(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) || rest.is_empty() {
        println!("Usage: Benito text-input ghost <text> [--ttl <ms>] [-v]");
        println!();
    ...
    rt.block_on(async {
        let params = crate::benito::text_input::ShowGhostTextParams {
            text,
            ttl_ms: Some(opts.ttl_ms),
            bounds: None,
        };
        let outcome = crate::benito::text_input::rpc::show_ghost(params)
            .await
            .map_err(|e| anyhow::anyhow!(e))?;
...
/// `Benito text-input dismiss` — dismiss the ghost text overlay.
fn run_dismiss(args: &[String]) -> Result<()> {
    if args.iter().any(|a| is_help(a)) {
        println!("Usage: Benito text-input dismiss [-v]");
        println!();
    ...
    rt.block_on(async {
        let outcome = crate::benito::text_input::rpc::dismiss_ghost()
            .await
            .map_err(|e| anyhow::anyhow!(e))?;
...
fn print_help() {
    println!("Benito text-input — text input intelligence\n");
    println!("Usage:");
    println!("  Benito text-input run       [--port <u16>] [-v]");
    println!("  Benito text-input read      [--bounds] [-v]");
    println!("  Benito text-input insert    <text> [-v]");
    println!("  Benito text-input ghost     <text> [--ttl <ms>] [-v]");
    println!("  Benito text-input dismiss   [-v]");
    println!();
    println!("Subcommands:");
