...
use crate::benito::config::rpc as config_rpc;
...
struct CloudProviderUpdate {
    ...
    label: Option<String>,
    endpoint: String,
    /// Auth style: "bearer" | "anthropic" | "BENITO_jwt" | "none".
    #[serde(default)]
...
struct ModelSettingsUpdate {
    /// Benito product backend URL. Used for auth, billing, voice, and
    ...
    /// through the Benito backend. Send an empty string to clear.
    ...
    /// the user switches back to the Benito backend whose built-in router
...
pub fn schemas(function: &str) -> ControllerSchema {
    match function {
        ...
        "get_client_config" => ControllerSchema {
            ...
            outputs: vec![
                FieldSchema {
                    ...
                    comment: "Configured Benito product backend URL, if any.",
                    required: false,
                },
                FieldSchema {
                    name: "inference_url",
                ...
                FieldSchema {
                    name: "model_routes",
                    ty: TypeSchema::Json,
                    comment: "Persisted task-hint -> model id pairs the core router will obey. Empty when the Benito built-in router is active.",
                    required: true,
                },
            ],
        },
        "update_model_settings" => ControllerSchema {
            ...
            inputs: vec![
                optional_string("api_url", "Benito product backend URL (auth/billing/voice). Almost always left blank; the inference URL is a separate `inference_url` field."),
                optional_string("inference_url", "Custom OpenAI-compatible LLM endpoint. When set together with `api_key`, inference goes direct to this URL instead of the Benito backend. Pass an empty string to clear."),
                ...
                FieldSchema {
                    ...
                    comment: "Optional list of {hint, model} pairs mapping task hints (reasoning, agentic, coding, summarization) to provider-specific model ids. Replaces config.model_routes wholesale; send [] to clear (e.g. when switching back to the Benito built-in router).",
                    required: false,
                },
                FieldSchema {
        ...
        "set_browser_allow_all" => ControllerSchema {
            ...
            description: "Set BENITO_BROWSER_ALLOW_ALL runtime flag.",
        ...
        "reset_local_data" => ControllerSchema {
            ...
            description:
                "Delete local Benito data for the active config/workspace so the next restart boots clean.",
            inputs: vec![],
        ...
        "get_data_paths" => ControllerSchema {
            ...
            description:
                "Resolve the Benito data directories (current workspace, default ~/.Benito, active workspace marker) that reset_local_data would remove. Read-only — performs no filesystem changes.",
            inputs: vec![],
            outputs: vec![json_output(
                "paths",
                "Resolved data paths: current_BENITO_dir, default_BENITO_dir, active_workspace_marker_path.",
            )],
        },
    ...
    Box::pin(async move {
        ...
        let patch = config_rpc::ModelSettingsPatch {
            ...
            model_routes: update.model_routes.map(|routes| {
                routes
                    .into_iter()
                    .map(|r| crate::benito::config::ModelRouteConfig {
                        hint: r.hint,
                        model: r.model,
                    })
                    .collect()
            ...
            cloud_providers: update
                .cloud_providers
                .map(|entries| {
                    use crate::benito::config::schema::cloud_providers::{
                    ...
                    entries
                        ...
                        .map(|e| {
                                ...
                                .as_str()
                            {
                                ...
                                "BENITO_jwt" | "Benitojwt" => AuthStyle::BenitoJwt,
                                "none" => AuthStyle::None,
                                other => {
                                    return Err(format!(
                                        "unknown auth_style '{}'; valid: bearer, anthropic, BENITO_jwt, none",
                                        other
                                    ))
                                }
                            };
