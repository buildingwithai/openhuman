...
async fn reset_local_data_removes_current_dir_default_dir_and_marker() {
    ...
    let default_BENITO_dir = temp.path().join("default-Benito");
    let current_BENITO_dir = temp.path().join("custom-Benito");
    let marker = active_workspace_marker_path(&default_BENITO_dir);

    tokio::fs::create_dir_all(default_BENITO_dir.join("workspace"))
        .await
        .unwrap();
    tokio::fs::create_dir_all(current_BENITO_dir.join("workspace"))
        .await
        .unwrap();
    tokio::fs::write(&marker, "config_dir = '/tmp/custom-Benito'\n")
        .await
        .unwrap();

    let outcome = reset_local_data_for_paths(&current_BENITO_dir, &default_BENITO_dir)
        .await
        .unwrap();

    assert!(!current_BENITO_dir.exists());
    assert!(!default_BENITO_dir.exists());
    assert!(outcome
        .value
        .get("removed_paths")
...
fn core_rpc_url_from_env_uses_override_when_set() {
    let _g = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    unsafe {
        std::env::set_var("benito_core_RPC_URL", "http://1.2.3.4:9999/rpc");
    }
...
#[test]
fn fallback_workspace_dir_ends_in_workspace_under_Benito() {
    let p = fallback_workspace_dir();
    assert!(p.ends_with("workspace"));
    assert!(p
        .parent()
        .map(|d| d.ends_with(".Benito"))
        .unwrap_or(false));
}

#[test]
fn default_BENITO_dir_ends_in_dot_Benito() {
    let p = default_BENITO_dir();
    assert!(p.ends_with(".Benito"));
}

#[test]
fn active_workspace_marker_path_is_under_default_dir() {
    let default_dir = std::path::Path::new("/tmp/Benito-test");
    let marker = active_workspace_marker_path(default_dir);
    assert_eq!(marker, default_dir.join("active_workspace.toml"));
}

#[test]
fn config_BENITO_dir_returns_config_path_parent() {
    let mut cfg = Config::default();
    cfg.config_path = PathBuf::from("/tmp/xyz/config.toml");
    assert_eq!(config_BENITO_dir(&cfg), PathBuf::from("/tmp/xyz"));
}
...
fn reset_local_data_remove_error_explains_windows_file_locks() {
    ...
    let msg =
        reset_local_data_remove_error(std::path::Path::new("C:\\Users\\me\\.Benito"), &err);

    assert!(msg.contains("locked by another Benito window or process"));
    assert!(msg.contains("Close all Benito windows and try again"));
}
...
fn set_browser_allow_all_toggles_env_var() {
    let _g = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    let before = std::env::var("BENITO_BROWSER_ALLOW_ALL").ok();

    let _ = set_browser_allow_all(true);
    assert!(env_flag_enabled("BENITO_BROWSER_ALLOW_ALL"));
    ...
    unsafe {
        match before {
            Some(v) => std::env::set_var("BENITO_BROWSER_ALLOW_ALL", v),
            None => std::env::remove_var("BENITO_BROWSER_ALLOW_ALL"),
        }
    }
}
...
async fn apply_model_settings_replaces_model_routes_when_some_and_keeps_when_none() {
    ...
    // Benito sends an empty vec to wipe them. Omitting the field leaves
    // existing routes alone.
    use crate::benito::config::ModelRouteConfig;
    let tmp = tempdir().unwrap();
    let mut cfg = tmp_config(&tmp);
...
async fn apply_memory_settings_updates_all_provided_fields() {
    ...
    assert_eq!(
        ...
        Some(crate::benito::config::schema::MemoryContextWindow::Extended)
    );
}
...
async fn apply_memory_settings_ignores_unknown_memory_window_label() {
    let tmp = tempdir().unwrap();
    let mut cfg = tmp_config(&tmp);
    cfg.agent.memory_window = Some(crate::benito::config::schema::MemoryContextWindow::Balanced);
...
async fn apply_memory_settings_round_trips_all_window_labels() {
    use crate::benito::config::schema::MemoryContextWindow;
    let tmp = tempdir().unwrap();
    let mut cfg = tmp_config(&tmp);
...
async fn load_and_apply_dictation_settings_rejects_invalid_activation_mode() {
    ...
    let tmp = tempdir().unwrap();
    unsafe {
        std::env::set_var("BENITO_WORKSPACE", tmp.path());
    }
    ...
    unsafe {
        std::env::remove_var("BENITO_WORKSPACE");
    }
}
