...
use crate::benito::config::Config;
use crate::benito::screen_intelligence;
...
pub fn core_rpc_url_from_env() -> String {
    std::env::var("benito_core_RPC_URL")
...
/// `parse_config_with_recovery` (see `src/Benito/config/schema/load.rs`)
...
/// Returns the default workspace directory fallback (~/.Benito/workspace).
fn fallback_workspace_dir() -> PathBuf {
    crate::benito::config::default_root_BENITO_dir()
        .unwrap_or_else(|_| env_scoped_fallback_root_dir())
        .join("workspace")
}

/// Returns the default Benito configuration directory (~/.Benito).
fn default_BENITO_dir() -> PathBuf {
    crate::benito::config::default_root_BENITO_dir()
        .unwrap_or_else(|_| env_scoped_fallback_root_dir())
}
...
fn env_scoped_fallback_root_dir() -> PathBuf {
    ...
    PathBuf::from(format!(".Benito{suffix}"))
}

/// Returns the path to the active workspace marker file.
fn active_workspace_marker_path(default_BENITO_dir: &Path) -> PathBuf {
    default_BENITO_dir.join("active_workspace.toml")
}

/// Returns the parent directory of the config file.
fn config_BENITO_dir(config: &Config) -> PathBuf {
    config
        .config_path
        .parent()
...
fn reset_local_data_remove_error(path: &Path, error: &std::io::Error) -> String {
    if is_windows_file_lock_error(error) {
        ...
        return format!(
            "Failed to remove {} because it is locked by another Benito window or process. Close all Benito windows and try again. ({error})",
            path.display()
        );
    }
...
fn reset_local_data_marker_remove_error(path: &Path, error: &std::io::Error) -> String {
    if is_windows_file_lock_error(error) {
        ...
        return format!(
            "Failed to remove active workspace marker {} because it is locked by another Benito window or process. Close all Benito windows and try again. ({error})",
            path.display()
        );
    }
...
async fn reset_local_data_for_paths(
    current_BENITO_dir: &Path,
    default_BENITO_dir: &Path,
) -> Result<RpcOutcome<serde_json::Value>, String> {
    let active_workspace_marker = active_workspace_marker_path(default_BENITO_dir);
    tracing::debug!(
        current_dir = %current_BENITO_dir.display(),
        default_dir = %default_BENITO_dir.display(),
    ...
    for target_dir in [current_BENITO_dir, default_BENITO_dir] {
    ...
    Ok(RpcOutcome::new(
        json!({
            ...
            "current_BENITO_dir": current_BENITO_dir.display().to_string(),
            "default_BENITO_dir": default_BENITO_dir.display().to_string(),
        }),
        vec![
            format!(
                "reset local data for active config dir {}",
                current_BENITO_dir.display()
            ),
            format!(
                "removed default data dir {} if present",
                default_BENITO_dir.display()
            ),
        ],
    ))
}
...
pub fn client_config_json(config: &Config) -> serde_json::Value {
    let app_version =
        std::env::var("BENITO_APP_VERSION").unwrap_or_else(|_| "unknown".to_string());
...
pub struct ModelSettingsPatch {
    ...
    /// override (inference falls back through the Benito backend).
    ...
    /// (e.g. when switching back to the Benito backend whose built-in
    /// router picks per-task models on its own). Leave `None` to keep the
    /// current routes untouched.
    pub model_routes: Option<Vec<crate::benito::config::ModelRouteConfig>>,
    ...
    /// `auth-profiles.json` via [`crate::benito::credentials::AuthService`]).
    /// Pass `Some(vec![])` to clear all third-party cloud providers.
    pub cloud_providers:
        Option<Vec<crate::benito::config::schema::cloud_providers::CloudProviderCreds>>,
    /// Id of the `cloud_providers` entry used when a workload routes to
    /// `"cloud"`. Empty string clears (factory falls back to Benito).
...
pub struct MemorySettingsPatch {
    ...
    /// [`crate::benito::config::schema::agent::MemoryContextWindow`]).
...
pub async fn apply_model_settings(
    ...
    if let Some(routes) = update.model_routes {
        ...
        // (or an empty vec when switching back to the Benito in-built router).
    ...
    crate::benito::memory::tree::jobs::ensure_reembed_backfill(config);
    let snapshot = snapshot_config_json(config)?;
    Ok(RpcOutcome::new(
        snapshot,
        vec![format!(
...
pub async fn apply_memory_settings(
    ...
    if let Some(window_label) = update.memory_window.as_deref() {
        if let Some(window) =
            crate::benito::config::schema::MemoryContextWindow::from_str_opt(window_label)
        {
...
pub async fn apply_local_ai_settings(
    ...
    if let Some(provider) = update.provider {
        config.local_ai.provider =
            crate::benito::inference::local::provider::normalize_provider(&provider);
    }
...
pub fn get_runtime_flags() -> RpcOutcome<RuntimeFlagsOut> {
    RpcOutcome::single_log(
        RuntimeFlagsOut {
            browser_allow_all: env_flag_enabled("BENITO_BROWSER_ALLOW_ALL"),
            log_prompts: env_flag_enabled("BENITO_LOG_PROMPTS"),
        },
        "runtime flags read",
    )
}

/// Updates the `BENITO_BROWSER_ALLOW_ALL` environment flag.
///
...
pub fn set_browser_allow_all(enabled: bool) -> RpcOutcome<RuntimeFlagsOut> {
    let was_enabled = env_flag_enabled("BENITO_BROWSER_ALLOW_ALL");
    if enabled {
        std::env::set_var("BENITO_BROWSER_ALLOW_ALL", "1");
    } else {
        std::env::remove_var("BENITO_BROWSER_ALLOW_ALL");
    }
    let now_enabled = env_flag_enabled("BENITO_BROWSER_ALLOW_ALL");
...
/// cron job via [`crate::benito::cron::seed::seed_proactive_agents`].
...
pub async fn set_onboarding_completed(value: bool) -> Result<RpcOutcome<bool>, String> {
    ...
    if value && !was_completed {
        ...
        tokio::task::spawn_blocking(move || {
            if let Err(e) = crate::benito::cron::seed::seed_proactive_agents(&seed_config) {
        ...
        match mode.as_str() {
            "tap" => {
                config.voice_server.activation_mode =
                    crate::benito::config::VoiceActivationMode::Tap;
            }
            "push" => {
                config.voice_server.activation_mode =
                    crate::benito::config::VoiceActivationMode::Push;
            }
            _ => {
                return Err(format!(
...
pub fn agent_server_status() -> RpcOutcome<serde_json::Value> {
    let running = crate::benito::service::mock::mock_agent_running().unwrap_or(true);
...
/// `ERROR_SHARING_VIOLATION` (os error 32) — see Benito-TAURI-AF.
///
...
pub async fn reset_local_data() -> Result<RpcOutcome<serde_json::Value>, String> {
    let config = load_config_with_timeout().await?;
    let current_BENITO_dir = config_BENITO_dir(&config);
    let default_BENITO_dir = default_BENITO_dir();
    reset_local_data_for_paths(&current_BENITO_dir, &default_BENITO_dir).await
}
...
/// workspace dir, the default `~/.Benito` dir (which can differ when
/// `BENITO_WORKSPACE` is set or a staging build is in use), and the
...
/// handles. See Benito-TAURI-AF for the Windows file-locking failure
