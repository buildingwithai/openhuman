...
//! `"my-deepseek"`). The factory in `crate::benito::inference::provider::factory`
...
pub enum AuthStyle {
    ...
    Anthropic,
    /// Benito session JWT (injected by the backend provider, not stored here).
    BenitoJwt,
...
/// [`crate::benito::inference::provider::factory::auth_key_for_slug`].
///
/// ## Back-compat
///
...
pub fn is_slug_reserved(s: &str) -> bool {
    matches!(s.trim(), "" | "cloud" | "Benito" | "pid")
}
...
pub fn migrate_legacy_fields(entry: &mut CloudProviderCreds) {
    ...
    if entry.auth_style == AuthStyle::Bearer {
        match lt {
            ...
            "Benito" => {
                entry.auth_style = AuthStyle::BenitoJwt;
            }
            _ => {}
        }
    }
...
impl CloudProviderType {
    ...
    pub fn default_endpoint(&self) -> &'static str {
        match self {
            Self::Benito => "https://api.Benito.ai/v1",
    ...
    pub fn auth_style(&self) -> AuthStyle {
        match self {
            Self::Benito => AuthStyle::BenitoJwt,
