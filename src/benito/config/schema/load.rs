...
pub(crate) trait EnvLookup {
    ...
    /// (see `BENITO_CONTEXT_TOOL_RESULT_BUDGET_BYTES` below).
    ...
    /// module (e.g. `BENITO_FOO` wins over the bare `FOO` alias).
...
/// Returns the root Benito directory (`~/.Benito`), independent of any
/// per-user scoping.  Used to locate `active_user.toml` and the shared
/// `users/` tree.
pub fn default_root_BENITO_dir() -> Result<PathBuf> {
...
impl ConfigResolutionSource {
    const fn as_str(self) -> &'static str {
        match self {
            Self::EnvWorkspace => "BENITO_WORKSPACE",
...
async fn resolve_runtime_config_dirs(
    default_BENITO_dir: &Path,
    default_workspace_dir: &Path,
) -> Result<(PathBuf, PathBuf, ConfigResolutionSource)> {
    resolve_runtime_config_dirs_with(default_BENITO_dir, default_workspace_dir, &ProcessEnv)
        .await
}

/// Env-injectable variant of [`resolve_runtime_config_dirs`]. Accepts any
/// [`EnvLookup`] so unit tests can exercise the `BENITO_WORKSPACE`
...
async fn resolve_runtime_config_dirs_with(
    ...
    if let Some(custom_workspace) = env.get("BENITO_WORKSPACE") {
        if !custom_workspace.is_empty() {
            let (BENITO_dir, workspace_dir) =
    ...
    resolve_config_dirs_ignoring_env(default_BENITO_dir, default_workspace_dir).await
}

/// Same as [`resolve_runtime_config_dirs`] but skips the
/// `BENITO_WORKSPACE` env var override. Used by
...
async fn resolve_config_dirs_ignoring_env(
    ...
    // 2. Active user — scopes the entire Benito dir to a per-user directory
    //    so that config, auth, encryption, and workspace are all user-isolated.
    if let Some(user_id) = read_active_user_id(default_BENITO_dir) {
        let user_dir = user_BENITO_dir(default_BENITO_dir, &user_id);
    ...
    if let Some((BENITO_dir, workspace_dir)) =
        load_persisted_workspace_dirs(default_BENITO_dir).await?
    {
        return Ok((
            BENITO_dir,
            workspace_dir,
    ...
    let user_dir = pre_login_user_dir(default_BENITO_dir);
    let user_workspace = user_dir.join("workspace");
    tracing::debug!(
...
fn decrypt_optional_secret(
    store: &crate::benito::security::SecretStore,
    value: &mut Option<String>,
    field_name: &str,
) -> Result<()> {
    if let Some(raw) = value.clone() {
        if crate::benito::security::SecretStore::is_encrypted(&raw) {
            *value = Some(
                store
                    .decrypt(&raw)
...
fn decrypt_config_secrets(config: &mut Config, BENITO_dir: &Path) -> Result<()> {
    if !config.secrets.encrypt {
        return Ok(());
    }
    let store = crate::benito::security::SecretStore::new(BENITO_dir, true);
...
fn encrypt_config_secrets(config: &mut Config) -> Result<()> {
    ...
    let store = crate::benito::security::SecretStore::new(parent_dir, true);
...
/// Reads the active user id from `{default_BENITO_dir}/active_user.toml`.
/// Returns `None` when the file does not exist, is empty, or cannot be parsed.
pub fn read_active_user_id(default_BENITO_dir: &Path) -> Option<String> {
    let path = default_BENITO_dir.join(ACTIVE_USER_STATE_FILE);
...
/// Writes the active user id to `{default_BENITO_dir}/active_user.toml`.
pub fn write_active_user_id(default_BENITO_dir: &Path, user_id: &str) -> Result<()> {
    let path = default_BENITO_dir.join(ACTIVE_USER_STATE_FILE);
...
/// use the default (unauthenticated) Benito directory.
pub fn clear_active_user(default_BENITO_dir: &Path) -> Result<()> {
    let path = default_BENITO_dir.join(ACTIVE_USER_STATE_FILE);
    if path.exists() {
...
/// Returns the user-scoped Benito directory for the given user id:
/// `{default_BENITO_dir}/users/{user_id}`.
pub fn user_BENITO_dir(default_BENITO_dir: &Path, user_id: &str) -> PathBuf {
    default_BENITO_dir.join("users").join(user_id)
}

/// Stable id used to scope the Benito directory before any user has
...
/// so nothing is ever written directly at the root `.Benito` path.
///
...
/// `{default_BENITO_dir}/users/local`.
pub fn pre_login_user_dir(default_BENITO_dir: &Path) -> PathBuf {
    user_BENITO_dir(default_BENITO_dir, PRE_LOGIN_USER_ID)
}
...
/// `config.api_url`, double-purposing it as both the Benito product
/// backend URL AND the inference URL. That broke auth/billing/voice as
/// soon as someone picked a non-Benito provider. We now keep them in
...
fn migrate_legacy_inference_url(config: &mut Config) {
    ...
    // Benito's hosted backend exposes inference at `/openai/v1/chat/completions`;
    ...
    let is_BENITO_backend = trimmed.starts_with("https://api.tinyhumans.ai/")
        || trimmed.starts_with("https://staging-api.tinyhumans.ai/");
    let moved = if is_BENITO_backend {
        None
    } else {
...
fn migrate_cloud_provider_slugs(config: &mut Config) {
    ...
    let legacy_custom_slug = config
        ...
        .as_deref()
        .map(str::trim)
        .filter(|url| !url.is_empty() && !looks_like_BENITO_provider_endpoint(url))
        .and_then(|url| {
            ...
            config
                ...
                .iter()
                .find(|entry| {
                    !is_BENITO_provider_entry(entry)
    ...
    // Legacy bare strings are: "cloud", "Benito", "openai", "anthropic",
    ...
    let rewrite = |field: &mut Option<String>| {
        ...
        // Already in new grammar (contains ':') or is the Benito sentinel.
        if raw.contains(':') || raw == "Benito" {
            return;
        }
        match raw.as_str() {
            "cloud" => {
                // "cloud" sentinel: look for the primary or first non-Benito entry.
                // If a legacy external inference_url exists and primary still points
                // at Benito, keep routing on that custom provider; that shape was
                // written by older builds that preserved the endpoint but defaulted
                // primary_cloud to Benito.
                ...
                let slug = match primary_slug.as_deref() {
                    Some("Benito") => legacy_custom_slug.clone().or(primary_slug),
                    Some(_) => primary_slug,
                    None => legacy_custom_slug.clone().or_else(|| {
                        config
                            .cloud_providers
                            .iter()
                            .find(|entry| !is_BENITO_provider_entry(entry))
                            .map(|entry| entry.slug.clone())
                    }),
                };
                if let Some(s) = slug {
                    if s == "Benito" {
                        tracing::debug!(
                            "[config][migrate] rewriting routing 'cloud' → 'Benito'"
                        );
                        *field = Some("Benito".to_string());
                    } else {
                        tracing::info!(
                    ...
                    tracing::debug!(
                        "[config][migrate] routing 'cloud' with no non-Benito provider → 'Benito'"
                    );
                    *field = Some("Benito".to_string());
                }
            }
    ...
    fn looks_like_BENITO_provider_endpoint(url: &str) -> bool {
        ...
        matches!(
            host_no_port,
            "api.Benito.ai" | "api.tinyhumans.ai" | "staging-api.tinyhumans.ai" | "Benito"
        ) || host_no_port.ends_with(".Benito.ai")
            || host_no_port.ends_with(".tinyhumans.ai")
    }

    fn is_BENITO_provider_entry(entry: &super::cloud_providers::CloudProviderCreds) -> bool {
        entry.slug == "Benito"
            || matches!(entry.auth_style, AuthStyle::BenitoJwt)
            || looks_like_BENITO_provider_endpoint(&entry.endpoint)
    }
}
...
impl Config {
    pub async fn load_or_init() -> Result<Self> {
        let (default_BENITO_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
    ...
    async fn load_or_init_with_env_lookup(
        ...
        let (BENITO_dir, workspace_dir, resolution_source) =
            resolve_runtime_config_dirs_with(default_BENITO_dir, default_workspace_dir, env)
                .await?;

        let config_path = BENITO_dir.join("config.toml");
        ...
        if config_path.exists() {
            ...
            crate::benito::migrations::run_pending(&mut config).await;
            decrypt_config_secrets(&mut config, &BENITO_dir)?;
            Ok(config)
        } else {
            ...
            let mut config = Config {
                config_path: config_path.clone(),
                workspace_dir,
                schema_version: crate::benito::migrations::CURRENT_SCHEMA_VERSION,
                ..Default::default()
            };
            config.save().await?;
    ...
    /// `BENITO_WORKSPACE` environment variable.
    ///
    /// This is used by the debug dump to load the real user config
    /// for auth token resolution when the dump script overrides
    /// `BENITO_WORKSPACE` to a throwaway temp directory.
    pub async fn load_from_default_paths() -> Result<Self> {
        let (default_BENITO_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
        let (BENITO_dir, workspace_dir, _source) =
            resolve_config_dirs_ignoring_env(&default_BENITO_dir, &default_workspace_dir)
                .await?;
        let config_path = BENITO_dir.join("config.toml");
    ...
    pub(crate) fn apply_env_overlay_with<E: EnvLookup + ?Sized>(&mut self, env: &E) {
        // Only the namespaced `BENITO_MODEL` is honoured. The bare `MODEL`
        ...
        // (Sentry Benito-TAURI-J8).
        if let Some(model) = env.get("BENITO_MODEL") {
            // Trim before checking so `BENITO_MODEL="   "` (a common
        ...
        if let Some(workspace) = env.get("BENITO_WORKSPACE") {
        ...
        if let Some(temp_str) = env.get("BENITO_TEMPERATURE") {
        ...
        if let Some(flag) = env.get_any(&["BENITO_REASONING_ENABLED", "REASONING_ENABLED"]) {
        ...
        if let Some(key) = env.get_any(&["BENITO_SELTZ_API_KEY", "SELTZ_API_KEY"]) {
            if !key.is_empty() {
        ...
        if let Some(url) = env.get_any(&["BENITO_SELTZ_API_URL", "SELTZ_API_URL"]) {
            if !url.is_empty() {
                self.seltz.api_url = Some(url);
            }
        }
        if let Some(max) = env.get_any(&["BENITO_SELTZ_MAX_RESULTS", "SELTZ_MAX_RESULTS"]) {
        ...
        if let Some(flag) = env.get_any(&["BENITO_SEARXNG_ENABLED", "SEARXNG_ENABLED"]) {
            if let Some(enabled) = parse_env_bool("BENITO_SEARXNG_ENABLED", &flag) {
                self.searxng.enabled = enabled;
            }
        }
        if let Some(url) = env.get_any(&["BENITO_SEARXNG_BASE_URL", "SEARXNG_BASE_URL"]) {
            let url = url.trim();
            if !url.is_empty() {
                self.searxng.base_url = url.to_string();
            }
        }
        if let Some(max) = env.get_any(&["BENITO_SEARXNG_MAX_RESULTS", "SEARXNG_MAX_RESULTS"]) {
            if let Ok(n) = max.parse::<usize>() {
                if (1..=50).contains(&n) {
        ...
        if let Some(language) = env.get_any(&[
            "BENITO_SEARXNG_DEFAULT_LANGUAGE",
        ...
        if let Some(timeout_secs) = env.get_any(&[
            "BENITO_SEARXNG_TIMEOUT_SECS",
            "BENITO_SEARXNG_TIMEOUT_SECONDS",
        ...
        // `BENITO_WEB_SEARCH_ENABLED` is intentionally ignored —
        ...
        if env.contains("BENITO_WEB_SEARCH_ENABLED") {
            log::warn!(
                "[config] BENITO_WEB_SEARCH_ENABLED is deprecated and ignored — \
                 web search is always registered; provider/API-key overrides were removed."
            );
        }

        if let Some(max_results) =
            env.get_any(&["BENITO_WEB_SEARCH_MAX_RESULTS", "WEB_SEARCH_MAX_RESULTS"])
        {
        ...
        if let Some(timeout_secs) = env.get_any(&[
            "BENITO_WEB_SEARCH_TIMEOUT_SECS",
        ...
        if let Some(proxy_url) = env.get_any(&["BENITO_HTTP_PROXY", "HTTP_PROXY"]) {
            self.proxy.http_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Some(proxy_url) = env.get_any(&["BENITO_HTTPS_PROXY", "HTTPS_PROXY"]) {
            self.proxy.https_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Some(proxy_url) = env.get_any(&["BENITO_ALL_PROXY", "ALL_PROXY"]) {
            self.proxy.all_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Some(no_proxy) = env.get_any(&["BENITO_NO_PROXY", "NO_PROXY"]) {
        ...
        if let Some(scope_raw) = env.get("BENITO_PROXY_SCOPE") {
            let trimmed = scope_raw.trim();
            if !trimmed.is_empty() {
                match parse_proxy_scope(trimmed) {
                    Some(scope) => self.proxy.scope = scope,
                    None => {
                        tracing::warn!("Invalid BENITO_PROXY_SCOPE value {:?} ignored", trimmed);
                    }
                }
            }
        }

        if let Some(services_raw) = env.get("BENITO_PROXY_SERVICES") {
        ...
        if let Some(tier_str) = env.get("BENITO_LOCAL_AI_TIER") {
            let tier_str = tier_str.trim().to_ascii_lowercase();
            if !tier_str.is_empty() {
                if let Some(tier) =
                    crate::benito::inference::presets::ModelTier::from_str_opt(&tier_str)
                {
                    if tier == crate::benito::inference::presets::ModelTier::Custom {
                        tracing::warn!(
                            tier = %tier_str,
                            "ignoring custom BENITO_LOCAL_AI_TIER; only built-in presets are supported"
                        );
                    } else if !tier.is_mvp_allowed() {
                        tracing::warn!(
                            tier = %tier_str,
                            "ignoring BENITO_LOCAL_AI_TIER outside the 1B local-model allowlist"
                        );
                    } else {
                        crate::benito::inference::presets::apply_preset_to_config(
                            &mut self.local_ai,
                            tier,
                        );
                        tracing::debug!(tier = %tier_str, "applied local AI tier from BENITO_LOCAL_AI_TIER");
                    }
                } else {
                    tracing::warn!(
                        tier = %tier_str,
                        "ignoring invalid BENITO_LOCAL_AI_TIER (valid: ram_2_4gb)"
                    );
                }
            }
        }
        ...
        if let Some(flag) = env.get("BENITO_NODE_ENABLED") {
            if let Some(enabled) = parse_env_bool("BENITO_NODE_ENABLED", &flag) {
                self.node.enabled = enabled;
            }
        }
        if let Some(version) = env.get("BENITO_NODE_VERSION") {
            let trimmed = version.trim();
            if !trimmed.is_empty() {
                self.node.version = trimmed.to_string();
            }
        }
        if let Some(dir) = env.get("BENITO_NODE_CACHE_DIR") {
            let trimmed = dir.trim();
            if !trimmed.is_empty() {
                self.node.cache_dir = trimmed.to_string();
            }
        }
        if let Some(flag) = env.get("BENITO_NODE_PREFER_SYSTEM") {
            if let Some(prefer_system) = parse_env_bool("BENITO_NODE_PREFER_SYSTEM", &flag) {
                self.node.prefer_system = prefer_system;
            }
        }

        // Python runtime overrides
        if let Some(flag) = env.get("BENITO_RUNTIME_PYTHON_ENABLED") {
            if let Some(enabled) = parse_env_bool("BENITO_RUNTIME_PYTHON_ENABLED", &flag) {
                self.runtime_python.enabled = enabled;
            }
        }
        if let Some(version) = env.get("BENITO_RUNTIME_PYTHON_MINIMUM_VERSION") {
            let trimmed = version.trim();
            if !trimmed.is_empty() {
                self.runtime_python.minimum_version = trimmed.to_string();
            }
        }
        if let Some(dir) = env.get("BENITO_RUNTIME_PYTHON_CACHE_DIR") {
            self.runtime_python.cache_dir = dir.trim().to_string();
        }
        if let Some(tag) = env.get("BENITO_RUNTIME_PYTHON_MANAGED_RELEASE_TAG") {
            self.runtime_python.managed_release_tag = tag.trim().to_string();
        }
        if let Some(flag) = env.get("BENITO_RUNTIME_PYTHON_PREFER_SYSTEM") {
            if let Some(prefer_system) =
                parse_env_bool("BENITO_RUNTIME_PYTHON_PREFER_SYSTEM", &flag)
            {
                self.runtime_python.prefer_system = prefer_system;
            }
        }
        if let Some(command) = env.get("BENITO_RUNTIME_PYTHON_PREFERRED_COMMAND") {
            self.runtime_python.preferred_command = command.trim().to_string();
        }

        // Prefer the namespaced name. `BENITO_SENTRY_DSN` is the legacy
        ...
        let dsn_value = env
            .get("benito_core_SENTRY_DSN")
            .or_else(|| env.get("BENITO_SENTRY_DSN"))
        ...
        if let Some(flag) = env.get("BENITO_ANALYTICS_ENABLED") {
            let normalized = flag.trim().to_ascii_lowercase();
            match normalized.as_str() {
        ...
        if let Some(flag) = env.get("BENITO_LEARNING_ENABLED") {
            let normalized = flag.trim().to_ascii_lowercase();
            match normalized.as_str() {
        ...
        if let Some(flag) = env.get("BENITO_LEARNING_REFLECTION_ENABLED") {
            let normalized = flag.trim().to_ascii_lowercase();
            match normalized.as_str() {
        ...
        if let Some(flag) = env.get("BENITO_LEARNING_USER_PROFILE_ENABLED") {
            let normalized = flag.trim().to_ascii_lowercase();
            match normalized.as_str() {
        ...
        if let Some(flag) = env.get("BENITO_LEARNING_TOOL_TRACKING_ENABLED") {
            let normalized = flag.trim().to_ascii_lowercase();
            match normalized.as_str() {
        ...
        if let Some(flag) = env.get("BENITO_LEARNING_TOOL_MEMORY_CAPTURE_ENABLED") {
            if let Some(enabled) = parse_env_bool(
                "BENITO_LEARNING_TOOL_MEMORY_CAPTURE_ENABLED",
                flag.as_str(),
            ) {
                self.learning.tool_memory_capture_enabled = enabled;
            }
        }
        if let Some(flag) = env.get("BENITO_LEARNING_EXPLICIT_PREFERENCES_ENABLED") {
            let normalized = flag.trim().to_ascii_lowercase();
            match normalized.as_str() {
        ...
        if let Some(source) = env.get("BENITO_LEARNING_REFLECTION_SOURCE") {
            let normalized = source.trim().to_ascii_lowercase();
            match normalized.as_str() {
                "local" => {
                    self.learning.reflection_source =
                        crate::benito::config::ReflectionSource::Local
                }
                "cloud" => {
                    self.learning.reflection_source =
                        crate::benito::config::ReflectionSource::Cloud
                }
                _ => {
                    tracing::warn!(
                        source = %source,
                        "ignoring invalid BENITO_LEARNING_REFLECTION_SOURCE (valid: local, cloud)"
                    );
                }
            }
        }
        if let Some(val) = env.get("BENITO_LEARNING_MAX_REFLECTIONS_PER_SESSION") {
            if let Ok(max) = val.trim().parse::<usize>() {
                self.learning.max_reflections_per_session = max;
            }
        }
        if let Some(val) = env.get("BENITO_LEARNING_MIN_TURN_COMPLEXITY") {
            if let Ok(min) = val.trim().parse::<usize>() {
                self.learning.min_turn_complexity = min;
            }
        }
        if let Some(flag) = env.get("BENITO_LEARNING_EPISODIC_CAPTURE_ENABLED") {
            if let Some(enabled) =
                parse_env_bool("BENITO_LEARNING_EPISODIC_CAPTURE_ENABLED", flag.as_str())
            {
                self.learning.episodic_capture_enabled = enabled;
            }
        }
        if let Some(flag) = env.get("BENITO_LEARNING_STM_RECALL_ENABLED") {
            if let Some(enabled) =
                parse_env_bool("BENITO_LEARNING_STM_RECALL_ENABLED", flag.as_str())
            {
                self.learning.stm_recall_enabled = enabled;
            }
        }
        if let Some(flag) = env.get("BENITO_LEARNING_UNIFIED_COMPACTION_ENABLED") {
            if let Some(enabled) = parse_env_bool(
                "BENITO_LEARNING_UNIFIED_COMPACTION_ENABLED",
                flag.as_str(),
            ) {
        ...
        if let Ok(endpoint) = std::env::var("BENITO_MEMORY_EMBED_ENDPOINT") {
        ...
        if let Ok(model) = std::env::var("BENITO_MEMORY_EMBED_MODEL") {
            let trimmed = model.trim();
        ...
        if let Ok(val) = std::env::var("BENITO_MEMORY_EMBED_TIMEOUT_MS") {
        ...
        if let Ok(flag) = std::env::var("BENITO_MEMORY_EMBED_STRICT") {
            if let Some(strict) = parse_env_bool("BENITO_MEMORY_EMBED_STRICT", &flag) {
        ...
        if let Ok(endpoint) = std::env::var("BENITO_MEMORY_EXTRACT_ENDPOINT") {
            let trimmed = endpoint.trim();
        ...
        if let Ok(model) = std::env::var("BENITO_MEMORY_EXTRACT_MODEL") {
            let trimmed = model.trim();
        ...
        if let Ok(val) = std::env::var("BENITO_MEMORY_EXTRACT_TIMEOUT_MS") {
        ...
        if let Ok(endpoint) = std::env::var("BENITO_MEMORY_SUMMARISE_ENDPOINT") {
            let trimmed = endpoint.trim();
        ...
        if let Ok(model) = std::env::var("BENITO_MEMORY_SUMMARISE_MODEL") {
            let trimmed = model.trim();
        ...
        if let Ok(val) = std::env::var("BENITO_MEMORY_SUMMARISE_TIMEOUT_MS") {
            if let Ok(ms) = val.trim().parse::<u64>() {
                if ms > 0 {
        ...
        if let Some(dir) = env.get("BENITO_MEMORY_TREE_CONTENT_DIR") {
            let trimmed = dir.trim();
        ...
        // the Benito backend's summarizer model; "local" keeps the legacy
        ...
        if let Some(raw) = env.get("BENITO_MEMORY_TREE_LLM_BACKEND") {
            let trimmed = raw.trim();
            if !trimmed.is_empty() {
                match crate::benito::config::LlmBackend::parse(trimmed) {
                    Ok(b) => {
                        log::debug!(
                            "[memory_tree] BENITO_MEMORY_TREE_LLM_BACKEND override applied: {}",
                            b.as_str()
                        );
                    ...
                    Err(e) => {
                        tracing::warn!(
                            value = trimmed,
                            error = %e,
                            "ignoring invalid BENITO_MEMORY_TREE_LLM_BACKEND (valid: cloud, local)"
                        );
                    }
                }
            }
        ...
        if let Some(raw) = env.get("BENITO_MEMORY_TREE_CLOUD_LLM_MODEL") {
            let trimmed = raw.trim();
        ...
        if let Some(flag) = env.get("BENITO_AUTO_UPDATE_ENABLED") {
            let normalized = flag.trim().to_ascii_lowercase();
            match normalized.as_str() {
        ...
        if let Some(val) = env.get("BENITO_AUTO_UPDATE_INTERVAL_MINUTES") {
            if let Ok(minutes) = val.trim().parse::<u32>() {
                self.update.interval_minutes = minutes;
            }
        }
        if let Some(raw) = env.get("BENITO_AUTO_UPDATE_RESTART_STRATEGY") {
            match raw.trim().to_ascii_lowercase().as_str() {
                ...
                other => {
                    tracing::warn!(
                        value = other,
                        "ignoring invalid BENITO_AUTO_UPDATE_RESTART_STRATEGY \
                         (valid: self_replace, supervisor)"
                    );
                }
            }
        }
        if let Some(flag) = env.get("BENITO_AUTO_UPDATE_RPC_MUTATIONS_ENABLED") {
            if let Some(enabled) =
                parse_env_bool("BENITO_AUTO_UPDATE_RPC_MUTATIONS_ENABLED", &flag)
            {
        ...
        // Dictation overrides
        if let Some(flag) = env.get("BENITO_DICTATION_ENABLED") {
            let normalized = flag.trim().to_ascii_lowercase();
            match normalized.as_str() {
        ...
        if let Some(hotkey) = env.get("BENITO_DICTATION_HOTKEY") {
            let hotkey = hotkey.trim();
            if !hotkey.is_empty() {
                self.dictation.hotkey = hotkey.to_string();
            }
        }
        if let Some(mode) = env.get("BENITO_DICTATION_ACTIVATION_MODE") {
            let normalized = mode.trim().to_ascii_lowercase();
            match normalized.as_str() {
                "toggle" => {
                    self.dictation.activation_mode =
                        crate::benito::config::DictationActivationMode::Toggle
                }
                "push" => {
                    self.dictation.activation_mode =
                        crate::benito::config::DictationActivationMode::Push
                }
                _ => {
                    tracing::warn!(
                        mode = %mode,
                        "ignoring invalid BENITO_DICTATION_ACTIVATION_MODE (valid: toggle, push)"
                    );
                }
            }
        }
        if let Some(flag) = env.get("BENITO_DICTATION_LLM_REFINEMENT") {
            let normalized = flag.trim().to_ascii_lowercase();
            match normalized.as_str() {
        ...
        if let Some(flag) = env.get("BENITO_DICTATION_STREAMING") {
            let normalized = flag.trim().to_ascii_lowercase();
            match normalized.as_str() {
        ...
        if let Some(val) = env.get("BENITO_DICTATION_STREAMING_INTERVAL_MS") {
            if let Ok(ms) = val.trim().parse::<u64>() {
        ...
        if let Some(flag) = env.get("BENITO_CONTEXT_ENABLED") {
            let normalized = flag.trim().to_ascii_lowercase();
            match normalized.as_str() {
        ...
        if let Some(flag) = env.get("BENITO_CONTEXT_MICROCOMPACT_ENABLED") {
            let normalized = flag.trim().to_ascii_lowercase();
            match normalized.as_str() {
        ...
        if let Some(flag) = env.get("BENITO_CONTEXT_AUTOCOMPACT_ENABLED") {
            let normalized = flag.trim().to_ascii_lowercase();
            match normalized.as_str() {
        ...
        if let Some(val) = env.get("BENITO_CONTEXT_TOOL_RESULT_BUDGET_BYTES") {
            if let Ok(n) = val.trim().parse::<usize>() {
                self.context.tool_result_budget_bytes = n;
            }
        }
        if let Some(model) = env.get("BENITO_CONTEXT_SUMMARIZER_MODEL") {
        ...
        // `BENITO_CONTEXT_TOOL_RESULT_BUDGET_BYTES` to the default
        // value would have their env override silently clobbered by the
        // agent-field migration.
        let context_default = crate::benito::context::DEFAULT_TOOL_RESULT_BUDGET_BYTES;
        let context_env_set = env.contains("BENITO_CONTEXT_TOOL_RESULT_BUDGET_BYTES");
        if !context_env_set
