use super::*;
use crate::benito::config::schema::{StreamMode, TelegramConfig};
...
#[test]
fn user_BENITO_dir_builds_correct_path() {
    let root = PathBuf::from("/home/test/.Benito");
    let dir = user_BENITO_dir(&root, "user-123");
    assert_eq!(dir, PathBuf::from("/home/test/.Benito/users/user-123"));
}

#[tokio::test]
// Races on `BENITO_WORKSPACE` env var with other tests holding
// `TEST_ENV_LOCK` — passes in isolation, intermittently fails in parallel.
// Runs reliably with `--ignored --test-threads=1`. See PR #1524.
#[ignore = "flaky in parallel cargo test; BENITO_WORKSPACE env-var race — see PR #1524"]
...
fn pre_login_user_dir_is_under_users_tree() {
    ...
    assert_eq!(
        dir,
        PathBuf::from("/home/test/.Benito/users").join(PRE_LOGIN_USER_ID)
    );
}
...
fn default_root_dir_name_uses_staging_suffix_for_staging_env() {
    ...
    assert_eq!(default_root_dir_name(), ".Benito-staging");

    std::env::set_var(crate::api::config::APP_ENV_VAR, "production");
    assert_eq!(default_root_dir_name(), ".Benito");
...
use crate::benito::config::TEST_ENV_LOCK as ENV_LOCK;
...
fn apply_env_overrides_picks_up_model() {
    let _g = env_lock();
    clear_env(&["BENITO_MODEL", "MODEL"]);
    unsafe {
        std::env::set_var("BENITO_MODEL", "gpt-5");
    }
    let mut cfg = Config::default();
    cfg.apply_env_overrides();
    assert_eq!(cfg.default_model.as_deref(), Some("gpt-5"));
    unsafe {
        std::env::remove_var("BENITO_MODEL");
    }
}
...
fn apply_env_overrides_validates_temperature_range() {
    let _g = env_lock();
    clear_env(&["BENITO_TEMPERATURE"]);
    let mut cfg = Config::default();
    cfg.default_temperature = 0.5;
    unsafe {
        std::env::set_var("BENITO_TEMPERATURE", "1.2");
    }
    cfg.apply_env_overrides();
    ...
    unsafe {
        std::env::set_var("BENITO_TEMPERATURE", "5");
    }
    cfg.apply_env_overrides();
    assert!((cfg.default_temperature - 1.2).abs() < f64::EPSILON);
    ...
    unsafe {
        std::env::set_var("BENITO_TEMPERATURE", "not-a-number");
    }
    cfg.apply_env_overrides();
    assert!((cfg.default_temperature - 1.2).abs() < f64::EPSILON);
    unsafe {
        std::env::remove_var("BENITO_TEMPERATURE");
    }
}
...
fn apply_env_overrides_reasoning_enabled_parses_truthy_falsy() {
    let _g = env_lock();
    clear_env(&["BENITO_REASONING_ENABLED", "REASONING_ENABLED"]);
    let mut cfg = Config::default();
    cfg.runtime.reasoning_enabled = None;

    unsafe {
        std::env::set_var("BENITO_REASONING_ENABLED", "yes");
    }
    cfg.apply_env_overrides();
    assert_eq!(cfg.runtime.reasoning_enabled, Some(true));

    unsafe {
        std::env::set_var("BENITO_REASONING_ENABLED", "off");
    }
    cfg.apply_env_overrides();
    ...
    unsafe {
        std::env::set_var("BENITO_REASONING_ENABLED", "maybe");
    }
    cfg.apply_env_overrides();
    assert_eq!(cfg.runtime.reasoning_enabled, Some(false));
    unsafe {
        std::env::remove_var("BENITO_REASONING_ENABLED");
    }
}
...
fn apply_env_overrides_web_search_limits_only() {
    ...
    let mut cfg = Config::default();
    unsafe {
        std::env::set_var("BENITO_WEB_SEARCH_MAX_RESULTS", "5");
        std::env::set_var("BENITO_WEB_SEARCH_TIMEOUT_SECS", "20");
    }
    cfg.apply_env_overrides();
...
fn apply_env_overrides_web_search_max_results_and_timeout_clamped() {
    ...
    unsafe {
        std::env::set_var("BENITO_WEB_SEARCH_MAX_RESULTS", "999");
        std::env::set_var("BENITO_WEB_SEARCH_TIMEOUT_SECS", "0");
    }
    cfg.apply_env_overrides();
    assert_eq!(
...
fn apply_env_overrides_searxng_config() {
    let _g = env_lock();
    clear_env(&[
        "BENITO_SEARXNG_ENABLED",
        "SEARXNG_ENABLED",
        "BENITO_SEARXNG_BASE_URL",
        "SEARXNG_BASE_URL",
        "BENITO_SEARXNG_MAX_RESULTS",
        "SEARXNG_MAX_RESULTS",
    ...
    let mut cfg = Config::default();
    unsafe {
        std::env::set_var("BENITO_SEARXNG_ENABLED", "yes");
        std::env::set_var("BENITO_SEARXNG_BASE_URL", "http://127.0.0.1:8081");
        std::env::set_var("BENITO_SEARXNG_MAX_RESULTS", "25");
        std::env::set_var("BENITO_SEARXNG_DEFAULT_LANGUAGE", "zh-CN");
        std::env::set_var("BENITO_SEARXNG_TIMEOUT_SECONDS", "12");
    }
...
fn searxng_timeout_seconds_alias_deserializes() {
    let cfg: crate::benito::config::SearxngConfig =
...
fn apply_env_overrides_picks_up_sentry_dsn() {
    let _g = env_lock();
    clear_env(&["benito_core_SENTRY_DSN", "BENITO_SENTRY_DSN"]);
    let mut cfg = Config::default();
    unsafe {
        std::env::set_var("BENITO_SENTRY_DSN", "https://token@sentry.io/1");
    }
    cfg.apply_env_overrides();
    assert_eq!(
...
fn apply_env_overrides_prefers_core_sentry_dsn_when_both_set() {
    ...
    let mut cfg = Config::default();
    unsafe {
        std::env::set_var("BENITO_SENTRY_DSN", "https://legacy@sentry.io/1");
        std::env::set_var("benito_core_SENTRY_DSN", "https://new@sentry.io/2");
    }
    cfg.apply_env_overrides();
    assert_eq!(
        cfg.observability.sentry_dsn.as_deref(),
...
fn apply_env_overrides_picks_up_core_sentry_dsn_alone() {
    ...
    let mut cfg = Config::default();
    unsafe {
        std::env::set_var("benito_core_SENTRY_DSN", "https://token@sentry.io/3");
    }
    cfg.apply_env_overrides();
    assert_eq!(
        cfg.observability.sentry_dsn.as_deref(),
...
async fn env_workspace_override_wins_via_seam() {
    ...
    let env = MapEnv::default().with("BENITO_WORKSPACE", ws_path.to_str().unwrap());
...
async fn empty_env_workspace_falls_through_to_active_user() {
    ...
    let env = MapEnv::default().with("BENITO_WORKSPACE", "");
...
async fn missing_env_workspace_uses_pre_login_default() {
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path();
    let env = MapEnv::default(); // no BENITO_WORKSPACE, no active user
...
fn resolve_config_dir_for_workspace_returns_parent_and_workspace() {
    let ws = PathBuf::from("/home/test/.Benito/workspace");
    let (config_dir, workspace_dir) = resolve_config_dir_for_workspace(&ws);
    // Config dir is the parent of workspace.
    assert!(
        config_dir.ends_with(".Benito") || config_dir == PathBuf::from("/home/test/.Benito")
    );
...
fn env_overlay_model_only_honours_namespaced_var() {
    // Both set → BENITO_MODEL wins; bare MODEL is ignored even when
    // BENITO_MODEL is absent.
    let env = HashMapEnv::new()
        .with("BENITO_MODEL", "specific-v2")
    ...
    // Whitespace-only BENITO_MODEL must not clobber either. Some
    // shells/CI runners pass an unset-but-declared env var through as
    // `"   "`, which `is_empty()` alone wouldn't reject.
    let env = HashMapEnv::new().with("BENITO_MODEL", "   ");
    let mut cfg = Config::default();
    let original = cfg.default_model.clone();
    cfg.apply_env_overlay_with(&env);
    assert_eq!(
        cfg.default_model, original,
        "whitespace-only BENITO_MODEL must not clobber default_model"
    );
}
...
fn env_overlay_temperature_accepts_valid_and_ignores_out_of_range_or_garbage() {
    ...
    cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_TEMPERATURE", "1.5"));
    assert!((cfg.default_temperature - 1.5).abs() < f64::EPSILON);

    // Negative (< 0.0) — ignored.
    cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_TEMPERATURE", "-0.1"));
    assert!((cfg.default_temperature - 1.5).abs() < f64::EPSILON);

    // Above cap (> 2.0) — ignored.
    cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_TEMPERATURE", "2.5"));
    assert!((cfg.default_temperature - 1.5).abs() < f64::EPSILON);

    // Garbage — ignored.
    cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_TEMPERATURE", "nope"));
    assert!((cfg.default_temperature - 1.5).abs() < f64::EPSILON);

    // Boundaries — inclusive on both ends.
    cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_TEMPERATURE", "0"));
    assert_eq!(cfg.default_temperature, 0.0);
    cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_TEMPERATURE", "2"));
...
fn env_overlay_reasoning_enabled_recognises_truthy_falsy_and_ignores_garbage() {
    ...
    for truthy in ["1", "true", "yes", "on", "TRUE", " On "] {
        cfg.runtime.reasoning_enabled = None;
        cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_REASONING_ENABLED", truthy));
        assert_eq!(
    ...
    for falsy in ["0", "false", "no", "off", "OFF"] {
        ...
        cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_REASONING_ENABLED", falsy));
        assert_eq!(
            cfg.runtime.reasoning_enabled,
            Some(false),
    ...
    cfg.runtime.reasoning_enabled = Some(true);
    cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_REASONING_ENABLED", "maybe"));
    assert_eq!(cfg.runtime.reasoning_enabled, Some(true));

    // Alias works when the Benito variant is absent.
    cfg.runtime.reasoning_enabled = None;
...
fn env_overlay_web_search_limits_validated() {
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new()
            .with("BENITO_WEB_SEARCH_MAX_RESULTS", "7")
            .with("BENITO_WEB_SEARCH_TIMEOUT_SECS", "25"),
    );
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new()
            .with("BENITO_WEB_SEARCH_MAX_RESULTS", "0")
            .with("BENITO_WEB_SEARCH_TIMEOUT_SECS", "0"),
    );
    assert_eq!(cfg.web_search.max_results, 7);
    assert_eq!(cfg.web_search.timeout_secs, 25);

    cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_WEB_SEARCH_MAX_RESULTS", "11"));
    assert_eq!(cfg.web_search.max_results, 7);

    // Bare aliases also accepted when the Benito-prefixed variant is absent.
...
fn env_overlay_searxng_config_validated() {
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new()
            .with("BENITO_SEARXNG_ENABLED", "true")
            .with("BENITO_SEARXNG_BASE_URL", "http://127.0.0.1:8888")
            .with("BENITO_SEARXNG_MAX_RESULTS", "40")
            .with("BENITO_SEARXNG_DEFAULT_LANGUAGE", "fr")
            .with("BENITO_SEARXNG_TIMEOUT_SECS", "9"),
    );
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new()
            .with("BENITO_SEARXNG_ENABLED", "no")
            .with("BENITO_SEARXNG_MAX_RESULTS", "0")
            .with("BENITO_SEARXNG_TIMEOUT_SECS", "0"),
    );
...
fn env_overlay_proxy_url_enables_proxy_when_not_explicit() {
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("BENITO_HTTP_PROXY", "http://proxy.local:3128"),
    );
...
fn env_overlay_explicit_proxy_enabled_overrides_auto_enable() {
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new()
            .with("BENITO_PROXY_ENABLED", "false")
            .with("BENITO_HTTP_PROXY", "http://proxy.local:3128"),
    );
    assert!(
        !cfg.proxy.enabled,
        "explicit BENITO_PROXY_ENABLED=false must win over URL-driven auto-enable"
    );
}
...
fn env_overlay_proxy_scope_invalid_value_leaves_scope_unchanged() {
    ...
    cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_PROXY_SCOPE", "bogus-scope"));
...
fn env_overlay_node_flags_respect_bool_parser() {
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new()
            .with("BENITO_NODE_ENABLED", "yes")
            .with("BENITO_NODE_PREFER_SYSTEM", "off")
            .with("BENITO_NODE_CACHE_DIR", "/tmp/oh-node"),
    );
    ...
    cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_NODE_ENABLED", "perhaps"));
    assert!(cfg.node.enabled);

    // Blank version does NOT clobber.
    cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_NODE_VERSION", "   "));
...
fn env_overlay_runtime_python_flags_respect_bool_parser() {
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new()
            .with("BENITO_RUNTIME_PYTHON_ENABLED", "yes")
            .with("BENITO_RUNTIME_PYTHON_PREFER_SYSTEM", "off")
            .with("BENITO_RUNTIME_PYTHON_CACHE_DIR", "/tmp/oh-python")
            .with("BENITO_RUNTIME_PYTHON_MANAGED_RELEASE_TAG", "20260510")
            .with("BENITO_RUNTIME_PYTHON_PREFERRED_COMMAND", "python3.12"),
    );
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("BENITO_RUNTIME_PYTHON_ENABLED", "perhaps"),
    );
    assert!(cfg.runtime_python.enabled);

    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("BENITO_RUNTIME_PYTHON_MINIMUM_VERSION", "   "),
    );
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new()
            .with("BENITO_RUNTIME_PYTHON_CACHE_DIR", "   ")
            .with("BENITO_RUNTIME_PYTHON_MANAGED_RELEASE_TAG", "   ")
            .with("BENITO_RUNTIME_PYTHON_PREFERRED_COMMAND", "   "),
    );
...
fn env_overlay_sentry_dsn_trims_and_ignores_blank() {
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("BENITO_SENTRY_DSN", "  https://t@sentry.io/42  "),
    );
    assert_eq!(
        cfg.observability.sentry_dsn.as_deref(),
    ...
    cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_SENTRY_DSN", "   "));
    assert_eq!(
        cfg.observability.sentry_dsn.as_deref(),
        Some("https://t@sentry.io/42")
    );
...
fn env_overlay_prefers_namespaced_core_sentry_dsn() {
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new()
            .with("BENITO_SENTRY_DSN", "https://legacy@sentry.io/1")
            .with("benito_core_SENTRY_DSN", "https://new@sentry.io/2"),
    );
    assert_eq!(
        cfg.observability.sentry_dsn.as_deref(),
        Some("https://new@sentry.io/2"),
        "benito_core_SENTRY_DSN must win over BENITO_SENTRY_DSN"
    );
}
...
fn env_overlay_namespaced_core_sentry_dsn_works_alone() {
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("benito_core_SENTRY_DSN", "https://token@sentry.io/3"),
    );
    assert_eq!(
        cfg.observability.sentry_dsn.as_deref(),
        Some("https://token@sentry.io/3")
...
fn env_overlay_analytics_enabled_parses_truthy_falsy() {
    ...
    cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_ANALYTICS_ENABLED", "1"));
    assert!(cfg.observability.analytics_enabled);

    cfg.apply_env_overlay_with(&HashMapEnv::new().with("BENITO_ANALYTICS_ENABLED", "0"));
    assert!(!cfg.observability.analytics_enabled);
}
...
fn env_overlay_learning_source_values_and_invalid_ignored() {
    let mut cfg = Config::default();
    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("BENITO_LEARNING_REFLECTION_SOURCE", "local"),
    );
    assert_eq!(
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("BENITO_LEARNING_REFLECTION_SOURCE", "cloud"),
    );
    assert_eq!(
        cfg.learning.reflection_source,
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("BENITO_LEARNING_REFLECTION_SOURCE", "bogus"),
    );
    assert_eq!(
        cfg.learning.reflection_source,
        crate::benito::config::ReflectionSource::Cloud
...
fn env_overlay_learning_numeric_values_parse() {
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new()
            .with("BENITO_LEARNING_MAX_REFLECTIONS_PER_SESSION", "8")
            .with("BENITO_LEARNING_MIN_TURN_COMPLEXITY", "2"),
    );
...
fn env_overlay_dictation_activation_mode_only_toggle_or_push() {
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("BENITO_DICTATION_ACTIVATION_MODE", "toggle"),
    );
    assert_eq!(
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("BENITO_DICTATION_ACTIVATION_MODE", "push"),
    );
    assert_eq!(
        cfg.dictation.activation_mode,
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("BENITO_DICTATION_ACTIVATION_MODE", "wave"),
    );
    assert_eq!(
        cfg.dictation.activation_mode,
        crate::benito::config::DictationActivationMode::Push
...
fn env_overlay_context_tool_result_budget_env_suppresses_legacy_migration() {
    ...
    // the default.
    let default_budget = crate::benito::context::DEFAULT_TOOL_RESULT_BUDGET_BYTES;
    let mut cfg = Config::default();
    ...
    cfg.apply_env_overlay_with(&HashMapEnv::new().with(
        "BENITO_CONTEXT_TOOL_RESULT_BUDGET_BYTES",
...
fn env_overlay_context_tool_result_budget_env_wins_over_legacy_migration() {
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("BENITO_CONTEXT_TOOL_RESULT_BUDGET_BYTES", "222222"),
    );
    assert_eq!(
...
fn env_overlay_auto_update_interval_parses_u32() {
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new()
            .with("BENITO_AUTO_UPDATE_ENABLED", "true")
            .with("BENITO_AUTO_UPDATE_INTERVAL_MINUTES", "60"),
    );
    ...
    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("BENITO_AUTO_UPDATE_INTERVAL_MINUTES", "hello"),
    );
    assert_eq!(cfg.update.interval_minutes, 60);
}
...
fn env_overlay_auto_update_restart_strategy_accepts_supported_values() {
    let mut cfg = Config::default();
    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("BENITO_AUTO_UPDATE_RESTART_STRATEGY", "supervisor"),
    );
    assert_eq!(
        cfg.update.restart_strategy,
        crate::benito::config::UpdateRestartStrategy::Supervisor
    );

    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("BENITO_AUTO_UPDATE_RESTART_STRATEGY", "self_replace"),
    );
    assert_eq!(
        cfg.update.restart_strategy,
        crate::benito::config::UpdateRestartStrategy::SelfReplace
    );
}
...
fn env_overlay_auto_update_rpc_mutations_enabled_parses_bool() {
    let mut cfg = Config::default();
    cfg.apply_env_overlay_with(
        &HashMapEnv::new().with("BENITO_AUTO_UPDATE_RPC_MUTATIONS_ENABLED", "false"),
    );
...
async fn resolve_runtime_config_dirs_with_env_workspace_override() {
    ...
    // Point BENITO_WORKSPACE at a custom path via HashMapEnv — no
    // process-env mutation needed.
    let custom_ws = tmp.path().join("custom_ws");
    let env = HashMapEnv::new().with("BENITO_WORKSPACE", custom_ws.to_str().unwrap());
...
async fn resolve_runtime_config_dirs_with_empty_env_falls_back_to_default() {
    ...
    // Empty env: no BENITO_WORKSPACE → falls through to the pre-login
...
fn apply_env_overrides_commits_side_effects_to_runtime_proxy() {
    use crate::benito::config::schema::proxy::{runtime_proxy_config, set_runtime_proxy_config};

    // Hold the env lock so no other test races on proxy-related env vars.
    let _g = env_lock();
    clear_env(&[
        "BENITO_PROXY_ENABLED",
        "BENITO_HTTP_PROXY",
        "HTTP_PROXY",
        "BENITO_HTTPS_PROXY",
        "HTTPS_PROXY",
...
async fn load_or_init_for_workspace(root: &std::path::Path) -> Config {
    let env = MapEnv::default().with("BENITO_WORKSPACE", root.to_str().unwrap());
...
async fn load_or_init_falls_back_to_defaults_when_backup_also_corrupted() {
    ...
    assert_eq!(
        config.default_model.as_deref(),
        Some(crate::benito::config::schema::DEFAULT_MODEL),
...
#[test]
fn migrate_legacy_inference_url_clears_BENITO_backend_form() {
    let mut cfg = Config::default();
    cfg.api_url = Some("https://api.tinyhumans.ai/openai/v1/chat/completions".to_string());
    cfg.inference_url = None;
    migrate_legacy_inference_url(&mut cfg);
    // The Benito host is the default backend — both fields end up None so
...
#[test]
fn migrate_cloud_provider_slugs_routes_cloud_to_legacy_custom_when_primary_is_Benito() {
    let mut cfg = Config::default();
    cfg.inference_url = Some("https://api.example.com/v1".into());
    cfg.primary_cloud = Some("p_oh".into());
    cfg.memory_provider = Some("cloud".into());
    cfg.reasoning_provider = Some("Benito".into());
    cfg.cloud_providers = vec![
        crate::benito::config::schema::CloudProviderCreds {
            id: "p_oh".into(),
            slug: "Benito".into(),
            label: "Benito".into(),
            endpoint: "https://api.Benito.ai/v1".into(),
            auth_style: crate::benito::config::schema::AuthStyle::BenitoJwt,
            ..Default::default()
        },
        crate::benito::config::schema::CloudProviderCreds {
            id: "p_custom".into(),
            ...
            auth_style: crate::benito::config::schema::AuthStyle::Bearer,
    ...
    assert_eq!(
        ...
        Some("Benito"),
        "explicit Benito routing must stay explicit"
    );
}

#[test]
fn migrate_cloud_provider_slugs_keeps_cloud_on_BENITO_without_legacy_custom() {
    let mut cfg = Config::default();
    cfg.primary_cloud = Some("p_oh".into());
    cfg.memory_provider = Some("cloud".into());
    cfg.cloud_providers = vec![crate::benito::config::schema::CloudProviderCreds {
        id: "p_oh".into(),
        slug: "Benito".into(),
        label: "Benito".into(),
    ...
    assert_eq!(cfg.memory_provider.as_deref(), Some("Benito"));
}
