...
fn default_embedding_provider() -> String {
    // Default to the Benito backend (Voyage-backed `embedding-v1`) so a
...
///   Benito backend with the `summarization-v1` model. No local Ollama
///   required.
...
pub enum LlmBackend {
    /// Route through the Benito backend (default).
    Cloud,
...
/// through the Benito backend; keep in sync with the backend's
...
/// - `BENITO_MEMORY_EMBED_ENDPOINT`
/// - `BENITO_MEMORY_EMBED_MODEL`
/// - `BENITO_MEMORY_EMBED_TIMEOUT_MS`
/// - `BENITO_MEMORY_EXTRACT_ENDPOINT`
/// - `BENITO_MEMORY_EXTRACT_MODEL`
/// - `BENITO_MEMORY_EXTRACT_TIMEOUT_MS`
/// - `BENITO_MEMORY_SUMMARISE_ENDPOINT`
/// - `BENITO_MEMORY_SUMMARISE_MODEL`
/// - `BENITO_MEMORY_SUMMARISE_TIMEOUT_MS`
/// - `BENITO_MEMORY_TREE_CONTENT_DIR` (Phase MD-content)
/// - `BENITO_MEMORY_TREE_LLM_BACKEND` (cloud|local)
/// - `BENITO_MEMORY_TREE_CLOUD_LLM_MODEL`
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(default)]
pub struct MemoryTreeConfig {
    ...
    ///
    /// Env override: `BENITO_MEMORY_TREE_CONTENT_DIR` (empty string = fall
    ...
    /// [`LlmBackend::Local`] (or `BENITO_MEMORY_TREE_LLM_BACKEND=local`) to
    ...
    /// Benito backend's chat-completions surface.
    ///
    /// Defaults to [`DEFAULT_CLOUD_LLM_MODEL`] (`summarization-v1`).
    /// Env override: `BENITO_MEMORY_TREE_CLOUD_LLM_MODEL`.
...
/// in TOML or the `BENITO_MEMORY_EMBED_ENDPOINT` env var.
...
/// `BENITO_MEMORY_TREE_LLM_*_ENDPOINT` to point at a different
/// Ollama host.
...
/// `llm_extractor_model` in TOML (or `BENITO_MEMORY_TREE_LLM_*_MODEL`
