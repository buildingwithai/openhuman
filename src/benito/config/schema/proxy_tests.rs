...
fn parse_proxy_scope_accepts_known_aliases() {
    ...
    assert_eq!(parse_proxy_scope("ENV"), Some(ProxyScope::Environment));
    assert_eq!(parse_proxy_scope("Benito"), Some(ProxyScope::Benito));
    assert_eq!(parse_proxy_scope("internal"), Some(ProxyScope::Benito));
    assert_eq!(parse_proxy_scope("core"), Some(ProxyScope::Benito));
...
#[test]
fn should_apply_BENITO_scope_always_true() {
    let c = ProxyConfig {
        enabled: true,
        http_proxy: Some("http://p:8080".into()),
        scope: ProxyScope::Benito,
        ..Default::default()
    };
    assert!(c.should_apply_to_service("provider.openai"));
    assert!(c.should_apply_to_service("anything"));
