...
pub struct Config {
    ...
    /// migration under [`crate::benito::migrations`] runs successfully.
    ...
    /// instead of routing through the Benito backend. Account/auth/billing
    ...
    /// [`crate::benito::context::ContextManager`].
    #[serde(default)]
    ...
    //                            Benito, behaves identically to "Benito"
    //   "Benito"            → Benito backend (api_url + api_key session JWT)
    ...
    /// Registered cloud providers. Index 0 is always the built-in Benito
    /// entry; additional entries are user-added third-party backends.
    #[serde(default)]
    pub cloud_providers: Vec<crate::benito::config::schema::cloud_providers::CloudProviderCreds>,

    /// Id of the `cloud_providers` entry that "cloud" and "primary" resolve to.
    /// When `None`, the factory falls back to the Benito entry.
    #[serde(default)]
    ...
    /// [`crate::benito::config::schema::MeetConfig`]).
    #[serde(default)]
    pub meet: MeetConfig,
...
impl Default for Config {
    fn default() -> Self {
        let BENITO_dir =
            crate::benito::config::default_root_BENITO_dir().unwrap_or_else(|_| {
        ...
        Self {
            workspace_dir: BENITO_dir.join("workspace"),
            config_path: BENITO_dir.join("config.toml"),
            schema_version: 0,
            api_url: None,
            api_key: None,
            inference_url: None,
