//! Tauri-focused daemon configuration wrapper for Benito.
...
pub struct DaemonConfig {
    /// Root data directory (defaults to Tauri's `app_data_dir/Benito`).
...
impl DaemonConfig {
    ...
    pub fn from_app_data_dir(app_data_dir: &std::path::Path) -> Self {
        let data_dir = app_data_dir.join("Benito");
        let workspace_dir = data_dir.join("workspace");
        log::info!(
            "[Benito:config] Initialized config: data_dir={}, workspace_dir={}",
            data_dir.display(),
...
mod tests {
    ...
    fn daemon_config_from_app_data_dir() {
        let app_data = std::path::PathBuf::from("/tmp/test-Benito");
        let config = DaemonConfig::from_app_data_dir(&app_data);

        assert_eq!(config.data_dir, app_data.join("Benito"));
        assert_eq!(
            config.workspace_dir,
            app_data.join("Benito").join("workspace")
        );
    }
}
