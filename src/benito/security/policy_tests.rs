...
// Regression: Benito-TAURI-GW (#1813). A multi-byte UTF-8 char straddling
...
fn from_config_maps_all_fields() {
    let autonomy_config = crate::benito::config::AutonomyConfig {
        ...
        ..crate::benito::config::AutonomyConfig::default()
    };
...
fn resolved_path_blocks_outside_workspace() {
    let workspace = std::env::temp_dir().join("BENITO_test_resolved_path");
    ...
    let outside = canonical_temp.join("outside_workspace_Benito");
    assert!(
...
fn resolved_path_blocks_root_escape() {
    let policy = SecurityPolicy {
        workspace_dir: PathBuf::from("/home/BENITO_user/project"),
        ..SecurityPolicy::default()
    };
...
fn resolved_path_blocks_symlink_escape() {
    ...
    let root = std::env::temp_dir().join("BENITO_test_symlink_escape");
...
async fn validate_path_expands_tilde_before_workspace_join() {
    ...
    let target = home.join("BENITO_tilde_validate_path_test.txt");
    ...
    let err = policy
        .validate_path("~/BENITO_tilde_validate_path_test.txt")
        .await
        .unwrap_err();
...
async fn validate_parent_path_expands_tilde_before_workspace_join() {
    ...
    let err = policy
        .validate_parent_path("~/BENITO_tilde_validate_parent_test.txt")
        .await
        .unwrap_err();
    assert!(
