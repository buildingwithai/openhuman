...
// in `{data_dir}/Benito/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`{data_dir}/Benito/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(BENITO_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: BENITO_dir.join(".secret_key"),
            enabled,
        }
    }
                ...
                #[cfg(windows)]
                {
                    msg.push_str(
                        "\n\nThis is often caused by incorrect file permissions on Windows. \
                         Try repairing ACLs on the .Benito directory:\n\
                         icacls \"%USERPROFILE%\\.Benito\" /reset /t /c\n\
                         icacls \"%USERPROFILE%\\.Benito\\.secret_key\" /reset /c",
                    );
                }
                msg
            })?;
