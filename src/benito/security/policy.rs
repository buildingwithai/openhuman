...
use crate::benito::util::floor_char_boundary;
...
impl SecurityPolicy {
    ...
    pub fn validate_command_execution(
        ...
        if !self.is_command_allowed(command) {
            ...
            log::warn!(
                "[Benito:policy] Command blocked by allowlist: {}",
                truncated
            );
            return Err(format!(
        ...
        if risk == CommandRiskLevel::High {
            if self.block_high_risk_commands {
                log::warn!(
                    "[Benito:policy] High-risk command blocked: {}",
            ...
            if self.autonomy == AutonomyLevel::Supervised && !approved {
                log::warn!(
                    "[Benito:policy] High-risk command needs approval: {}",
                    &command[..floor_char_boundary(command, 80)]
                );
                return Err(
        ...
        {
            log::info!(
                "[Benito:policy] Medium-risk command needs approval: {}",
                &command[..floor_char_boundary(command, 80)]
            );
            return Err(
        ...
        log::debug!(
            "[Benito:policy] Command validated: risk={:?}, approved={}, cmd={}",
            risk,
            approved,
            &command[..floor_char_boundary(command, 80)]
        );
    ...
    pub fn enforce_tool_operation(
        ...
        match operation {
            ...
            ToolOperation::Act => {
                if !self.can_act() {
                    log::warn!(
                        "[Benito:policy] Operation '{}' blocked: read-only mode",
                        operation_name
                    );
                    return Err(format!(
                ...
                if !self.record_action() {
                    log::warn!(
                        "[Benito:policy] Operation '{}' blocked: rate limit exceeded",
                        operation_name
                    );
                ...
                log::debug!(
                    "[Benito:policy] Operation '{}' allowed (actions: {}/{})",
                    operation_name,
                    self.tracker.count(),
    ...
    pub fn from_config(
        autonomy_config: &crate::benito::config::AutonomyConfig,
        workspace_dir: &Path,
    ) -> Self {
        log::info!(
            "[Benito:policy] SecurityPolicy created: autonomy={:?}, workspace_only={}, allowed_cmds={}, max_actions/hr={}",
