...
use crate::benito::config::AuditConfig;
use anyhow::Result;
...
impl AuditLogger {
    ...
    pub fn new(config: AuditConfig, BENITO_dir: PathBuf) -> Result<Self> {
        let log_path = BENITO_dir.join(&config.log_path);
        log::info!(
            "[Benito:audit] Logger initialized: enabled={}, path={}",
            config.enabled,
            log_path.display()
        );
        Ok(Self {
    ...
    pub fn log(&self, event: &AuditEvent) -> Result<()> {
        ...
        log::debug!(
            "[Benito:audit] Logging event: type={:?}, id={}",
            event.event_type,
            event.event_id
        );
    ...
    fn rotate(&self) -> Result<()> {
        log::info!(
            "[Benito:audit] Rotating audit log: {}",
