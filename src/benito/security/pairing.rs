...
pub const CORE_TOKEN_ENV_VAR: &str = "benito_core_TOKEN";
...
impl PairingGuard {
    ...
    pub fn new(require_pairing: bool, existing_tokens: &[String]) -> (Self, Option<String>) {
        ...
        log::info!(
            "[Benito:pairing] Guard created: require_pairing={}, existing_tokens={}, code_generated={}",
            require_pairing,
            tokens.len(),
            code.is_some()
        );
    ...
    fn try_pair_blocking(&self, code: &str) -> Result<Option<String>, u64> {
            ...
            if let (count, Some(locked_at)) = &*attempts {
                if *count >= MAX_PAIR_ATTEMPTS {
                    ...
                    if elapsed < PAIR_LOCKOUT_SECS {
                        log::warn!(
                            "[Benito:pairing] Pairing locked out: {} failed attempts, {}s remaining",
                            count,
            ...
            if let Some(ref expected) = *pairing_code {
                if constant_time_eq(code.trim(), expected.trim()) {
                    ...
                    *pairing_code = None;
                    ...
                    log::info!("[Benito:pairing] Pairing successful, token issued");
        ...
        // Increment failed attempts
        {
            ...
            attempts.0 += 1;
            log::warn!(
                "[Benito:pairing] Pairing attempt failed ({}/{})",
                attempts.0,
                MAX_PAIR_ATTEMPTS
            );
            if attempts.0 >= MAX_PAIR_ATTEMPTS {
                attempts.1 = Some(Instant::now());
                log::warn!("[Benito:pairing] Max attempts reached, lockout activated");
            }
        }
    ...
    pub fn is_authenticated(&self, token: &str) -> bool {
        if token.trim().is_empty() {
            log::debug!("[Benito:pairing] is_authenticated: rejected empty bearer token");
            return false;
        }
        ...
        if tokens.is_empty() {
            log::debug!(
                "[Benito:pairing] is_authenticated: no paired tokens configured (require_pairing={})",
                self.require_pairing
            );
            return false;
        }
        ...
        if !ok {
            log::debug!("[Benito:pairing] is_authenticated: bearer token not in paired set");
        }
        ok
    }
...
pub fn ensure_core_rpc_token_for_bind(
    ...
    if let Some(raw) = env_token {
        ...
        if !trimmed.is_empty() {
            log::info!(
                "[Benito:pairing] core RPC token supplied via {CORE_TOKEN_ENV_VAR} for bind host={host}"
            );
            return Ok(Some(trimmed.to_string()));
        }
        if is_public_bind(host) {
            log::error!(
                "[Benito:pairing] {CORE_TOKEN_ENV_VAR} is set but empty on public bind host={host}"
            );
    ...
    if !is_public_bind(host) {
        log::debug!(
            "[Benito:pairing] loopback bind host={host}: no {CORE_TOKEN_ENV_VAR} configured"
        );
        return Ok(None);
    }
    ...
    log::warn!(
        "[Benito:pairing] Public bind on {host} without {CORE_TOKEN_ENV_VAR}: \
