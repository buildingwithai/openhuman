...
- `src/Benito/config/` — `SecurityConfig`, `AutonomyConfig` for policy + sandbox selection.
...
- `src/Benito/cron/scheduler.rs` — wraps shell jobs in `SecurityPolicy::from_config`.
- `src/Benito/tools/local_cli.rs`, `tools/ops.rs`, and most `tools/impl/{system,network,memory,agent}/*.rs` — every executable tool consults `SecurityPolicy`.
- `src/Benito/tools/impl/network/{curl,http_request,composio}.rs` — risk-classify outbound calls.
- `src/Benito/tools/impl/memory/{store,forget}.rs` — sensitive-write tracking.
- `src/Benito/tools/impl/agent/delegate.rs` — sub-agent dispatch goes through autonomy gate.
- `src/Benito/credentials/` — uses `SecretStore` and `redact`.
