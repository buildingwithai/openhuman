...
use crate::core::all;
use crate::benito::agent::harness::AgentDefinitionRegistry;
use crate::benito::agent::Agent;
use crate::benito::config::rpc as config_rpc;
use crate::benito::inference::provider::traits::build_tool_instructions_text;
use crate::benito::integrations::searxng::MAX_RESULTS as SEARXNG_MAX_RESULTS;
use crate::benito::security::{SecurityPolicy, ToolOperation};
...
fn base_tool_specs() -> Vec<McpToolSpec> {
    vec![
        McpToolSpec {
            ...
            description: "List the live core agent tool catalog that Benito exposes to its orchestrator session.",
            rpc_method: None,
        ...
        McpToolSpec {
            ...
            description: "Emit the markdown tool-use instructions block that Benito injects into prompt-guided agents.",
            rpc_method: None,
            input_schema: no_args_schema(),
        },
        McpToolSpec {
        ...
        McpToolSpec {
            ...
            title: "Run Subagent",
            description: "Run a registered Benito sub-agent directly from the core and return its final response.",
            rpc_method: None,
            input_schema: json!({
                "type": "object",
                "properties": {
        ...
        McpToolSpec {
            name: "memory.search",
            title: "Search Memory",
            description: "Keyword-search Benito's local memory tree and return matching chunks ordered by recency.",
            rpc_method: Some("Benito.memory_tree_search"),
        ...
        McpToolSpec {
            ...
            rpc_method: Some("Benito.memory_tree_recall"),
        ...
        McpToolSpec {
            ...
            rpc_method: Some("Benito.memory_tree_get_chunk"),
            input_schema: json!({
                "type": "object",
                "properties": {
                    "chunk_id": {
        ...
        McpToolSpec {
            ...
            description: "Paginated listing of memory-tree chunks in reverse-chronological order, \
                          ...
                          pagination.",
            rpc_method: Some("Benito.memory_tree_list_chunks"),
        ...
        McpToolSpec {
            ...
            rpc_method: Some("Benito.memory_tree_top_entities"),
        ...
        McpToolSpec {
            ...
            description: "List every distinct ingest source (Gmail account, Slack channel, Notion \
                          ...
                          `tree.browse`.",
            rpc_method: Some("Benito.memory_tree_list_sources"),
...
fn searxng_tool_spec() -> McpToolSpec {
    McpToolSpec {
        ...
        description: "Search the configured self-hosted SearXNG instance and return normalized title, URL, snippet, and source results. Requires searxng.enabled=true in Benito config.",
        rpc_method: Some("Benito.tools_searxng_search"),
...
fn list_tools_result_for_config(config: &crate::benito::config::Config) -> Value {
    let mut specs = base_tool_specs();
    ...
    match tool_name {
        ...
        "searxng_search" => {
            ...
            if let Some(categories) = optional_string_array(&args, "categories")? {
                crate::benito::integrations::searxng::normalize_categories(categories.clone())
...
async fn load_config_and_init_registry() -> Result<crate::benito::config::Config, ToolCallError>
{
...
mod tests {
    ...
    fn list_tools_exposes_base_mcp_surface_when_searxng_disabled() {
        let config = crate::benito::config::Config::default();
