...
use crate::benito::memory::{MemoryClient, MemoryClientRef};
...
/// Initialise using the default `~/.Benito/workspace` directory.
///
...
/// would pin the singleton to `~/.Benito/workspace`, causing every
...
/// fallback: a fallback would pin the global to `~/.Benito/workspace` on
