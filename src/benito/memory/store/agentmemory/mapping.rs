//! Maps Benito `MemoryEntry` / `MemoryCategory` to the agentmemory REST
...
//! through Benito's trait. We map the Benito-visible columns and let
//! agentmemory own the rest.

use crate::benito::memory::traits::{MemoryCategory, MemoryEntry};
use serde::{Deserialize, Serialize};

/// Globally well-known "default" project name used when an Benito caller
...
/// — we only deserialise what Benito's `MemoryEntry` needs, leaving the
...
impl WireMemory {
    /// Project the wire row into an Benito `MemoryEntry`. `key` falls
