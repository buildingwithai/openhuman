...
use crate::benito::config::{EmbeddingRouteConfig, MemoryConfig, StorageProviderConfig};
use crate::benito::embeddings::{
...
use crate::benito::memory::store::agentmemory::AgentMemoryBackend;
use crate::benito::memory::store::unified::UnifiedMemory;
use crate::benito::memory::traits::Memory;
...
/// REST surface. Benito's `embedding_provider` / `embedding_model` /
...
///
/// Delegates to [`crate::benito::inference::local::ollama_base_url`] so the probe
...
fn ollama_base_url_for_probe() -> String {
    crate::benito::inference::local::ollama_base_url()
}
...
/// embedding backend.
///
/// The user-facing default is `"cloud"` (Benito backend, Voyage-backed) so
...
/// Benito-TAURI-B7: a user who's flipped `local_ai.usage.embeddings = true`
...
fn create_memory_full(
    ...
    //    embedding pipeline and create a divergence between Benito's
    ...
    if is_agentmemory_backend(&config.backend) {
        log::info!(
            ...
            config
                .agentmemory_url
                .as_deref()
                .unwrap_or(crate::benito::memory::store::agentmemory_default_url()),
        );
    ...
    //    Prevents Benito-TAURI-B7's 226-event Sentry flood: instead of
...
mod tests {
    ...
    /// RAII helper that swaps `BENITO_OLLAMA_BASE_URL` to `value` for the
    ...
    impl EnvGuard {
        fn set(value: &str) -> Self {
            let lock = crate::benito::inference::local::inference_test_guard();
            let prev = std::env::var_os("BENITO_OLLAMA_BASE_URL");
            ...
            unsafe {
                std::env::set_var("BENITO_OLLAMA_BASE_URL", value);
            }
        ...
        fn drop(&mut self) {
            ...
            unsafe {
                match self.prev.take() {
                    Some(v) => std::env::set_var("BENITO_OLLAMA_BASE_URL", v),
                    None => std::env::remove_var("BENITO_OLLAMA_BASE_URL"),
                }
            }
        }
    }
    ...
    fn embedding_settings_local_overrides_memory_config() {
        ...
        assert_eq!(
            dims,
            crate::benito::embeddings::DEFAULT_OLLAMA_DIMENSIONS,
    ...
    fn embedding_settings_local_with_empty_model_uses_default() {
        ...
        assert_eq!(
            model,
            crate::benito::embeddings::DEFAULT_OLLAMA_MODEL,
    ...
    /// Sets `BENITO_OLLAMA_BASE_URL` to a deliberately unreachable address
    ...
    fn ollama_health_gate_reports_at_most_once_per_process() {
        let _lock = crate::benito::inference::local::inference_test_guard();
        reset_health_gate_for_test();
