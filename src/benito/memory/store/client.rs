//! # Memory Client
//!
//! High-level client interface for interacting with the Benito memory system.
//!
...
use crate::benito::embeddings::{self, EmbeddingProvider};
use crate::benito::memory::ingestion::queue as ingestion_queue;
use crate::benito::memory::ingestion::{
    IngestionJob, IngestionQueue, IngestionState, MemoryIngestionConfig, MemoryIngestionRequest,
    MemoryIngestionResult,
};
use crate::benito::memory::store::types::{
...
/// [`MemoryConfig.embedding_provider`](crate::benito::config::MemoryConfig)
/// resolves to — cloud (Benito backend, the default returned by
/// [`crate::benito::embeddings::default_embedding_provider`]) or local Ollama
...
/// store via [`crate::benito::memory::create_memory_with_storage_and_routes`]
/// (or [`crate::benito::memory::create_memory_with_local_ai`]) with the
...
impl MemoryClient {
    ...
    /// [`crate::benito::memory::store::unified::profile::profile_upsert`].
    ///
    ...
    pub fn memory_handle(&self) -> Arc<dyn crate::benito::memory::Memory> {
        Arc::clone(&self.inner) as Arc<dyn crate::benito::memory::Memory>
    }

    /// Create a new local memory client using the default `.Benito` directory.
    ///
    /// # Errors
    ///
    ...
    pub fn new_local() -> Result<Self, String> {
        let workspace_dir = crate::benito::config::default_root_BENITO_dir()
    ...
    pub fn from_workspace_dir(workspace_dir: PathBuf) -> Result<Self, String> {
        ...
        // Default to cloud embeddings (Benito backend, Voyage-backed). The
    ...
    /// Used by the `Benito.memory_ingestion_status` RPC handler.
