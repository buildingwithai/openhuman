...
use crate::benito::memory::{NamespaceDocumentInput, UnifiedMemory};
...
async fn recall_namespace_memories_includes_namespace_kv() {
    ...
    assert!(hits
        .iter()
        .any(|hit| matches!(hit.kind, crate::benito::memory::MemoryItemKind::Kv)));
}
...
async fn query_returns_episodic_hits_when_available() {
    ...
    let episodic_hits: Vec<_> = hits
        .iter()
        .filter(|h| h.kind == crate::benito::memory::MemoryItemKind::Episodic)
        .collect();
    assert!(
...
async fn query_returns_event_hits_when_available() {
    ...
    let event_hits: Vec<_> = hits
        .iter()
        .filter(|h| h.kind == crate::benito::memory::MemoryItemKind::Event)
        .collect();
    assert!(
        !event_hits.is_empty(),
...
async fn query_episodic_hits_have_correct_kind() {
    ...
    for hit in hits.iter().filter(|h| h.id.starts_with("episodic:")) {
        assert_eq!(
            hit.kind,
            crate::benito::memory::MemoryItemKind::Episodic,
