...
async fn gmail_fixture_ingestion_recovers_required_signals() {
    ...
    let result = memory
        .ingest_document(MemoryIngestionRequest {
            document: NamespaceDocumentInput {
                ...
                title: "Memory integration plan for Benito desktop".to_string(),
    ...
    assert!(result
        .entities
        .iter()
        .any(|entity| entity.name == "Benito"));
    assert!(result
        .relations
        .iter()
        .any(|relation| relation.subject == "Benito"
    ...
    assert!(recall
        .hits
        .iter()
        .any(|hit| hit.content.contains("Benito") || hit.content.contains("JSON-RPC")));
    assert!(recall
        .hits
        .iter()
    ...
    assert!(memories
        .iter()
        .any(|hit| matches!(hit.kind, crate::benito::memory::MemoryItemKind::Document)));
    assert!(memories
        .iter()
        .any(|hit| !hit.supporting_relations.is_empty()));
}
...
async fn notion_fixture_ingestion_recovers_required_signals() {
    ...
    let result = memory
        .ingest_document(MemoryIngestionRequest {
            document: NamespaceDocumentInput {
                ...
                title: "Benito Memory Layer Roadmap".to_string(),
    ...
    let graph_rows = memory
        .graph_query_namespace("skill-notion", Some("Benito"), Some("USES"))
        .await
        .unwrap();
    ...
    assert!(recall
        .hits
        .iter()
        .any(|hit| hit.content.contains("Benito")));
    ...
    assert!(memories
        .iter()
        .any(|hit| hit.content.contains("Benito") || hit.content.contains("core-first")));
    assert!(memories
        .iter()
        .any(|hit| matches!(hit.kind, crate::benito::memory::MemoryItemKind::Document)));
    assert!(memories
