...
use super::*;
use crate::benito::memory::tree::types::chunk_id;
use chrono::TimeZone;
use rusqlite::params;
...
/// Regression: Benito-TAURI-HH / -ZM / -MB.
///
...
fn legacy_embeddings_migrate_to_sidecar_once() {
    ...
    let (p, m, dims) = crate::benito::memory::store::effective_embedding_settings(
        &cfg.memory,
        cfg.workload_local_model("embeddings").as_deref(),
    );
    let sig = crate::benito::embeddings::format_embedding_signature(&p, &m, dims);
