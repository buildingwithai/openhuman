...
use crate::benito::memory::tree::content_store::StagedChunk;
use crate::benito::memory::tree::types::{Chunk, Metadata, SourceKind, SourceRef};
...
// busy returns (see Benito-TAURI-BP).
...
fn migrate_legacy_embeddings_to_sidecar(conn: &Connection, config: &Config) -> Result<()> {
    ...
    let (provider, model, dims) = crate::benito::memory::store::effective_embedding_settings(
        &config.memory,
        config.workload_local_model("embeddings").as_deref(),
    );
    let sig = crate::benito::embeddings::format_embedding_signature(&provider, &model, dims);
    log::info!(
    ...
    if has_uncovered {
        let backfill_job = crate::benito::memory::tree::jobs::types::NewJob::reembed_backfill(
            &crate::benito::memory::tree::jobs::types::ReembedBackfillPayload {
                signature: sig.clone(),
            },
        )?;
        crate::benito::memory::tree::jobs::enqueue_tx(&tx, &backfill_job)?;
...
pub(crate) fn tree_active_signature(config: &Config) -> String {
    ...
    crate::benito::memory::store::active_embedding_signature(
        &config.memory,
