...
use crate::benito::memory::tree::content_store::paths::{
...
pub const benito_core_VERSION: &str = env!("CARGO_PKG_VERSION");
...
fn build_summary_front_matter(r: &SummaryComposeInput<'_>) -> String {
    ...
    fm.push_str(&format!(
        "benito_core_version: {}\n",
        yaml_scalar(benito_core_VERSION)
    ));
    fm.push_str(&format!(
...
fn upsert_summary_provenance(front_matter: &str) -> String {
    ...
    for raw in front_matter.lines() {
        if raw.starts_with("benito_core_version: ")
...
mod tests {
    ...
    use crate::benito::memory::tree::types::{Metadata, SourceKind, SourceRef};
    use chrono::TimeZone;
