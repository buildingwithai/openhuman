...
//! [`crate::benito::scheduler_gate`] — its global single-slot
...
use crate::benito::memory::tree::jobs::handlers;
use crate::benito::memory::tree::jobs::redact::scrub_for_log;
use crate::benito::memory::tree::jobs::store::{
    claim_next, mark_deferred, mark_done, mark_failed, recover_stale_locks,
    DEFAULT_LOCK_DURATION_MS,
};
use crate::benito::memory::tree::jobs::types::JobOutcome;
...
pub async fn run_once(config: &Config) -> Result<bool> {
    ...
    let gate_permit = crate::benito::scheduler_gate::wait_for_capacity().await;
