...
pub fn ensure_reembed_backfill(config: &crate::benito::config::Config) {
    let sig = crate::benito::memory::tree::store::tree_active_signature(config);
    let result = crate::benito::memory::tree::store::with_connection(config, |conn| {
