...
use crate::benito::memory::tree::content_store::{
    self as content_store, read as content_read, tags as content_tags,
};
use crate::benito::memory::tree::jobs::store;
use crate::benito::memory::tree::jobs::types::{
...
use crate::benito::memory::tree::score;
use crate::benito::memory::tree::score::embed::{build_embedder_from_config, pack_checked};
use crate::benito::memory::tree::score::extract::build_summary_extractor;
use crate::benito::memory::tree::score::store as score_store;
use crate::benito::memory::tree::store as chunk_store;
use crate::benito::memory::tree::tree_global::digest::{self, DigestOutcome};
use crate::benito::memory::tree::tree_source::{
    build_summariser, get_or_create_source_tree, LabelStrategy, LeafRef,
};
use crate::benito::memory::tree::tree_topic::curator;
        ...
        if let Some(content_path) = chunk_store::get_chunk_content_path(config, &chunk.id)? {
            ...
            if let Err(e) = content_tags::update_chunk_tags(&abs_path, &obsidian_tags) {
                log::warn!(
                    ...
                    chunk.id,
                    crate::benito::memory::tree::util::redact::redact(&content_path),
                );
...
async fn handle_append_buffer(config: &Config, job: &Job) -> Result<JobOutcome> {
    use crate::benito::memory::tree::tree_source::bucket_seal::should_seal;
    use crate::benito::memory::tree::tree_source::store as src_store;
...
async fn handle_seal(config: &Config, job: &Job) -> Result<JobOutcome> {
    use crate::benito::memory::tree::tree_source::bucket_seal::{seal_one_level, should_seal};
    use crate::benito::memory::tree::tree_source::store as src_store;
    use crate::benito::memory::tree::tree_source::types::TreeKind;
...
async fn handle_topic_route(config: &Config, job: &Job) -> Result<JobOutcome> {
    ...
    let node_id: String = match &payload.node {
        ...
        NodeRef::Summary { summary_id } => {
            if crate::benito::memory::tree::tree_source::store::get_summary(config, summary_id)?
                .is_none()
            {
                log::warn!(
    ...
    for entity_id in entity_ids {
        ...
        if let Some(tree) = crate::benito::memory::tree::tree_source::store::get_tree_by_scope(
            config,
            crate::benito::memory::tree::tree_source::types::TreeKind::Topic,
            &entity_id,
        )? {
...
async fn handle_flush_stale(config: &Config, job: &Job) -> Result<JobOutcome> {
    ...
    let buffers =
        crate::benito::memory::tree::tree_source::store::list_stale_buffers(config, cutoff)?;
    for buf in buffers {
        let seal = SealPayload {
...
async fn handle_reembed_backfill(config: &Config, job: &Job) -> Result<JobOutcome> {
    ...
    if chunk_ids.is_empty() && summary_ids.is_empty() {
        crate::benito::memory::tree::jobs::set_backfill_in_progress(false);
        log::info!(
            "[memory_tree::jobs] reembed_backfill: sig={active_sig} fully covered; chain complete"
        );
        return Ok(JobOutcome::Done);
    }
    crate::benito::memory::tree::jobs::set_backfill_in_progress(true);
    ...
    chunk_store::with_connection(config, |conn| {
        ...
        for (id, v) in &summary_vecs {
            crate::benito::memory::tree::tree_source::store::set_summary_embedding_for_signature_tx(
                &tx, id, &active_sig, v,
            )?;
        }
        tx.commit()?;
...
mod tests {
    use super::*;
    use crate::benito::memory::tree::content_store;
    use crate::benito::memory::tree::jobs::store::{count_by_status, count_total};
    use crate::benito::memory::tree::jobs::types::JobStatus;
    use crate::benito::memory::tree::store::with_connection;
    use crate::benito::memory::tree::tree_source::bucket_seal::{append_leaf_deferred, LeafRef};
    use crate::benito::memory::tree::tree_source::registry::get_or_create_source_tree;
    use crate::benito::memory::tree::tree_source::store as src_store;
    use chrono::TimeZone;
    use rusqlite::params;
    ...
    async fn seed_source_tree_ready_to_seal(
        cfg: &Config,
    ) -> crate::benito::memory::tree::tree_source::types::Tree {
        use crate::benito::memory::tree::store::upsert_chunks;
        use crate::benito::memory::tree::types::{
        ...
        with_connection(cfg, |conn| {
            let tx = conn.unchecked_transaction()?;
            crate::benito::memory::tree::store::upsert_staged_chunks_tx(&tx, &staged)?;
            tx.commit()?;
            Ok(())
        })
        .unwrap();
    ...
    async fn topic_tree_seal_handler_does_not_enqueue_topic_route() {
        ...
        let topic_tree =
            crate::benito::memory::tree::tree_topic::registry::get_or_create_topic_tree(
                &cfg,
    ...
    async fn handle_append_buffer_with_summary_payload_pushes_into_topic_tree() {
        ...
        use crate::benito::memory::tree::store::upsert_chunks;
        use crate::benito::memory::tree::tree_source::bucket_seal::seal_one_level;
        use crate::benito::memory::tree::types::{
            chunk_id, Chunk, Metadata, SourceKind, SourceRef,
        };
        let ts = chrono::Utc.timestamp_millis_opt(1_700_000_000_000).unwrap();
        ...
        let summary_id = seal_one_level(
            ...
            &buf,
            summariser.as_ref(),
            &crate::benito::memory::tree::tree_source::bucket_seal::LabelStrategy::Empty,
    ...
    async fn reembed_backfill_repopulates_then_completes() {
        use crate::benito::memory::tree::store::{
    ...
    async fn ensure_reembed_backfill_enqueues_only_when_uncovered() {
        use crate::benito::memory::tree::jobs::ensure_reembed_backfill;
        use crate::benito::memory::tree::store::{upsert_chunks, upsert_staged_chunks_tx};
        use crate::benito::memory::tree::types::{
            chunk_id, Chunk, Metadata, SourceKind, SourceRef,
        };
