...
use crate::benito::memory::tree::retrieval::types::{
    hit_from_summary, QueryResponse, RetrievalHit,
};
use crate::benito::memory::tree::score::embed::{build_embedder_from_config, cosine_similarity};
use crate::benito::memory::tree::tree_source::store;
use crate::benito::memory::tree::tree_source::types::{SummaryNode, Tree, TreeKind};
...
mod tests {
    ...
    use crate::benito::memory::tree::tree_source::bucket_seal::{
        append_leaf, LabelStrategy, LeafRef,
    };
    use crate::benito::memory::tree::tree_source::registry::get_or_create_source_tree;
    use crate::benito::memory::tree::tree_source::summariser::inert::InertSummariser;
    use crate::benito::memory::tree::types::{chunk_id, Chunk, Metadata, SourceKind, SourceRef};
    ...
    async fn seed_source(cfg: &Config, scope: &str, ts: DateTime<Utc>) {
        ...
        for seq in 0..2u32 {
            let c = Chunk {
                ...
                token_count: crate::benito::memory::tree::tree_source::types::INPUT_TOKEN_BUDGET
                    * 6
                    / 10,
                seq_in_source: seq,
                created_at: ts,
            ...
            crate::benito::memory::tree::store::with_connection(cfg, |conn| {
            ...
            append_leaf(
                ...
                &LeafRef {
                    chunk_id: c.id.clone(),
                    token_count:
                        crate::benito::memory::tree::tree_source::types::INPUT_TOKEN_BUDGET * 6
                            / 10,
                    timestamp: ts,
    ...
    async fn query_reranks_by_cosine_similarity() {
        use crate::benito::memory::tree::score::embed::{pack_embedding, EMBEDDING_DIM};
        ...
        let phoenix_tree = src_store::get_tree_by_scope(
            &cfg,
            crate::benito::memory::tree::tree_source::types::TreeKind::Source,
            "slack:#phoenix",
        )
        .unwrap()
        .unwrap();
        ...
        use crate::benito::memory::tree::score::embed::cosine_similarity;
