...
//! 3. **Default** — [`CloudEmbedder`] (Benito backend / Voyage,
...
//!
//! Env var overrides applied in [`crate::benito::config::load`]:
    ...
    match (
        ...
        _ => {
            ...
            if let Some(model) = config.workload_local_model("embeddings") {
                ...
                // Default for logged-in users: cloud (Benito backend /
                ...
                log::warn!(
                    "[memory_tree::embed::factory] no backend session found — \
                     using InertEmbedder (zero vectors). Log in to Benito, or \
