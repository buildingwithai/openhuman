...
//!
//! Adapts the Benito backend's `POST /openai/v1/embeddings` surface
...
//! [`BenitoCloudEmbedding`], so a session refresh between batches is
...
use crate::benito::embeddings::cloud::{
    BenitoCloudEmbedding, DEFAULT_CLOUD_EMBEDDING_DIMENSIONS, DEFAULT_CLOUD_EMBEDDING_MODEL,
};
use crate::benito::embeddings::EmbeddingProvider;

/// Cloud-backed memory_tree embedder.
///
/// Wraps [`BenitoCloudEmbedding`] (which speaks the OpenAI-compatible
/// `/openai/v1/embeddings` shape backed by Voyage on the Benito
/// backend) and adapts it to the memory_tree [`Embedder`] trait.
pub struct CloudEmbedder {
    inner: BenitoCloudEmbedding,
}
...
impl CloudEmbedder {
    ...
    pub fn new(config: &Config) -> Self {
        let BENITO_dir = config.config_path.parent().map(std::path::PathBuf::from);
        Self {
            inner: BenitoCloudEmbedding::new(
                None,
                BENITO_dir,
