...
//! `qwen2.5:0.5b`) because Benito already ran Ollama for embeddings.
//! After the cloud-default refactor, the same prompt now routes through
//! whichever backend the workspace selected — typically the Benito
...
/// [`crate::benito::memory::tree::chat::build_chat_provider`]). Tests
...
Kinds guide:
  ...
  product      commercial offering                    (\"Claude Code\", \"Benito\")
