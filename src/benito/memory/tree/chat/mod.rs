...
//!
//! - **Cloud** — `providers::router` against the Benito backend with
...
//!   `BENITO_MEMORY_TREE_LLM_BACKEND=local`.
//!
//! ## Why a memory-tree-local trait
//!
//! The existing top-level [`crate::benito::inference::provider::Provider`] trait
...
use crate::benito::config::{Config, DEFAULT_CLOUD_LLM_MODEL};
...
///   wires [`cloud::CloudChatProvider`] against the Benito backend with
...
pub fn build_chat_provider(
    ...
    if let Some(routed_model) = config.workload_local_model("memory") {
        ...
        // BENITO_dir is the parent of config_path. Without this the
        // inner BenitoBackendProvider falls back to `~/.Benito`
        // and fails with "No backend session" on any workspace not
        // located at the home default — the bug observed when running
        // with `BENITO_WORKSPACE` pointed elsewhere.
