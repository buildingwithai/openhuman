//! Cloud chat provider — routes through the Benito backend's
...
use crate::benito::inference::provider::traits::{ChatMessage, Provider};
...
impl CloudChatProvider {
    ...
    /// matching the existing `BenitoBackendProvider` contract. That way
    /// a session refresh between memory-tree calls is picked up
    /// transparently.
    ///
    /// `BENITO_dir` is the directory containing `auth-profiles.json` (i.e.
    /// the parent of `config.config_path`). Without it the inner provider
    /// would fall back to `~/.Benito` and fail with "No backend session"
    ...
    pub fn new(
        ...
        let inner = BenitoBackendProvider::new(api_url.as_deref(), &opts);
...
mod tests {
    ...
    fn detects_model_unavailable_error() {
        let err = anyhow::anyhow!(
            "Benito API error (404 Not Found): {{\"success\":false,\"error\":\"GMI model \
    ...
    fn generic_404_with_model_not_treated_as_unavailable() {
        ...
        let err =
            anyhow::anyhow!("Benito API error (404 Not Found): model endpoint returned 404");
        assert!(!is_model_unavailable_error(&err));
    }
