...
use crate::benito::memory::tree::canonicalize::{
    chat::{self, ChatBatch},
    document::{self, DocumentInput},
    email::{self, EmailThread},
    CanonicalisedSource,
};
use crate::benito::memory::tree::chunker::{chunk_markdown, ChunkerInput, ChunkerOptions};
use crate::benito::memory::tree::content_store;
use crate::benito::memory::tree::jobs::{self, ExtractChunkPayload, NewJob};
use crate::benito::memory::tree::score::{self, ScoreResult, ScoringConfig};
            ...
            match std::panic::catch_unwind(move || markdown_body_preview(&md_for_preview)) {
                ...
                Err(_) => {
                    log::error!(
                        ...
                        crate::benito::memory::tree::util::redact::redact(source_id)
                    );
                    None
                }
            }
...
fn markdown_body_preview(md: &str) -> String {
    ...
    if len <= BODY_PREVIEW_MAX_BYTES {
        ...
        let start = crate::benito::util::ceil_char_boundary(md, len - BODY_PREVIEW_MAX_BYTES);
        debug_assert!(
...
mod tests {
    use super::*;
    use crate::benito::memory::tree::canonicalize::chat::ChatMessage;
    use crate::benito::memory::tree::jobs::drain_until_idle;
    use crate::benito::memory::tree::score::store::{count_scores, lookup_entity};
    use crate::benito::memory::tree::store::{
