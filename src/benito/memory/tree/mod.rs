...
//!
//! This is an isolated subdir under `benito::memory` implementing the
...
//! - `Benito.memory_tree_ingest` — unified ingest; caller supplies
//!   `source_kind` (chat|email|document) and a JSON `payload` whose shape
//!   the handler validates based on the kind
//! - `Benito.memory_tree_list_chunks`
//! - `Benito.memory_tree_get_chunk`
//!
