...
use crate::benito::memory::tree::content_store::read as content_read;
use crate::benito::memory::tree::retrieval::types::NodeKind;
use crate::benito::memory::tree::score::store as score_store;
use crate::benito::memory::tree::store::{self as chunk_store, with_connection};
use crate::benito::memory::tree::types::SourceKind;
...
///
/// Distinct from [`crate::benito::memory::tree::types::Chunk`] in two
...
pub async fn recall_rpc(
    ...
    let resp = crate::benito::memory::tree::retrieval::query_source(
        config,
        None,
        None,
        None,
    ...
    let result = tokio::task::spawn_blocking(move || -> Result<Option<ScoreBreakdown>> {
        ...
        Ok(row.map(|r| {
            ...
            ScoreBreakdown {
                signals,
                total: r.total,
                threshold: crate::benito::memory::tree::score::DEFAULT_DROP_THRESHOLD,
                kept: !r.dropped,
                llm_consulted,
            }
        }))
                ...
                if let Err(e) = std::fs::remove_file(&path) {
                    if e.kind() != std::io::ErrorKind::NotFound {
                        log::warn!(
                            ...
                            crate::benito::memory::tree::util::redact::redact(&rel),
                        );
                    }
                }
            }
...
pub async fn graph_export_rpc(
    ...
    let log = format!(
        ...
        resp.nodes.len(),
        resp.edges.len(),
        crate::benito::memory::tree::util::redact::redact(&resp.content_root_abs),
    );
...
pub async fn wipe_all_rpc(config: &Config) -> Result<RpcOutcome<WipeAllResponse>, String> {
    ...
    for dir in DIRS {
        ...
        let remove_result = crate::benito::util::retry_with_backoff_async(
...
/// keyed under [`crate::benito::composio::providers::sync_state::KV_NAMESPACE`].
///
/// We open the SQLite file directly rather than going through
/// [`crate::benito::memory::store::client::MemoryClientRef`] so
...
fn clear_composio_sync_state(db_path: &std::path::Path) -> Result<u64> {
    use crate::benito::composio::providers::sync_state::KV_NAMESPACE;
...
pub async fn reset_tree_rpc(config: &Config) -> Result<RpcOutcome<ResetTreeResponse>, String> {
    use crate::benito::memory::tree::jobs::store as jobs_store;
    use crate::benito::memory::tree::jobs::types::{ExtractChunkPayload, NewJob};
    ...
    crate::benito::memory::tree::jobs::wake_workers();
...
pub async fn flush_now_rpc(config: &Config) -> Result<RpcOutcome<FlushNowResponse>, String> {
    use crate::benito::memory::tree::jobs::store as jobs_store;
    use crate::benito::memory::tree::jobs::types::{FlushStalePayload, NewJob};
    use crate::benito::memory::tree::tree_source::store as tree_store;
...
pub async fn set_llm_rpc(
    ...
    let parsed = crate::benito::config::LlmBackend::parse(&req.backend)
    ...
    staged.memory_provider = Some(match parsed {
        ...
        crate::benito::config::schema::LlmBackend::Cloud => "cloud".to_string(),
    });
...
mod tests {
    use super::*;
    use crate::benito::memory::tree::canonicalize::chat::{ChatBatch, ChatMessage};
    use crate::benito::memory::tree::ingest::ingest_chat;
    ...
    async fn set_llm_switches_in_memory_and_persists_to_config_toml() {
        ...
        assert_eq!(
            ...
            crate::benito::config::LlmBackend::Local
        );
