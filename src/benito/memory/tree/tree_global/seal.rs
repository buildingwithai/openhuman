...
use crate::benito::memory::tree::tree_global::{
...
use crate::benito::memory::tree::tree_source::types::{Buffer, SummaryNode, Tree, TreeKind};
...
async fn seal_one_level(
    ...
    let global_scope_slug =
        crate::benito::memory::tree::content_store::paths::slugify_source_id(&tree.scope);
    ...
    fn insert_daily(cfg: &Config, node: &SummaryNode) {
        with_connection(cfg, |conn| {
            ...
            store::insert_summary_tx(
                ...
                node,
                None,
                &crate::benito::memory::tree::store::tree_active_signature(cfg),
            )?;
            tx.commit()?;
            Ok(())
        })
