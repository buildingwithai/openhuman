...
//! - `Benito.memory_tree_ingest` — one unified ingest. Caller supplies
//!   `source_kind` + generic JSON `payload` (adapter-specific). Internally
//!   dispatches to chat / email / document canonicalisers.
//! - `Benito.memory_tree_list_chunks` — listing with filters.
//! - `Benito.memory_tree_get_chunk` — single chunk fetch.
...
use crate::benito::memory::tree::store::{self, ListChunksQuery};
use crate::benito::memory::tree::types::{Chunk, SourceKind};
...
pub async fn trigger_digest_rpc(
    ...
    use crate::benito::memory::tree::jobs;
...
pub async fn backfill_status_rpc(
    ...
    let in_progress =
        crate::benito::memory::tree::jobs::backfill_in_progress() || pending_jobs > 0;
    Ok(RpcOutcome::single_log(
...
mod tests {
    use super::*;
    use crate::benito::memory::tree::jobs::store::count_total;
