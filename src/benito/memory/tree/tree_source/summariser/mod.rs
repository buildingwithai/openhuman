...
///   routed through the Benito backend's `cloud_llm_model`
...
pub fn build_summariser(config: &Config) -> Arc<dyn Summariser> {
    use crate::benito::config::DEFAULT_CLOUD_LLM_MODEL;
    use crate::benito::memory::tree::chat::{build_chat_provider, ChatConsumer};
