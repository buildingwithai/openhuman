...
use crate::benito::memory::tree::score::embed::build_embedder_from_config;
use crate::benito::memory::tree::score::extract::EntityExtractor;
use crate::benito::memory::tree::score::resolver::canonicalise;
use crate::benito::memory::tree::store::with_connection;
use crate::benito::memory::tree::tree_source::registry::new_summary_id;
use crate::benito::memory::tree::tree_source::store;
use crate::benito::memory::tree::tree_source::summariser::{
    Summariser, SummaryContext, SummaryInput,
};
use crate::benito::memory::tree::tree_source::types::{
...
/// by `flush_stale_buffers` (see [`crate::benito::memory::tree::
...
pub(crate) async fn seal_one_level(
    ...
    let child_basename_overrides: Option<Vec<Option<String>>> = if node.level == 1 {
        let overrides: Vec<Option<String>> = node
            ...
            .map(|chunk_id| {
                ...
                match crate::benito::memory::tree::store::get_chunk_raw_refs(config, chunk_id) {
    ...
    if let Err(err) =
        crate::benito::memory::tree::content_store::obsidian::ensure_obsidian_defaults(
            &content_root,
        )
    {
        log::warn!(
    ...
    with_connection(config, move |conn| {
        ...
        store::insert_summary_tx(
            ...
            &node,
            Some(&staged),
            &crate::benito::memory::tree::store::tree_active_signature(config),
        )?;
        ...
        crate::benito::memory::tree::score::store::index_summary_entity_ids_tx(
            &tx,
            &node.entities,
            &node.id,
            node.score,
        ...
        if enqueue_follow_ups {
            ...
            if should_seal(&parent) {
                use crate::benito::memory::tree::jobs::store::enqueue_tx as enqueue_job_tx;
                use crate::benito::memory::tree::jobs::types::{NewJob, SealPayload};
...
fn truncate_for_embed(text: &str, max_tokens: u32) -> String {
    let approx = crate::benito::memory::tree::types::approx_token_count(text);
...
fn hydrate_leaf_inputs(config: &Config, chunk_ids: &[String]) -> Result<Vec<SummaryInput>> {
    ...
    use crate::benito::memory::tree::score::store::{get_score, list_entity_ids_for_node};
    use crate::benito::memory::tree::store::get_chunk;
