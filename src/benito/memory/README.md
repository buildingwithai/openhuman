...
- `src/Benito/local_ai/` — embedding model, sentiment scoring, extraction LLM.
- `src/Benito/embeddings/` — vector backend selection.
- `src/Benito/config/` — memory backend choice + filesystem paths.
- `src/Benito/encryption/` — at-rest secrets for KV namespaces.
- `src/core/event_bus/` — emits `DomainEvent::Memory(*)` on ingestion / mutation.

## Called by

- `src/Benito/agent/` (`memory_loader.rs`, `harness/memory_context.rs`, `harness/archivist*.rs`, `harness/fork_context.rs`) — context injection and episodic indexing.
- `src/Benito/learning/{reflection,tool_tracker,user_profile,prompt_sections}.rs` — long-term insight storage.
- `src/Benito/screen_intelligence/{helpers,tests}.rs` — recall surfaces for visual context.
- `src/Benito/autocomplete/history.rs` — query-history recall.
- `src/Benito/tools/ops.rs` and `tools/impl/system/tool_stats.rs` — memory-backed tool stats.
