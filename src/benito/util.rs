...
/// # Examples
/// ```
/// use benito_core::benito::util::truncate_with_ellipsis;
///
    ...
    if let Some(io_err) = io_err {
        #[cfg(windows)]
        {
            if let Some(code) = io_err.raw_os_error() {
                ...
                //      the holder closes its handle. Sentry Benito-TAURI-H8
                //      bails at `elapsed_ms ≈ 2` against
                //      `Benito.team_get_usage` because this code was not
