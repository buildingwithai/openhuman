...
use crate::benito::socket::manager::global_socket_manager;
...
/// Lightweight diagnostic payload returned by `Benito.connectivity_diag`.
///
...
impl ListenerFingerprint {
    fn as_human_readable(&self) -> String {
        match self {
            Self::BenitoCore => "Benito-core".to_string(),
...
pub enum PickListenPortError {
    /// Port is occupied by another Benito core; caller should run the stale
...
/// - if still occupied by another Benito core, asks caller to takeover
...
async fn pick_listen_port_with_policy(
    ...
    if matches!(fingerprint, ListenerFingerprint::BenitoCore) {
...
async fn identify_listener(host: &str, port: u16) -> ListenerFingerprint {
    ...
    // mis-classifies live Benito cores on IPv6 hosts as `Other`.
    ...
    if is_BENITO_root_body(&body) {
        ListenerFingerprint::BenitoCore
    } else {
        let preview: String = body.chars().take(80).collect();
        ListenerFingerprint::Other(format!(
            "probe body did not identify as Benito ({preview:?})"
        ))
    }
}

fn is_BENITO_root_body(body: &str) -> bool {
...
fn resolve_listen_port() -> u16 {
    if let Ok(raw_url) = std::env::var("benito_core_RPC_URL") {
        if let Ok(url) = url::Url::parse(raw_url.trim()) {
            if let Some(port) = url.port() {
                debug!(
                    "[connectivity][rpc] resolve_listen_port: using benito_core_RPC_URL port={}",
                    port
                );
                return port;
            }
    ...
    if let Ok(raw) = std::env::var("benito_core_PORT") {
        match raw.trim().parse::<u16>() {
            ...
            Err(err) => {
                ...
                // on rpc.rs:56)
                warn!(
                    "[connectivity][rpc] resolve_listen_port: invalid benito_core_PORT='{}': {}",
                    raw, err
                );
            }
        }
...
mod tests {
    ...
    async fn spawn_BENITO_probe_listener(
        port: u16,
    ) -> (
            ...
            loop {
                tokio::select! {
                    ...
                    accepted = listener.accept() => {
                        ...
                        let body = r#"{"name":"Benito","ok":true}"#;
                        let response = format!(
    ...
    #[tokio::test]
    async fn pick_listen_port_BENITO_listener_requests_takeover() {
        let holder = reserve_port();
        let preferred = holder.local_addr().expect("preferred local addr").port();
        drop(holder);

        let (server_task, shutdown_tx) = spawn_BENITO_probe_listener(preferred).await;
        ...
        let err = result.expect_err("Benito listener should trigger takeover");
        assert!(
    ...
    fn resolve_listen_port_defaults_to_7788_when_env_unset() {
        ...
        let prev_port = std::env::var("benito_core_PORT").ok();
        // resolve_listen_port() also reads benito_core_RPC_URL ahead of
        // benito_core_PORT, so an inherited URL from the runner would
        // make this assertion nondeterministic. Save + clear both.
        let prev_url = std::env::var("benito_core_RPC_URL").ok();
        ...
        unsafe {
            std::env::remove_var("benito_core_PORT");
            std::env::remove_var("benito_core_RPC_URL");
        }
        assert_eq!(resolve_listen_port(), DEFAULT_CORE_PORT);
        if let Some(value) = prev_port {
            unsafe {
                std::env::set_var("benito_core_PORT", value);
            }
        }
        if let Some(value) = prev_url {
            unsafe {
                std::env::set_var("benito_core_RPC_URL", value);
            }
        }
    }
    ...
    fn resolve_listen_port_honours_env_override() {
        ...
        let prev_url = std::env::var("benito_core_RPC_URL").ok();
        unsafe {
            // Clear benito_core_RPC_URL so benito_core_PORT is the
            // resolved value (URL has higher priority in resolve_listen_port).
            std::env::remove_var("benito_core_RPC_URL");
            std::env::set_var("benito_core_PORT", "65000");
        }
        assert_eq!(resolve_listen_port(), 65000);
        match prev_port {
            Some(value) => unsafe { std::env::set_var("benito_core_PORT", value) },
            None => unsafe { std::env::remove_var("benito_core_PORT") },
        }
        match prev_url {
            Some(value) => unsafe { std::env::set_var("benito_core_RPC_URL", value) },
            None => unsafe { std::env::remove_var("benito_core_RPC_URL") },
        }
    }
    ...
    fn resolve_listen_port_falls_back_on_invalid_env() {
        ...
        unsafe {
            std::env::remove_var("benito_core_RPC_URL");
            std::env::set_var("benito_core_PORT", "not-a-number");
        }
        assert_eq!(resolve_listen_port(), DEFAULT_CORE_PORT);
        match prev_port {
            Some(value) => unsafe { std::env::set_var("benito_core_PORT", value) },
    ...
    #[test]
    fn resolve_listen_port_prefers_benito_core_rpc_url() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|p| p.into_inner());
        let prev_rpc = std::env::var("benito_core_RPC_URL").ok();
        let prev_port = std::env::var("benito_core_PORT").ok();
        unsafe {
            std::env::set_var("benito_core_RPC_URL", "http://127.0.0.1:7794/rpc");
            std::env::set_var("benito_core_PORT", "7788");
        }
