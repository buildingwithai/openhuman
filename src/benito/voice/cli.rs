...
//!
//! Handles the `Benito voice` / `Benito dictate` subcommand which runs a
...
use crate::benito::voice::server::{run_standalone, VoiceServerConfig};

/// Parse and execute the `Benito voice` / `Benito dictate` subcommand.
///
/// Supported flags:
...
pub(crate) fn run_standalone_subcommand(args: &[String]) -> Result<()> {
    ...
    rt.block_on(async {
        let mut config = match crate::benito::config::Config::load_or_init().await {
            Ok(cfg) => cfg,
            Err(e) => {
...
fn print_help() {
    println!("Usage: Benito voice [--hotkey <combo>] [--mode <tap|push>] [--skip-cleanup] [-v]");
    println!();
