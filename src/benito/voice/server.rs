...
#[cfg(target_os = "macos")]
use crate::benito::accessibility;
...
fn start_globe_hotkey_listener(
    ...
    String,
> {
    use crate::benito::accessibility::{globe_listener_poll, globe_listener_start};
...
async fn process_recording_bg(
    ...
    match handle.stop().await {
        Ok(result) => {
            ...
            match crate::benito::voice::voice_transcribe_bytes(
                config,
                &result.wav_bytes,
                Some("wav".to_string()),
                context,
                ...
                Ok(outcome) => {
                    ...
                    if !text.trim().is_empty() {
                        ...
                        let is_self = expected_app
                            .as_deref()
                            .map(|app| app.to_lowercase().contains("Benito"))
                            .unwrap_or(false);
