...
use crate::benito::voice::hotkey::{self, ActivationMode, HotkeyEvent};
...
pub async fn start_if_enabled(config: &Config) {
    ...
    let mode = match config.dictation.activation_mode {
        crate::benito::config::DictationActivationMode::Push => ActivationMode::Push,
        crate::benito::config::DictationActivationMode::Toggle => ActivationMode::Tap,
    };
