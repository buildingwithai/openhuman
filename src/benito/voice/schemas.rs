...
fn handle_voice_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::voice::voice_status(&config).await?)
    })
}
...
fn handle_voice_transcribe(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::voice::voice_transcribe(
                &config,
                &p.audio_path,
                p.context.as_deref(),
                p.skip_cleanup,
...
fn handle_voice_transcribe_bytes(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::voice::voice_transcribe_bytes(
                &config,
                &p.audio_bytes,
                p.extension,
                p.context.as_deref(),
...
fn handle_voice_tts(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::voice::voice_tts(&config, &p.text, p.output_path.as_deref()).await?,
        )
    })
}
...
fn handle_voice_reply_synthesize(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let voice = p
            ...
            .unwrap_or_else(|| {
                if provider_name == "piper" {
                    crate::benito::voice::DEFAULT_PIPER_VOICE.to_string()
                } else {
                    String::new()
                }
            });
        ...
        let provider =
            crate::benito::voice::create_tts_provider(&provider_name, &voice, &config)
...
fn handle_voice_cloud_transcribe(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let opts = crate::benito::voice::cloud_transcribe::CloudTranscribeOptions {
            model: p.model,
            language: p.language,
        ...
        to_json(
            crate::benito::voice::cloud_transcribe::transcribe_cloud(
                &config,
                &p.audio_base64,
                &opts,
            )
...
fn handle_voice_stt_dispatch(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let model = p
            ...
            .filter(|s| !s.is_empty())
            .map(str::to_string)
            .unwrap_or_else(|| crate::benito::voice::DEFAULT_WHISPER_MODEL.to_string());

        log::debug!(
            "[voice-factory] RPC voice_stt_dispatch provider={provider_name} model={model}"
        );
        let provider =
            crate::benito::voice::create_stt_provider(&provider_name, &model, &config)
                .map_err(|e| e.to_string())?;
...
fn effective_stt_provider(config: &crate::benito::config::Config) -> String {
...
fn effective_tts_provider(config: &crate::benito::config::Config) -> String {
...
fn handle_voice_server_start(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        use crate::benito::voice::hotkey::ActivationMode;
        use crate::benito::voice::server::{global_server, VoiceServerConfig};
        ...
        let activation_mode = match params.get("activation_mode").and_then(|v| v.as_str()) {
            ...
            None => match config.voice_server.activation_mode {
                crate::benito::config::VoiceActivationMode::Push => ActivationMode::Push,
                crate::benito::config::VoiceActivationMode::Tap => ActivationMode::Tap,
            },
        };
        ...
        if let Some(existing) = crate::benito::voice::server::try_global_server() {
            let existing_status = existing.status().await;
            if existing_status.state != crate::benito::voice::server::ServerState::Stopped {
        ...
        if let Some(s) = crate::benito::voice::server::try_global_server() {
...
fn handle_voice_server_stop(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        if let Some(server) = crate::benito::voice::server::try_global_server() {
            server.stop().await;
            tokio::time::sleep(std::time::Duration::from_millis(200)).await;
            ...
            let status = crate::benito::voice::server::VoiceServerStatus {
                state: crate::benito::voice::server::ServerState::Stopped,
                hotkey: String::new(),
                activation_mode: crate::benito::voice::hotkey::ActivationMode::Push,
...
fn handle_overlay_stt_notify(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        use crate::benito::voice::dictation_listener::{
