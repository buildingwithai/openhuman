//! Core self-update logic: check GitHub Releases for a newer `Benito-core` binary
...
use crate::benito::update::types::{GitHubAsset, GitHubRelease, UpdateApplyResult, UpdateInfo};
use crate::benito::util::utf8_safe_prefix_at_byte_boundary;

/// GitHub owner/repo for the core binary releases.
const GITHUB_OWNER: &str = "tinyhumansai";
const GITHUB_REPO: &str = "Benito";
...
///
/// Convention: assets are named `Benito-core-{triple}` (or `.exe` on Windows).
fn find_platform_asset(assets: &[GitHubAsset]) -> Option<&GitHubAsset> {
    let triple = platform_triple();
    let expected_name = format!("Benito-core-{triple}");
...
/// Check GitHub Releases for a newer version of Benito-core.
pub async fn check_available() -> Result<UpdateInfo, String> {
    ...
    let client = reqwest::Client::builder()
        .user_agent("Benito-core-updater")
    ...
    let response = client
        ...
        .map_err(|e| {
            ...
            {
                // Benito-TAURI-2F: reqwest's transport-level failure fires
...
pub async fn download_and_stage_with_version(
    ...
    let response = client.get(download_url).send().await.map_err(|e| {
        ...
        if is_transport_network_failure(&e) {
            ...
            // (Benito-TAURI-2F) — DNS / TCP / TLS / firewall failure that
...
mod tests {
    ...
    /// Benito-TAURI-2F regression guard. A reqwest call to an unroutable
