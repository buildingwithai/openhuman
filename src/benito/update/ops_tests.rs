...
use super::*;
use crate::benito::config::UpdateRestartStrategy;
use crate::benito::update::types::{UpdateApplyResult, UpdateInfo};
...
async fn update_version_returns_cargo_pkg_version() {
    ...
    assert_eq!(v, crate::benito::update::current_version());
}
...
async fn update_version_returns_target_triple() {
    ...
    assert_eq!(triple, crate::benito::update::platform_triple());
}

#[tokio::test]
async fn update_version_asset_prefix_contains_target_triple() {
    let outcome = update_version().await;
    let triple = crate::benito::update::platform_triple();
    let prefix = outcome
        .value
        .get("asset_prefix")
        .and_then(|v| v.as_str())
    ...
    assert!(
        prefix.starts_with("Benito-core-"),
        "asset_prefix must start with 'Benito-core-', got: {prefix}"
    );
    assert!(
...
async fn supervisor_strategy_sets_restart_requested_false() {
    ...
    let applied = UpdateApplyResult {
        ...
        staged_path: "/tmp/Benito-core-test".to_owned(),
...
async fn supervisor_strategy_staged_path_preserved() {
    ...
    let applied = UpdateApplyResult {
        installed_version: "0.52.0".to_owned(),
        staged_path: "/tmp/Benito-core-x86_64".to_owned(),
        restart_required: true,
        restart_strategy: UpdateRestartStrategy::SelfReplace,
    };
    let result =
...
async fn supervisor_strategy_message_mentions_supervisor() {
    ...
    let applied = UpdateApplyResult {
        installed_version: "0.52.0".to_owned(),
        staged_path: "/tmp/Benito-core".to_owned(),
        restart_required: true,
        restart_strategy: UpdateRestartStrategy::SelfReplace,
    };
    let result =
...
async fn update_run_rejected_when_rpc_mutations_disabled() {
    ...
    cfg.update = crate::benito::config::UpdateConfig {
        rpc_mutations_enabled: false,
        ..crate::benito::config::UpdateConfig::default()
    };
