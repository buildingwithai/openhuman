        ...
        "apply" => ControllerSchema {
            ...
            inputs: vec![
                ...
                FieldSchema {
                    name: "asset_name",
                    ty: TypeSchema::String,
                    comment: "Asset file name (e.g. Benito-core-aarch64-apple-darwin).",
                    required: true,
                },
                FieldSchema {
                    name: "staging_dir",
...
fn handle_version(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async { to_json(crate::benito::update::rpc::update_version().await) })
}

fn handle_check(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async { to_json(crate::benito::update::rpc::update_check().await) })
}

fn handle_run(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async { to_json(crate::benito::update::rpc::update_run().await) })
}
...
fn handle_apply(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::update::rpc::update_apply(download_url, asset_name, staging_dir)
                .await,
        )
    })
}
