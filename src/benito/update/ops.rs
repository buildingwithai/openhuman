...
use crate::benito::config::{self, UpdateConfig, UpdateRestartStrategy};
use crate::benito::update;
use crate::benito::update::types::{
...
async fn build_run_result_from_staged_update(
    ...
    match restart_strategy {
        UpdateRestartStrategy::SelfReplace => {
            let restart_requested = match crate::benito::service::rpc::service_restart(
...
pub async fn update_version() -> RpcOutcome<Value> {
    let info = VersionInfo {
        ...
        asset_prefix: format!("Benito-core-{}", update::platform_triple()),
    };
    log::debug!(
...
pub async fn update_run() -> RpcOutcome<Value> {
    ...
    let policy = match enforce_update_mutation_policy("Benito.update_run").await {
        Ok(policy) => policy,
        Err(error) => {
...
fn validate_asset_name(name: &str) -> Result<(), String> {
    ...
    if !name.starts_with("Benito-core-") {
        return Err(format!(
            "asset_name must start with 'Benito-core-', got '{name}'"
        ));
    }
    Ok(())
}
...
///   - `asset_name` (string, required): must be a safe filename starting with `Benito-core-`.
...
pub async fn update_apply(
    ...
    let policy = match enforce_update_mutation_policy("Benito.update_apply").await {
        Ok(policy) => policy,
        Err(error) => {
            return RpcOutcome::single_log(
...
mod tests {
    ...
    async fn write_update_policy(tmp: &TempDir, update: UpdateConfig) {
        let mut cfg = crate::benito::config::Config {
            workspace_dir: tmp.path().join("workspace"),
            config_path: tmp.path().join("config.toml"),
            ..crate::benito::config::Config::default()
        };
        cfg.update = update;
    ...
    fn validate_asset_name_accepts_well_formed_core_asset() {
        validate_asset_name("Benito-core-aarch64-apple-darwin.tar.gz")
    ...
    fn validate_asset_name_rejects_path_separators_and_traversal() {
        for bad in [
            "Benito-core-../etc/passwd",
            "../Benito-core-x86.tar.gz",
            "Benito-core/x86.tar.gz",
            "Benito-core\\x86.tar.gz",
        ] {
    ...
    fn validate_asset_name_rejects_unprefixed_asset() {
        ...
        assert!(
            err.contains("must start with 'Benito-core-'"),
            "got: {err}"
        );
    }
    ...
    // path is resolved through the process-global `BENITO_WORKSPACE`
    ...
    async fn update_apply_rejects_non_github_url_before_network_call() {
        ...
        let outcome = update_apply(
            ...
            "Benito-core-x86_64.tar.gz".to_string(),
            None,
        )
        .await;
    ...
    async fn supervisor_restart_strategy_stages_without_restart_request() {
        let info = UpdateInfo {
            ...
            update_available: true,
            download_url: Some(
                "https://github.com/owner/repo/releases/download/v9/Benito-core".into(),
            ),
            asset_name: Some("Benito-core-x86_64-unknown-linux-gnu".into()),
            release_notes: None,
            published_at: None,
        };
        let applied = UpdateApplyResult {
            installed_version: "9.9.9".into(),
            staged_path: "/tmp/Benito-core".into(),
