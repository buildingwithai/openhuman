...
use crate::benito::config::schema::RuntimePythonConfig;
...
impl PythonBootstrap {
    ...
    pub async fn spawn_stdio(
        &self,
        spec: &crate::benito::runtime_python::process::PythonLaunchSpec,
    ) -> Result<tokio::process::Child> {
        let resolved = self.resolve().await?;
        crate::benito::runtime_python::process::spawn_stdio_process(&resolved, spec)
    }
}
...
impl PythonBootstrap {
    ...
    fn cache_root(&self) -> PathBuf {
        ...
        if let Some(user_cache) = dirs::cache_dir() {
            return user_cache.join("Benito").join("runtime-python");
        }
        PathBuf::from(".Benito").join("runtime-python")
    }
}
