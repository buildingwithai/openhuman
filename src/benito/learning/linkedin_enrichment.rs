...
use crate::benito::integrations::{build_client, IntegrationClient};
use regex::Regex;
use serde_json::json;
...
fn build_memory_client() -> anyhow::Result<crate::benito::memory::store::MemoryClient> {
    crate::benito::memory::store::MemoryClient::new_local()
...
async fn persist_linkedin_profile(
    memory: &crate::benito::memory::store::MemoryClient,
    url: &str,
    data: &serde_json::Value,
) -> anyhow::Result<()> {
    let content = format!(
