...
fn handle_rebuild_cache(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        use crate::benito::learning::stability_detector::StabilityDetector;
        use std::time::{SystemTime, UNIX_EPOCH};

        tracing::debug!("[learning.rebuild_cache] manual rebuild requested via RPC");

        let client = crate::benito::memory::global::client_if_ready()
...
fn handle_cache_stats(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        use crate::benito::learning::cache::FacetCache;
        use crate::benito::memory::store::profile::FacetState;
...
fn get_cache() -> Result<crate::benito::learning::cache::FacetCache, String> {
    let client = crate::benito::memory::global::client_if_ready()
        .ok_or_else(|| "memory client not ready".to_string())?;
    Ok(crate::benito::learning::cache::FacetCache::new(
        client.profile_conn(),
    ))
}
...
fn facet_to_json(f: &crate::benito::memory::store::profile::ProfileFacet) -> serde_json::Value {
    serde_json::json!({
        "key": f.key,
        "value": f.value,
...
fn handle_update_facet(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        use crate::benito::memory::store::profile::UserState;
...
fn handle_forget_facet(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        use crate::benito::memory::store::profile::{FacetState, UserState};
