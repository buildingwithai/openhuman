...
use crate::benito::context::prompt::{PromptContext, PromptSection};
use anyhow::Result;
...
impl LearnedContextSection {
    pub fn new(_memory: std::sync::Arc<dyn crate::benito::memory::Memory>) -> Self {
...
pub fn load_learned_from_cache(
    cache: &crate::benito::learning::cache::FacetCache,
) -> Vec<String> {
    ...
    use crate::benito::memory::store::profile::ProfileFacet;
    ...
    'outer: for indices in by_class.values() {
        for &idx in indices {
            ...
            let pinned =
                if f.user_state == crate::benito::memory::store::profile::UserState::Pinned {
                    " *(pinned)*"
                } else {
                    ""
                };
...
mod tests {
    use super::*;
    use crate::benito::context::prompt::LearnedContextData;
    ...
    fn load_learned_from_cache_formats_active_facets() {
        ...
        use crate::benito::memory::store::profile::{
    ...
    fn load_learned_from_cache_empty_when_no_active_facets() {
        use crate::benito::learning::cache::FacetCache;
        use crate::benito::memory::store::profile::PROFILE_INIT_SQL;
        use parking_lot::Mutex;
        use rusqlite::Connection;
