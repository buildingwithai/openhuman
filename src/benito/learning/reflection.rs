...
use crate::benito::config::{Config, LearningConfig, ReflectionSource};
use crate::benito::learning::candidate::{self, CueFamily, EvidenceRef, FacetClass};
use crate::benito::memory::{Memory, MemoryCategory};
...
pub struct ReflectionHook {
    ...
    provider: Option<Arc<dyn crate::benito::inference::provider::Provider>>,
...
impl ReflectionHook {
    ...
    async fn run_reflection(&self, prompt: &str) -> anyhow::Result<String> {
        match self.config.reflection_source {
            ReflectionSource::Local => {
                ...
                let service = crate::benito::inference::local::global(&self.full_config);
                service
    ...
    fn emit_candidates_from_reflection(output: &ReflectionOutput, episodic_id: i64) {
        ...
        for (n, pref) in output.user_preferences.iter().enumerate() {
            ...
            buf.push(crate::benito::learning::candidate::LearningCandidate {
...
impl PostTurnHook for ReflectionHook {
    ...
    async fn on_turn_complete(&self, ctx: &TurnContext) -> anyhow::Result<()> {
        ...
        if self.config.enabled {
            ...
            let episodic_id = crate::benito::learning::candidate::global().len() as i64;
