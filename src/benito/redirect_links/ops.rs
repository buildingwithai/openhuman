...
use crate::benito::redirect_links::store;
use crate::benito::redirect_links::types::{RedirectLink, RewriteReplacement, RewriteResult};
use crate::rpc::RpcOutcome;

/// URLs shorter than this are not worth rewriting — the `Benito://link/<id>`
...
fn short_url_regex() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r"Benito://link/([0-9a-f]+)").unwrap())
}
...
/// Rewrite every long URL in `text` to `Benito://link/<id>`, using the
...
/// Replace every `Benito://link/<id>` placeholder with its stored URL.
...
mod tests {
    ...
    fn inbound_shortens_long_urls_and_preserves_surrounding_text() {
        ...
        assert!(result.text.starts_with("click here: Benito://link/"));
    ...
    fn outbound_leaves_unknown_ids_unchanged() {
        let tmp = TempDir::new().unwrap();
        let cfg = test_config(&tmp);
        let text = "no match: Benito://link/ffffffff";
    ...
    fn rewrite_outbound_for_user_expands_placeholder_and_tags_public_url() {
        ...
        // Shorten LONG into an Benito:// placeholder, then craft outbound text
