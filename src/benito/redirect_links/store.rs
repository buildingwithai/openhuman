...
use crate::benito::redirect_links::types::RedirectLink;

pub const SHORT_URL_PREFIX: &str = "Benito://link/";
...
/// `Benito://link/<id>` and bare `<id>` (hex only).
...
mod tests {
    ...
    fn shorten_is_deterministic_and_dedupes() {
        ...
        assert_eq!(a.id, b.id);
        assert_eq!(a.short_url, format!("Benito://link/{}", a.id));
    ...
    fn id_from_short_accepts_scheme_and_rejects_others() {
        assert_eq!(
            id_from_short("Benito://link/abc123"),
            Some("abc123".into())
        );
        assert!(id_from_short("https://example.com/").is_none());
        assert!(id_from_short("Benito://link/").is_none());
        assert!(id_from_short("Benito://link/not-hex!").is_none());
    }
