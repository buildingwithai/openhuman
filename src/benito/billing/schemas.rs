...
fn handle_billing_get_current_plan(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::billing::get_current_plan(&config).await?)
    })
}
...
fn handle_billing_get_balance(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::benito::billing::get_balance(&config).await?)
    })
}
...
fn handle_billing_purchase_plan(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::billing::purchase_plan(&config, payload.plan.trim()).await?)
    })
}
...
fn handle_billing_create_portal_session(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::benito::billing::create_portal_session(&config).await?)
    })
}
...
fn handle_billing_top_up(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::billing::top_up_credits(&config, payload.amount_usd, payload.gateway)
                .await?,
        )
    })
}
...
fn handle_billing_create_coinbase_charge(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::billing::create_coinbase_charge(
                &config,
                payload.plan.trim(),
                payload.interval,
            )
...
fn handle_billing_get_transactions(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::billing::get_transactions(&config, payload.limit, payload.offset)
                .await?,
        )
    })
}
...
fn handle_billing_get_auto_recharge(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::benito::billing::get_auto_recharge(&config).await?)
    })
}
...
fn handle_billing_update_auto_recharge(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::billing::update_auto_recharge(&config, payload.payload).await?)
    })
}
...
fn handle_billing_get_cards(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::benito::billing::get_cards(&config).await?)
    })
}
...
fn handle_billing_create_setup_intent(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::benito::billing::create_setup_intent(&config).await?)
    })
}
...
fn handle_billing_update_card(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::billing::update_card(
                &config,
...
fn handle_billing_delete_card(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::billing::delete_card(&config, payload.payment_method_id.trim())
                .await?,
        )
    })
}
...
fn handle_billing_redeem_coupon(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::billing::redeem_coupon(&config, payload.code.trim()).await?)
    })
}
...
fn handle_billing_get_coupons(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::benito::billing::get_user_coupons(&config).await?)
    })
}
