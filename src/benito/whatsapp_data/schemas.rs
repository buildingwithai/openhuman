...
//!   - `Benito.whatsapp_data_list_chats`
//!   - `Benito.whatsapp_data_list_messages`
//!   - `Benito.whatsapp_data_search_messages`
//!
//! Internal write path (NOT exposed to the agent controller registry):
//!   - `Benito.whatsapp_data_ingest` — called by the Tauri scanner only
//!
...
/// `Benito.whatsapp_data_ingest` over JSON-RPC without exposing it to agents.
...
fn handle_ingest(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::whatsapp_data::rpc::whatsapp_data_ingest(req).await?)
    })
}
...
fn handle_list_chats(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let req = deserialize_params(params)?;
        to_json(crate::benito::whatsapp_data::rpc::whatsapp_data_list_chats(req).await?)
    })
}
...
fn handle_list_messages(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let req = deserialize_params(params)?;
        to_json(crate::benito::whatsapp_data::rpc::whatsapp_data_list_messages(req).await?)
    })
}
...
fn handle_search_messages(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let req = deserialize_params(params)?;
        to_json(crate::benito::whatsapp_data::rpc::whatsapp_data_search_messages(req).await?)
    })
}
