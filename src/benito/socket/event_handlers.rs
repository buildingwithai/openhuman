...
pub(super) fn handle_sio_event(
    ...
    match event_name {
        ...
        "webhook:request" => {
            ...
            match serde_json::from_value::<WebhookRequest>(data.clone()) {
                ...
                Err(e) => {
                    ...
                    if let Some(mgr) = crate::benito::socket::global_socket_manager() {
        ...
        "composio:trigger" => {
            ...
            match serde_json::from_value::<crate::benito::composio::ComposioTriggerEvent>(
                data.clone(),
            ) {
                Ok(event) => {
...
mod tests {
    ...
    fn handle_sio_event_debug_truncation_respects_utf8_boundary() {
        ...
        // straddling byte 500 — mirrors Benito-TAURI-KC (Cyrillic at 499..501).
    ...
    // Regression: Benito-TAURI-KC (#1814). A multi-byte UTF-8 char
