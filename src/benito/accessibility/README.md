...
- `src/Benito/config/` — overlay sizing and helper paths (light dependency).

## Called by

- `src/Benito/autocomplete/core/{terminal,text,overlay,types,focus}.rs` — focus-driven autocomplete needs every accessibility primitive.
- `src/Benito/screen_intelligence/{types,state,capture_worker,input,tests}.rs` — screen capture + focus context for vision pipelines.
- `src/Benito/voice/` — microphone permission + foreground app context (indirect, via re-exports).
