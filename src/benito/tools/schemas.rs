...
use crate::benito::tools::traits::Tool;
    ...
    match function {
        "tools_composio_execute" => ControllerSchema {
            ...
            description: "Execute a Composio action. Routes through the mode-aware \
                          factory: backend mode proxies via the Benito backend; \
        ...
        "tools_searxng_search" => ControllerSchema {
            ...
            description:
                "Web search via a user-configured SearXNG instance. Returns normalized \
                          ...
                          self-hosted search without routing queries through the Benito backend.",
            inputs: vec![
                FieldSchema {
                    name: "query",
                    ty: TypeSchema::String,
...
fn handle_composio_execute(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        use crate::benito::composio::client::{
...
fn handle_web_search(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        let client = crate::benito::integrations::build_client(&config).ok_or_else(|| {
        ...
        let resp = client
            .post::<crate::benito::integrations::parallel::SearchResponse>(
...
fn handle_seltz_search(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let tool = crate::benito::integrations::SeltzSearchTool::new(
...
fn handle_searxng_search(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        if !config.searxng.enabled {
            ...
            return Err(
                "SearXNG search is not enabled. Set searxng.enabled=true or BENITO_SEARXNG_ENABLED=true."
                    .to_string(),
            );
        }
        ...
        let tool = crate::benito::integrations::SearxngSearchTool::new(
        ...
        let response = tool
            .search(crate::benito::integrations::SearxngSearchArgs {
                query,
                categories,
                language,
                max_results,
...
fn handle_apify_linkedin_scrape(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let data = crate::benito::learning::linkedin_enrichment::scrape_linkedin_profile(
            &client,
            &profile_url,
        )
        .await
        ...
        let markdown = crate::benito::learning::linkedin_enrichment::render_profile_markdown(
            &profile_url,
            &data,
        );
...
fn handle_polymarket_execute(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let security = std::sync::Arc::new(crate::benito::security::SecurityPolicy::default());
        let tool = crate::benito::tools::implementations::network::PolymarketTool::new(
