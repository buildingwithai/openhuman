...
use crate::benito::agent::host_runtime::{NativeRuntime, RuntimeAdapter};
use crate::benito::config::{Config, DelegateAgentConfig};
use crate::benito::javascript::NodeBootstrap;
use crate::benito::memory::Memory;
...
pub fn all_tools(
    ...
    browser_config: &crate::benito::config::BrowserConfig,
    http_config: &crate::benito::config::HttpRequestConfig,
    workspace_dir: &std::path::Path,
    agents: &HashMap<String, DelegateAgentConfig>,
    root_config: &crate::benito::config::Config,
) -> Vec<Box<dyn Tool>> {
...
pub fn all_tools_with_runtime(
    ...
    if root_config.learning.stm_recall_enabled {
        tools.push(Box::new(
            crate::benito::memory::stm_recall::tool::StmRecallTool::new(
                memory.clone(),
                String::new(),
                None,
            ),
    ...
    // gitbooks — answers questions about Benito by calling the
    ...
    let mcp_registry =
        Arc::new(crate::benito::mcp_client::McpServerRegistry::from_config(root_config));
    ...
    let direct_seltz_for_web_search = if root_config.seltz.enabled && seltz_has_api_key {
        ...
        Some(crate::benito::integrations::SeltzSearchTool::new(
    ...
    tools.push(Box::new(
        WebSearchTool::new(
            crate::benito::integrations::build_client(root_config),
    ...
    if root_config.seltz.enabled {
        tools.push(Box::new(
            crate::benito::integrations::SeltzSearchTool::new(
                root_config.seltz.api_key.clone(),
                root_config.seltz.api_url.clone(),
                root_config.seltz.max_results,
                root_config.seltz.timeout_secs,
    ...
    // queries through Benito's backend or a hosted search API.
    if root_config.searxng.enabled {
        tools.push(Box::new(
            crate::benito::integrations::SearxngSearchTool::new(
    ...
    if !agents.is_empty() {
        ...
        tools.push(Box::new(DelegateTool::new_with_options(
            delegate_agents,
            security.clone(),
            crate::benito::inference::provider::ProviderRuntimeOptions {
                auth_profile_override: None,
                BENITO_dir: root_config
                    .config_path
                    .parent()
    ...
    if let Some(client) = crate::benito::integrations::build_client(root_config) {
        tracing::debug!("[integrations] client built successfully");
        if root_config.integrations.apify.is_active() {
            tools.push(Box::new(
                crate::benito::integrations::ApifyRunActorTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::benito::integrations::ApifyGetRunStatusTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::benito::integrations::ApifyGetRunResultsTool::new(Arc::clone(&client)),
            ));
        ...
        if root_config.integrations.google_places.is_active() {
            tools.push(Box::new(
                crate::benito::integrations::GooglePlacesSearchTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::benito::integrations::GooglePlacesDetailsTool::new(Arc::clone(&client)),
            ));
        ...
        if root_config.integrations.parallel.is_active() {
            tools.push(Box::new(
                crate::benito::integrations::ParallelSearchTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::benito::integrations::ParallelExtractTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::benito::integrations::ParallelChatTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::benito::integrations::ParallelResearchTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::benito::integrations::ParallelEnrichTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::benito::integrations::ParallelDatasetTool::new(Arc::clone(&client)),
            ));
        ...
        if root_config.integrations.tinyfish.is_active() {
            tools.push(Box::new(
                crate::benito::integrations::TinyFishSearchTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::benito::integrations::TinyFishFetchTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::benito::integrations::TinyFishAgentRunTool::new(Arc::clone(&client)),
            ));
        ...
        if root_config.integrations.stock_prices.is_active() {
            tools.push(Box::new(
                crate::benito::integrations::StockQuoteTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::benito::integrations::StockExchangeRateTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::benito::integrations::StockOptionsTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::benito::integrations::StockCryptoSeriesTool::new(Arc::clone(&client)),
            ));
            tools.push(Box::new(
                crate::benito::integrations::StockCommodityTool::new(Arc::clone(&client)),
            ));
        ...
        if root_config.integrations.twilio.is_active() {
            tools.push(Box::new(
                crate::benito::integrations::TwilioCallTool::new(Arc::clone(&client)),
            ));
        ...
        // `src/Benito/composio/tools.rs` for per-tool details.
        let composio_tools = crate::benito::composio::all_composio_agent_tools(root_config);
    ...
    // BENITO_LSP_ENABLED env var. The backend (real language-server
    // bridge) is a follow-up; today the gate just controls visibility
    // so agents don't see a method that always errors.
    if crate::benito::tools::implementations::lsp_capability_enabled() {
        tools.push(Box::new(
            crate::benito::tools::implementations::LspTool::new(),
        ));
        tracing::debug!("[lsp] capability gate on — LspTool registered");
    } else {
        tracing::debug!("[lsp] capability gate off (set BENITO_LSP_ENABLED=1 to register)");
    }
