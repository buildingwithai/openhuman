...
use crate::benito::audio_toolkit::{
...
fn optional_format(
    ...
    key: &str,
) -> anyhow::Result<Option<crate::benito::audio_toolkit::AudioFormat>> {
    let Some(raw) = args.get(key).and_then(|v| v.as_str()) else {
        return Ok(None);
    };
    match raw.trim() {
        "mp3" => Ok(Some(crate::benito::audio_toolkit::AudioFormat::Mp3)),
        "wav" => Ok(Some(crate::benito::audio_toolkit::AudioFormat::Wav)),
