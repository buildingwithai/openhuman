...
mod tests {
    ...
    async fn glob_matches_extension() {
        let dir = std::env::temp_dir().join("BENITO_test_glob_ext");
    ...
    async fn glob_invalid_pattern() {
        let dir = std::env::temp_dir().join("BENITO_test_glob_invalid");
        let _ = tokio::fs::remove_dir_all(&dir).await;
    ...
    async fn glob_skips_node_modules() {
        let dir = std::env::temp_dir().join("BENITO_test_glob_skip");
        let _ = tokio::fs::remove_dir_all(&dir).await;
