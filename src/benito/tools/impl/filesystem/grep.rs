        ...
        .filter_map(|e| e.ok())
    {
        ...
        for (lineno, line) in contents.lines().enumerate() {
            if regex.is_match(line) {
                ...
                // PR #1549.
                let display_line =
                    crate::benito::util::truncate_at_byte_boundary(line, MAX_LINE_BYTES);
...
mod tests {
    ...
    async fn grep_finds_matches() {
        let dir = std::env::temp_dir().join("BENITO_test_grep_finds");
    ...
    async fn grep_invalid_regex() {
        let dir = std::env::temp_dir().join("BENITO_test_grep_invalid");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn grep_case_insensitive() {
        let dir = std::env::temp_dir().join("BENITO_test_grep_ci");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn grep_skips_node_modules_and_git() {
        let dir = std::env::temp_dir().join("BENITO_test_grep_skip");
        let _ = tokio::fs::remove_dir_all(&dir).await;
    ...
    async fn grep_respects_max_matches() {
        let dir = std::env::temp_dir().join("BENITO_test_grep_max");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
