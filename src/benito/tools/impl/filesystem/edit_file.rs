...
mod tests {
    ...
    async fn edit_replaces_unique_match() {
        let dir = std::env::temp_dir().join("BENITO_test_edit_unique");
    ...
    async fn edit_rejects_ambiguous_match() {
        let dir = std::env::temp_dir().join("BENITO_test_edit_ambig");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn edit_replace_all() {
        let dir = std::env::temp_dir().join("BENITO_test_edit_all");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("f.txt"), "x x x").await.unwrap();
    ...
    async fn edit_no_match() {
        let dir = std::env::temp_dir().join("BENITO_test_edit_nomatch");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn edit_blocks_readonly_mode() {
        let dir = std::env::temp_dir().join("BENITO_test_edit_ro");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn edit_rejects_empty_old_string() {
        let dir = std::env::temp_dir().join("BENITO_test_edit_empty_old");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("f.txt"), "abc").await.unwrap();
    ...
    async fn edit_rejects_identical_strings() {
        let dir = std::env::temp_dir().join("BENITO_test_edit_same");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("f.txt"), "abc").await.unwrap();
