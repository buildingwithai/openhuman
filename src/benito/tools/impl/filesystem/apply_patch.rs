...
mod tests {
    ...
    async fn apply_patch_applies_multiple_edits() {
        let dir = std::env::temp_dir().join("BENITO_test_patch_multi");
    ...
    async fn apply_patch_atomic_on_validation_failure() {
        let dir = std::env::temp_dir().join("BENITO_test_patch_atomic");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn apply_patch_chained_edits_same_file() {
        let dir = std::env::temp_dir().join("BENITO_test_patch_chain");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn apply_patch_rejects_empty_edits() {
        let dir = std::env::temp_dir().join("BENITO_test_patch_empty");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
