...
impl Tool for CsvExportTool {
    ...
    fn permission_level(&self) -> crate::benito::tools::traits::PermissionLevel {
        crate::benito::tools::traits::PermissionLevel::Write
    }
...
mod tests {
    ...
    async fn csv_export_formats_simple_array() {
        let dir = std::env::temp_dir().join("BENITO_test_csv_export_simple");
    ...
    async fn csv_export_handles_missing_keys() {
        let dir = std::env::temp_dir().join("BENITO_test_csv_export_missing_keys");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn csv_export_respects_column_order() {
        let dir = std::env::temp_dir().join("BENITO_test_csv_export_column_order");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn csv_export_rejects_non_array_input() {
        let dir = std::env::temp_dir().join("BENITO_test_csv_export_non_array");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn csv_export_handles_nested_values() {
        let dir = std::env::temp_dir().join("BENITO_test_csv_export_nested");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
