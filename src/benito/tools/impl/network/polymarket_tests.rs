...
async fn configure_wallet_for_place_order_test(
    ...
    let encrypted_mnemonic = crate::benito::encryption::rpc::encrypt_secret(&config, mnemonic)
        .await
...
async fn place_order_happy_path_posts_signed_order() {
    ...
    let _workspace_guard = EnvVarGuard::set_to_path("BENITO_WORKSPACE", tmp.path());
