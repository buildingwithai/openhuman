use crate::benito::mcp_client::{McpRegistrySource, McpServerRegistry};
use crate::benito::security::{SecurityPolicy, ToolOperation};
use crate::benito::tools::traits::{PermissionLevel, Tool, ToolCallOptions, ToolResult};
...
impl Tool for McpListServersTool {
    ...
    fn description(&self) -> &str {
        "List named remote MCP servers registered in Benito core. Use this before browsing tools on a specific MCP server."
    }
        ...
        let markdown = if servers.is_empty() {
            ...
            for server in self.registry.list() {
                ...
                md.push_str(&format!(
                    ...
                    server.endpoint,
                    match &server.auth {
                        crate::benito::config::McpAuthConfig::None => "none",
                        crate::benito::config::McpAuthConfig::BearerToken { .. } =>
                            "bearer_token",
                        crate::benito::config::McpAuthConfig::Basic { .. } => "basic",
                        crate::benito::config::McpAuthConfig::Header { .. } => "header",
                        crate::benito::config::McpAuthConfig::QueryParam { .. } => "query_param",
                    }
                ));
...
mod tests {
    use super::*;
    use crate::benito::config::{Config, McpServerConfig};
    ...
    fn test_registry() -> Arc<McpServerRegistry> {
        ...
        config.mcp_client.servers.push(McpServerConfig {
            ...
            timeout_secs: 30,
            auth: crate::benito::config::McpAuthConfig::None,
        });
