//! `gitbooks` — answer questions about Benito by talking to the
//! GitBook MCP server through the shared `benito::mcp_client` path.

use crate::benito::mcp_client::{redact_endpoint, McpHttpClient};
...
impl Tool for GitbooksSearchTool {
    ...
    fn description(&self) -> &str {
        "Search the Benito product documentation. Use this to answer questions about how \
        Benito works, find features, look up configuration, or locate guides. Returns \
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "query": {
                    "type": "string",
                    "description": "Natural-language question or keyword query about Benito."
                }
            },
            "required": ["query"]
        })
...
impl Tool for GitbooksGetPageTool {
    ...
    fn description(&self) -> &str {
        "Fetch the full markdown of a specific Benito documentation page by URL. Pair this \
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "url": {
                    "type": "string",
                    "description": "The full URL of the Benito documentation page (e.g. https://tinyhumans.gitbook.io/Benito/getting-started)."
                }
            },
            "required": ["url"]
        })
...
mod tests {
    ...
    fn redact_endpoint_keeps_only_origin() {
        assert_eq!(
            redact_endpoint("https://tinyhumans.gitbook.io/Benito/~gitbook/mcp"),
    ...
    async fn live_search_smoke() {
        if std::env::var("BENITO_GITBOOKS_LIVE_TEST")
            .ok()
            .as_deref()
            != Some("1")
        {
        ...
        let t = GitbooksSearchTool::new(
            "https://tinyhumans.gitbook.io/Benito/~gitbook/mcp".into(),
            30,
        );
        let result = t
            .execute(json!({"query": "what is Benito"}))
            .await
            .unwrap();
        assert!(
            !result.is_error,
