use crate::benito::integrations::parallel::{SearchResponse, SearchResultItem};
use crate::benito::integrations::{IntegrationClient, SeltzSearchTool};
    ...
    fn parse_parallel_results(
        ...
        for (i, result) in results.iter().take(self.max_results).enumerate() {
            ...
            if let Some(first) = result.excerpts.first() {
                ...
                if !excerpt.is_empty() {
                    let truncated = crate::benito::util::truncate_with_ellipsis(excerpt, 500);
    ...
    fn render_results_markdown(&self, results: &[SearchResultItem], query: &str) -> String {
        ...
        for r in results.iter().take(self.max_results) {
            ...
            if let Some(first) = r.excerpts.first() {
                let excerpt = first.trim();
                if !excerpt.is_empty() {
                    let truncated = crate::benito::util::truncate_with_suffix(excerpt, 500, "…");
