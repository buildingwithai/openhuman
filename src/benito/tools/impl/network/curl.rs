...
impl Tool for CurlTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let builder =
            crate::benito::config::apply_runtime_proxy_to_builder(builder, "tool.curl");
...
mod tests {
    ...
    /// public page). Gated behind `BENITO_CURL_LIVE_TEST=1` so CI /
    /// offline runs don't depend on the network.
    #[tokio::test]
    async fn live_download_example_com() {
        if std::env::var("BENITO_CURL_LIVE_TEST").ok().as_deref() != Some("1") {
            return;
        }
        let tmp = TempDir::new().unwrap();
        let t = tool(&tmp, vec!["example.com"]);
