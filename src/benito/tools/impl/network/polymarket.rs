...
use crate::benito::config::{PolymarketClobCredentials, PolymarketConfig};
use crate::benito::security::policy::ToolOperation;
use crate::benito::security::SecurityPolicy;
use crate::benito::tools::traits::{Tool, ToolCategory, ToolResult};
use crate::benito::wallet::{secret_material, status as wallet_status, WalletChain};
...
impl PolymarketTool {
    pub fn new(config: &PolymarketConfig, security: Arc<SecurityPolicy>) -> Self {
        ...
        let builder =
            crate::benito::config::apply_runtime_proxy_to_builder(builder, "tool.polymarket");
    ...
    async fn resolve_signer_and_user(&self, user: Option<String>) -> Result<(LocalWallet, String)> {
        ...
        let mnemonic =
            crate::benito::encryption::rpc::decrypt_secret(&config, &secret.encrypted_mnemonic)
                .await
                .map_err(anyhow::Error::msg)
    ...
    async fn build_signed_limit_order(
        ...
        // pattern used by src/Benito/encryption/core.rs.
        let salt = {
...
fn summarize_error_body(body: &str) -> String {
    ...
    if compact.is_empty() {
        ...
        crate::benito::util::truncate_with_ellipsis(&compact, MAX_ERROR_BODY_CHARS)
    }
}
