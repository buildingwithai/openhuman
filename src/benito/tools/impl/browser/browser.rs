    ...
    fn validate_url(&self, url: &str) -> anyhow::Result<()> {
        ...
        if self.allowed_domains.is_empty() && !allow_all_browser_domains() {
            anyhow::bail!(
                ...
                Add [browser].allowed_domains in config.toml or set BENITO_BROWSER_ALLOW_ALL=1"
            );
        }
    ...
    async fn execute_computer_use_action(
        ...
        let payload = json!({
            ...
            "metadata": {
                ...
                "source": "Benito.browser",
                "version": env!("CARGO_PKG_VERSION"),
            }
        });

        let client = crate::benito::config::build_runtime_proxy_client("tool.browser");
