...
//!   heuristic (see `benito::webview_accounts`).
//!
...
pub(crate) async fn compute_state(
    ...
    let connected_channels =
        crate::benito::channels::controllers::connected_channel_slugs(config)
            .await
            .unwrap_or_else(|err| {
                tracing::warn!(
                    error = %err,
