...
use crate::benito::agent::harness::fork_context::current_parent;
use crate::benito::agent::harness::subagent_runner::{run_subagent, SubagentRunOptions};
use crate::benito::memory::conversations::{
    self, ConversationMessage, CreateConversationThread,
};
use crate::benito::tools::traits::{PermissionLevel, Tool, ToolResult};
...
impl Tool for SpawnWorkerThreadTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let current_thread_id =
            crate::benito::inference::provider::thread_context::current_thread_id()
...
mod tests {
    ...
    use crate::benito::agent::harness::ParentExecutionContext;
    ...
    struct MockProvider;
    #[async_trait]
    impl crate::benito::inference::provider::Provider for MockProvider {
        ...
        async fn chat(
            &self,
            _: crate::benito::inference::provider::ChatRequest<'_>,
            _: &str,
            _: f64,
        ) -> anyhow::Result<crate::benito::inference::provider::ChatResponse> {
            Ok(crate::benito::inference::provider::ChatResponse {
                text: Some("done".into()),
                tool_calls: vec![],
                usage: None,
            })
    ...
    struct MockMemory;
    #[async_trait]
    impl crate::benito::memory::Memory for MockMemory {
        async fn store(
            &self,
            _: &str,
            _: &str,
            _: &str,
            _: crate::benito::memory::MemoryCategory,
            _: Option<&str>,
        ) -> anyhow::Result<()> {
            Ok(())
        }
        async fn recall(
            ...
            _: &str,
            _: usize,
            _: crate::benito::memory::RecallOpts<'_>,
        ...
        async fn list(
            &self,
            _: Option<&str>,
            _: Option<&crate::benito::memory::MemoryCategory>,
            _: Option<&str>,
        ) -> anyhow::Result<Vec<crate::benito::memory::MemoryEntry>> {
            Ok(vec![])
        }
    ...
    fn test_parent_ctx(workspace_dir: PathBuf) -> ParentExecutionContext {
        ParentExecutionContext {
            ...
            tool_call_format: crate::benito::context::prompt::ToolCallFormat::Native,
        }
    }
    ...
    async fn rejects_if_already_worker_thread() {
        ...
        crate::benito::inference::provider::thread_context::with_thread_id(
            thread_id.to_string(),
            async {
