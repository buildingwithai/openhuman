...
use crate::benito::agent::harness::subagent_runner::{
...
/// [`crate::benito::tools::all_tools_with_runtime`]. The orchestrator
...
impl Tool for SpawnSubagentTool {
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        ...
        json!({
            ...
            "properties": {
                ...
                "dedicated_thread": {
                    "type": "boolean",
                    "description": "Temporarily disabled (see tinyhumansai/Benito#1624). Passing `true` causes this tool to return an explicit error. Omit the field or pass `false` until the worker-thread UI surface lands."
                }
            }
        })
    }
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // showcase lands (see tinyhumansai/Benito#1624). Return an
        ...
        if dedicated_thread_requested {
            log::debug!(
                "[spawn_subagent] dedicated_thread requested but temporarily \
                 disabled (see tinyhumansai/Benito#1624); rejecting call"
            );
            return Ok(ToolResult::error(
                "spawn_subagent: `dedicated_thread` is temporarily disabled \
                 (see tinyhumansai/Benito#1624); retry without it.",
            ));
        }
        ...
        if definition.id == "integrations_agent" {
            ...
            let live_integrations: Vec<crate::benito::context::prompt::ConnectedIntegration> = {
                match crate::benito::config::Config::load_or_init().await {
                    Ok(config) => {
                        ...
                        // a transient 5xx).
                        match crate::benito::composio::fetch_connected_integrations_status(
                            &config,
                        )
                        .await
                        {
            ...
            let allowlist: Vec<&crate::benito::context::prompt::ConnectedIntegration> =
...
mod tests {
    use super::*;
    use crate::benito::agent::harness::subagent_runner::SubagentMode;
