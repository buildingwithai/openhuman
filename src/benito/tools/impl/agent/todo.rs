...
//! one) via [`crate::benito::todos::ops`]; without a thread context the
//! tool falls back to a process-global scratch list. Returns a markdown
//! rendering so transcripts read cleanly.

use crate::benito::agent::task_board::{TaskBoardCard, TaskCardStatus};
use crate::benito::inference::provider::thread_context;
use crate::benito::todos::ops::{self, BoardLocation, CardPatch};
...
fn current_location() -> BoardLocation {
    let Some(parent) = crate::benito::agent::harness::fork_context::current_parent() else {
...
mod tests {
    use super::*;
    use crate::benito::todos::global_scratch_store;
    ...
    fn scratch_lock() -> std::sync::MutexGuard<'static, ()> {
        crate::benito::todos::ops::scratch_test_lock()
    }
