use crate::benito::agent::host_runtime::RuntimeAdapter;
        ...
        match result {
            Ok(Ok(output)) => {
                ...
                if stdout.len() > MAX_OUTPUT_BYTES {
                    stdout.truncate(crate::benito::util::floor_char_boundary(
                        &stdout,
                        MAX_OUTPUT_BYTES,
                    ));
                ...
                if stderr.len() > MAX_OUTPUT_BYTES {
                    stderr.truncate(crate::benito::util::floor_char_boundary(
                        &stderr,
                        MAX_OUTPUT_BYTES,
                    ));
...
mod tests {
    ...
    async fn shell_requires_approval_for_medium_risk_command() {
        ...
        let command = if cfg!(windows) {
            "mkdir BENITO_shell_approval_test"
        } else {
            "touch BENITO_shell_approval_test"
        };
        ...
        let cleanup = std::env::temp_dir().join("BENITO_shell_approval_test");
        if cfg!(windows) {
