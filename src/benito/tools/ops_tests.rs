use super::*;
use crate::benito::config::{BrowserConfig, Config, MemoryConfig};
use crate::benito::credentials::{AuthService, APP_SESSION_PROVIDER, DEFAULT_AUTH_PROFILE_NAME};
...
fn test_memory(tmp: &TempDir) -> Arc<dyn Memory> {
    ...
    Arc::from(crate::benito::memory::create_memory(&mem_cfg, tmp.path()).unwrap())
}
...
fn integration_tools_for_config(tmp: &TempDir, cfg: &Config) -> Vec<Box<dyn Tool>> {
    ...
    let http = crate::benito::config::HttpRequestConfig::default();
    all_tools(
        Arc::new(cfg.clone()),
        &security,
        mem,
...
fn all_tools_registers_generic_mcp_bridge_tools_when_servers_exist() {
    ...
    cfg.mcp_client
        .servers
        .push(crate::benito::config::McpServerConfig {
            name: "docs".into(),
...
fn all_tools_registers_optional_search_lsp_and_tool_stats_when_enabled() {
    ...
    let _env_guard = crate::benito::config::TEST_ENV_LOCK
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    unsafe {
        std::env::set_var(
            crate::benito::tools::implementations::LSP_ENABLED_ENV,
            "1",
        );
    }
    ...
    unsafe {
        std::env::remove_var(crate::benito::tools::implementations::LSP_ENABLED_ENV);
    }
}
