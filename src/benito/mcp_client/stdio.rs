...
use crate::benito::config::McpClientIdentityConfig;
use anyhow::Context;
...
mod tests {
    ...
    fn benito_core_bin() -> PathBuf {
        let status = std::process::Command::new("cargo")
            .args(["build", "--quiet", "--bin", "Benito-core"])
            .status()
            .expect("spawn cargo build");
        assert!(status.success(), "cargo build --bin Benito-core failed");
        ...
        let bin = debug_dir.join("Benito-core");
        assert!(bin.exists(), "expected Benito-core at {}", bin.display());
        bin
    }

    #[tokio::test]
    async fn stdio_client_talks_to_BENITO_mcp_server() {
        let client = McpStdioClient::new(
            benito_core_bin().display().to_string(),
            vec!["mcp".into()],
            Vec::new(),
