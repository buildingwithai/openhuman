...
pub async fn start_login_gated_services(config: &Config) {
    ...
    if config.local_ai.runtime_enabled {
        let service = crate::benito::inference::local::global(config);
    ...
    crate::benito::voice::server::start_if_enabled(config).await;
    ...
    if !config.voice_server.auto_start {
        crate::benito::voice::dictation_listener::start_if_enabled(config).await;
    }

    // 4. Screen intelligence (capture + vision analysis)
    crate::benito::screen_intelligence::server::start_if_enabled(config).await;

    // 5. Autocomplete (text suggestions + Swift overlay helper)
    crate::benito::autocomplete::start_if_enabled(config).await;
...
pub async fn stop_login_gated_services(config: &Config) {
    ...
    if let Some(server) = crate::benito::screen_intelligence::server::try_global_server() {
        server.stop().await;
    ...
    crate::benito::voice::dictation_listener::stop();
...
pub async fn store_session(
    ...
    // Determine user_id so we can scope the Benito directory to this user.
    ...
    if let Some(ref uid) = resolved_user_id {
        if let Ok(root_dir) = default_root_BENITO_dir() {
            ...
            let user_dir = user_BENITO_dir(&root_dir, uid);
    ...
    let effective_config = if resolved_user_id.is_some() {
        match crate::benito::config::load_config_with_timeout().await {
            Ok(c) => c,
            Err(_) => config.clone(),
        }
    } else {
    ...
    if let Err(e) = crate::benito::subconscious::global::bootstrap_after_login().await {
...
pub async fn clear_session(config: &Config) -> Result<RpcOutcome<serde_json::Value>, String> {
    ...
    // default (unauthenticated) Benito directory.
    if let Ok(root_dir) = default_root_BENITO_dir() {
        if let Err(e) = crate::benito::config::clear_active_user(&root_dir) {
    ...
    crate::benito::subconscious::global::reset_engine_for_user_switch().await;
...
///
/// Used by [`crate::benito::composio::client::create_composio_client`]
