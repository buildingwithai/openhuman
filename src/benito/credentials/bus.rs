...
//! `Benito-TAURI-1T` (5,414 Sentry events from one user's
//! cron-driven LLM calls after session expiry).

use crate::core::event_bus::{DomainEvent, EventHandler};
use crate::benito::scheduler_gate;
...
impl EventHandler for SessionExpiredSubscriber {
    ...
    async fn handle(&self, event: &DomainEvent) {
        ...
        match crate::benito::config::rpc::load_config_with_timeout().await {
            Ok(config) => {
                if let Err(err) = crate::benito::credentials::rpc::clear_session(&config).await {
                    tracing::warn!(
                        source = %source,
                        error = %err,
