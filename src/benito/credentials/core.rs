...
mod tests {
    use super::*;
    use crate::benito::credentials::profiles::{AuthProfile, AuthProfileKind};
    ...
    fn state_dir_from_config_uses_config_path_parent() {
        ...
        config.config_path = PathBuf::from("/tmp/Benito-test/config.toml");
        assert_eq!(
            state_dir_from_config(&config),
            PathBuf::from("/tmp/Benito-test")
        );
    }
