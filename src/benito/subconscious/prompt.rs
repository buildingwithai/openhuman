...
- `proposed_action` (optional): one-tap action text. When the user taps
  the action button, Benito opens a *new* conversation thread seeded
...
pub fn format_recent_reflections_for_prompt(
    reflections: &[crate::benito::subconscious::reflection::Reflection],
) -> String {
    crate::benito::subconscious::situation_report::reflections::build_section(reflections)
}
...
pub fn load_identity_context(workspace_dir: &Path) -> String {
    ...
    if ctx.is_empty() {
        "You are Benito, an AI assistant for productivity and collaboration.".to_string()
    } else {
        ctx
    }
}
...
fn resolve_prompts_dir(workspace_dir: &Path) -> Option<std::path::PathBuf> {
    ...
    if let Ok(cwd) = std::env::current_dir() {
        return crate::benito::dev_paths::repo_ai_prompts_dir(&cwd);
    }
...
mod tests {
    use super::*;
    use crate::benito::subconscious::types::{TaskRecurrence, TaskSource};
