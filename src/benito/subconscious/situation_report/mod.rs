...
fn build_identifiers_section() -> String {
    let identities = crate::benito::composio::providers::profile::load_connected_identities();
    if identities.is_empty() {
        return String::new();
    }
    let body = crate::benito::composio::providers::profile::render_connected_identities_section(
        &identities,
    );
...
fn append_section(report: &mut String, remaining: &mut usize, section: &str) {
    ...
    if needed <= *remaining {
        ...
        *remaining -= needed;
        ...
        let truncate_at = crate::benito::util::floor_char_boundary(section, budget);
