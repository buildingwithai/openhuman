...
async fn execute_with_local_model(
    ...
    let messages = vec![
        crate::benito::inference::local::ops::LocalAiChatMessage {
    ...
    let outcome = crate::benito::inference::ops::inference_chat(&config, messages, None)
        .await
...
async fn execute_with_agent_full(
    config: &mut crate::benito::config::Config,
    task: &SubconsciousTask,
    situation_report: &str,
    identity_context: &str,
) -> Result<String, String> {
...
async fn agent_chat_with_retry(
    ...
    loop {
        let result =
            crate::benito::inference::local::ops::agent_chat(config, prompt, None, Some(0.3))
                .await;
...
"#
        ...
        let _: crate::benito::config::Config =
    ...
    async fn execute_task_routes_to_cloud_when_local_disabled() {
        let _env_lock = crate::benito::config::TEST_ENV_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        test_mocks::reset();
        let tmp = tempdir().expect("tempdir");
        let _workspace = EnvVarGuard::set_to_path("BENITO_WORKSPACE", tmp.path());
