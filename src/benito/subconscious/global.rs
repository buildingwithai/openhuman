...
//! per-user workspace (`~/.Benito/users/<id>/workspace/`) instead of the
...
pub async fn get_or_init_engine() -> Result<Arc<Mutex<Option<SubconsciousEngine>>>, String> {
    ...
    let memory =
        crate::benito::memory::MemoryClient::from_workspace_dir(config.workspace_dir.clone())
            .ok()
            .map(Arc::new);
...
pub async fn bootstrap_after_login() -> Result<(), String> {
    ...
    let heartbeat = crate::benito::heartbeat::engine::HeartbeatEngine::new(
