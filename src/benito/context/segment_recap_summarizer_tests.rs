...
use crate::benito::agent::hooks::{PostTurnHook as _, TurnContext};
use crate::benito::context::summarizer::{Summarizer, SummaryStats};
use crate::benito::inference::provider::{ChatMessage, ConversationMessage};
use crate::benito::memory::store::{fts5, segments as seg};
use crate::benito::memory::tree::chat::ChatPrompt;
use anyhow::Result;
...
fn setup_conn() -> Arc<Mutex<Connection>> {
    ...
    conn.execute_batch(crate::benito::memory::store::events::EVENTS_INIT_SQL)
        .unwrap();
    conn.execute_batch(crate::benito::memory::store::profile::PROFILE_INIT_SQL)
        .unwrap();
...
#[async_trait]
impl crate::benito::memory::tree::chat::ChatProvider for StubChatProvider {
    fn name(&self) -> &str {
        "stub:test"
    }
...
#[async_trait]
impl crate::benito::memory::tree::chat::ChatProvider for FailingChatProvider {
    fn name(&self) -> &str {
        "stub:failing"
    }
    async fn chat_for_json(&self, _prompt: &ChatPrompt) -> Result<String> {
...
#[async_trait]
impl crate::benito::memory::tree::score::embed::Embedder for StubEmbedder {
