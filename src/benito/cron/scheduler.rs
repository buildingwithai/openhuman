...
async fn execute_and_persist_job(
    ...
    publish_global(DomainEvent::CronJobCompleted {
        job_id: job.id.clone(),
        success,
        output: crate::benito::util::truncate_with_ellipsis(&output, 512),
    });
    let failure_message =
        (!success).then(|| crate::benito::util::truncate_with_ellipsis(&output, 256));
    ...
    if let Some(ref agent_id) = job.agent_id {
            ...
            crate::benito::agent::harness::definition::AgentDefinitionRegistry::global()
        {
            if let Some(def) = registry.get(agent_id) {
                ...
                // `{hint}-v1` for Hint specs, which only the Benito
                ...
                let fallback_model = effective
                    .default_model
                    .clone()
                    .unwrap_or_else(|| crate::benito::config::DEFAULT_MODEL.to_string());
                let resolved_model = match &def.model {
                    ModelSpec::Hint(workload) => {
                        match crate::benito::inference::provider::create_chat_provider(
                            workload, &effective,
                        ) {
                            Ok((_, m)) => {
                                tracing::debug!(
...
fn push_cron_alert(config: &Config, job: &CronJob, output: &str) {
    use crate::benito::notifications::store as notif_store;
    use crate::benito::notifications::types::{IntegrationNotification, NotificationStatus};

    let name = job.name.as_deref().unwrap_or("Cron job");
    let truncated = crate::benito::util::truncate_with_ellipsis(output, 512);
