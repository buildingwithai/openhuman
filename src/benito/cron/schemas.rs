...
use crate::benito::cron::CronJobPatch;
...
fn handle_list(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async {
        ...
        to_json(crate::benito::cron::rpc::cron_list(&config).await?)
    })
}
...
fn handle_update(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::cron::rpc::cron_update(&config, job_id.trim(), patch).await?)
    })
}
...
fn handle_remove(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let job_id = read_required::<String>(&params, "job_id")?;
        to_json(crate::benito::cron::rpc::cron_remove(&config, job_id.trim()).await?)
    })
}
...
fn handle_run(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let job_id = read_required::<String>(&params, "job_id")?;
        to_json(crate::benito::cron::rpc::cron_run(&config, job_id.trim()).await?)
    })
}
...
fn handle_runs(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let limit = read_optional_u64(&params, "limit")?
            ...
            .transpose()?;
        to_json(crate::benito::cron::rpc::cron_runs(&config, job_id.trim(), limit).await?)
    })
}
