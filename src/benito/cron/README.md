...
- `src/Benito/agent/` — `agent` job type runs through `agent::triage::TriggerEnvelope::from_cron` + `apply_decision`.
- `src/Benito/security/` — `SecurityPolicy::from_config` sandboxes shell jobs.
- `src/Benito/config/` — `Config` provides poll interval, workspace dir, autonomy policy.
- `src/Benito/health/` — `health::bus::register_health_subscriber` on startup.
- `src/Benito/channels/` — `bus.rs` can fan delivery events into channels.
- `src/core/event_bus/` — `init_global`, `publish_global(DomainEvent::Cron(*))`.

## Called by

- `src/Benito/tools/impl/system/schedule.rs` — `schedule` tool exposes cron operations to agents.
