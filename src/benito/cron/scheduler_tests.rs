...
use crate::benito::cron::{self, ActiveHours, DeliveryConfig};
...
fn test_job(command: &str) -> CronJob {
    CronJob {
        id: "test-job".into(),
        expression: "* * * * *".into(),
        schedule: crate::benito::cron::Schedule::Cron {
            expr: "* * * * *".into(),
            tz: None,
            active_hours: None,
        },
...
async fn run_job_command_blocks_readonly_mode() {
    let tmp = TempDir::new().unwrap();
    let mut config = test_config(&tmp).await;
    config.autonomy.level = crate::benito::security::AutonomyLevel::ReadOnly;
...
async fn persist_job_result_success_deletes_one_shot() {
    ...
    let job = cron::add_agent_job(
        &config,
        Some("one-shot".into()),
        crate::benito::cron::Schedule::At { at },
        "Hello",
