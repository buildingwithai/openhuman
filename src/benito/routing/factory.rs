...
use crate::benito::config::LocalAiConfig;
use crate::benito::inference::local::lm_studio::lm_studio_base_url_from_local_ai;
use crate::benito::inference::local::ollama_base_url;
use crate::benito::inference::local::provider::normalize_provider;
...
pub fn new_provider(
    ...
    //
    // `BENITO_LOCAL_INFERENCE_URL` — full `/v1` base URL of the local
    // OpenAI-compat server. When set, health is probed via `GET {base}/models`
    // instead of Ollama's `/api/tags`.
    let override_base = std::env::var("BENITO_LOCAL_INFERENCE_URL")
        .ok()
...
mod tests {
    use super::*;
    use crate::benito::config::LocalAiConfig;
    use crate::benito::inference::provider::traits::{ProviderCapabilities, ToolsPayload};
    ...
    fn factory_env_override_url_takes_precedence_over_base_url() {
        // BENITO_LOCAL_INFERENCE_URL env var must override config.base_url.
        // This is tested by ensuring construction succeeds when the env var
        // is set — a real URL check would require a running server.
        let _guard = crate::benito::inference::local::inference_test_guard();
        unsafe {
            std::env::set_var("BENITO_LOCAL_INFERENCE_URL", "http://127.0.0.1:9999/v1");
        }
        let mut cfg = LocalAiConfig::default();
        cfg.runtime_enabled = true;
        ...
        let _p = make_provider(&cfg);
        unsafe {
            std::env::remove_var("BENITO_LOCAL_INFERENCE_URL");
        }
    }
}
