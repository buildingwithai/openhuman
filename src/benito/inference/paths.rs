...
///
/// Default callers see the shared `~/.Benito/` root, which avoids
/// duplicating multi-GB model files across users on a single machine.
///
/// When `BENITO_WORKSPACE` is **explicitly** set (test/dev parallel
...
fn shared_root_dir(config: &Config) -> PathBuf {
    if std::env::var_os("BENITO_WORKSPACE").is_some() {
...
/// Spawn marker file recording the PID of any `ollama serve` Benito
/// itself spawned. Read on next launch to recognise our own orphan when
/// Benito crashed before its graceful-shutdown hook ran. Lives under
...
pub(crate) fn resolve_whisper_binary() -> Option<PathBuf> {
    ...
    if let Ok(shared) = crate::benito::config::default_root_BENITO_dir() {
...
pub(crate) fn resolve_whisper_binary_with_config(config: &Config) -> Option<PathBuf> {
    if let Some(workspace) =
        crate::benito::inference::local::install_whisper::find_workspace_whisper_binary(config)
    {
...
pub(crate) fn resolve_piper_binary_with_config(config: &Config) -> Option<PathBuf> {
    if let Some(workspace) =
        crate::benito::inference::local::install_piper::find_workspace_piper_binary(config)
    {
        return Some(workspace);
    }
...
mod tests {
    ...
    fn resolve_tts_voice_path_appends_onnx_for_voice_ids() {
        ...
        // root (`~/.Benito/`) — not the temp config. If a sibling
