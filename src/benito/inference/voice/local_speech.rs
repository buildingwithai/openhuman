...
//! pipeline ([`crate::benito::inference::paths::resolve_piper_binary`]),
...
//! [`crate::benito::inference::local::install_piper`] which downloads the
//! Piper binary archive (`.zip` on Windows, `.tar.gz` on macOS / Linux)
//! into `~/.Benito/bin/piper/`, extracts it, and stages the bundled
...
use crate::benito::voice::reply_speech::{ReplySpeechResult, VisemeFrame};
...
pub async fn synthesize_piper(
    ...
    let out_dir = std::env::temp_dir().join("BENITO_voice_output");
