...
//! [`crate::benito::inference::paths::resolve_whisper_binary`] — kept in
...
//! [`crate::benito::inference::local::install_whisper`] which streams the
//! GGML model file (`ggml-<size>.bin`) into
//! `~/.Benito/bin/whisper/` via a `.part` file + atomic rename, plus
...
use crate::benito::inference::paths::resolve_whisper_binary_with_config;
...
///    `$TMP/BENITO_voice_input/voice-<ts>-<uuid>.<ext>` — whisper-cli
...
pub async fn transcribe_whisper(
    ...
    let model_path =
        crate::benito::inference::paths::resolve_stt_model_path_by_id(&model_id, config)
