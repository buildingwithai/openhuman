...
use crate::benito::inference::local::install::{
    find_system_ollama_binary, run_ollama_install_script,
};
use crate::benito::inference::local::lm_studio::lm_studio_base_url;
use crate::benito::inference::local::model_requirements::{
    evaluate_context, ContextEligibility, MIN_CONTEXT_TOKENS,
};
use crate::benito::inference::local::ollama::{
...
use crate::benito::inference::local::process_util::apply_no_window;
use crate::benito::inference::local::provider::{provider_from_config, LocalAiProvider};
use crate::benito::inference::model_ids;
use crate::benito::inference::paths::{find_workspace_ollama_binary, workspace_ollama_binary};
use crate::benito::inference::presets::{self, VisionMode};
...
impl LocalAiService {
    pub(in crate::benito::inference::local::service) async fn ensure_ollama_server(
        &self,
        _config: &Config,
    ) -> Result<(), String> {
        ...
        Err(format!(
            "Benito no longer starts or installs Ollama automatically. Start your inference runtime yourself and make sure it is reachable at {base_url}."
        ))
    }

    /// Alias of `ensure_ollama_server` in external-runtime mode.
    /// Benito no longer installs or starts Ollama automatically; the
    /// "fresh" retry path is a no-op that defers to the standard check.
    pub(in crate::benito::inference::local::service) async fn ensure_ollama_server_fresh(
        &self,
        config: &Config,
    ) -> Result<(), String> {
    ...
    /// Check if a healthy daemon on `:11434` is actually Benito's own
    ...
    async fn reclaim_orphan_if_ours(&self, config: &Config) {
        ...
        log::info!(
            "[local_ai] reclaiming Benito-owned ollama orphan from prior session \
             (pid={}, binary={})",
            marker.pid,
            marker.binary_path
        );
    ...
    async fn start_and_wait_for_server(
        ...
        if self.ollama_healthy().await {
            ...
            // so `owned_ollama` stays `None` and the daemon survives Benito
        ...
        for _ in 0..20 {
            if self.ollama_healthy().await {
                ...
                // write the spawn marker so a crashed Benito can reclaim
    ...
    async fn download_and_install_ollama(&self, config: &Config) -> Result<(), String> {
        let install_dir = crate::benito::inference::paths::workspace_ollama_dir(config);
        ...
        // It outlives Benito crashing, the user closing the app, or
        ...
        if crate::benito::inference::local::install::is_ollama_installer_running() {
            log::info!(
            ...
            while crate::benito::inference::local::install::is_ollama_installer_running() {
    ...
    pub(in crate::benito::inference::local::service) async fn ollama_healthy(&self) -> bool {
        self.http
    ...
    pub(in crate::benito::inference::local::service) fn ollama_binary_present(
        &self,
        config: &Config,
    ) -> bool {
    ...
    pub(in crate::benito::inference::local::service) async fn ensure_models_available(
        &self,
        config: &Config,
    ) -> Result<(), String> {
    ...
    pub(in crate::benito::inference::local::service) async fn ensure_ollama_model_available(
        &self,
        model_id: &str,
        label: &str,
    ) -> Result<(), String> {
    ...
    fn resolve_binary_path(&self, config: &Config) -> Option<String> {
        ...
        crate::benito::inference::local::install::find_system_ollama_binary()
    ...
    /// Kill the `ollama serve` daemon Benito itself spawned, if any.
    ///
    /// **No-op when Benito never spawned a daemon** (i.e. it adopted an
    ...
    async fn kill_ollama_server(&self) {
        ...
        let Some(mut child) = maybe_child else {
            log::debug!(
                "[local_ai] kill_ollama_server: no Benito-owned daemon; \
        ...
        match child.kill().await {
            Ok(()) => {
                log::info!("[local_ai] killed Benito-owned ollama serve (pid={pid})");
    ...
    ///
    /// Kills the Benito-owned `ollama serve` (if any) and clears the
    ...
    pub(in crate::benito::inference::local::service) async fn has_model(
        &self,
        model: &str,
    ) -> Result<bool, String> {
...
/// `tokio::process::Child` handle (the spawning Benito crashed) but
