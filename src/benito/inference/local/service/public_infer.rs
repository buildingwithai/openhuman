...
use crate::benito::inference::parse::sanitize_inline_completion;
...
fn external_ollama_request_error(prefix: &str, error: &reqwest::Error) -> String {
    ...
    format!(
        "{prefix}: Benito routes inference through an external Ollama endpoint. \
...
impl LocalAiService {
    ...
    pub(crate) async fn chat_with_history(
        &self,
        config: &Config,
        messages: Vec<crate::benito::inference::local::ollama::OllamaChatMessage>,
        max_tokens: Option<u32>,
    ) -> Result<String, String> {
        if !config.local_ai.runtime_enabled {
            return Err("local ai is disabled".to_string());
        ...
        let _gate_permit = crate::benito::scheduler_gate::wait_for_capacity().await;
        ...
        if provider_from_config(config) == LocalAiProvider::LmStudio {
            ...
            let lm_messages = messages
                .into_iter()
                .map(
                    |message| crate::benito::inference::local::lm_studio::LmStudioChatMessage {
                        role: message.role,
        ...
        tracing::debug!(
            ...
            model = %crate::benito::inference::model_ids::effective_chat_model_id(config),
            "[local_ai:chat] sending to ollama /api/chat"
        );

        let started = std::time::Instant::now();

        let body = crate::benito::inference::local::ollama::OllamaChatRequest {
            model: crate::benito::inference::model_ids::effective_chat_model_id(config),
            messages,
            stream: false,
            options: Some(
                crate::benito::inference::local::ollama::OllamaGenerateOptions {
        ...
        let payload: crate::benito::inference::local::ollama::OllamaChatResponse = response
            .json()
            .await
    ...
    async fn inference_with_temperature_internal(
        ...
        let _gate_permit = if gated {
            crate::benito::scheduler_gate::wait_for_capacity().await
        } else {
            None
        };
        ...
        if provider_from_config(config) == LocalAiProvider::LmStudio {
            let messages = vec![
                crate::benito::inference::local::lm_studio::LmStudioChatMessage {
        ...
        let payload: crate::benito::inference::local::ollama::OllamaGenerateResponse = response
            .json()
            .await
