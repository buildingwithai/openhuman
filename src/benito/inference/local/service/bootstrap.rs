...
use crate::benito::inference::device::DeviceProfile;
...
use crate::benito::inference::types::LocalAiStatus;
...
impl LocalAiService {
    ...
    pub async fn bootstrap(&self, config: &Config) {
        ...
        let device = crate::benito::inference::device::detect_device_profile();
        ...
        if effective_config.local_ai.whisper_in_process {
            if let Ok(model_path) =
                crate::benito::inference::paths::resolve_stt_model_path(&effective_config)
            {
...
fn config_with_recommended_tier_if_unselected(config: &Config, device: &DeviceProfile) -> Config {
    let current_tier =
        crate::benito::inference::presets::current_tier_from_config(&config.local_ai);
    ...
    if !config.local_ai.opt_in_confirmed {
        tracing::debug!(
            ...
            min_required_gb = crate::benito::inference::presets::MIN_RAM_GB_FOR_LOCAL_AI,
            ?current_tier,
...
fn format_degraded_warning(err: &str, config: &Config) -> String {
    let current = crate::benito::inference::presets::current_tier_from_config(&config.local_ai);
    match current {
        crate::benito::inference::presets::ModelTier::Ram16PlusGb => {
            format!(
        ...
        crate::benito::inference::presets::ModelTier::Ram8To16Gb => {
            format!(
        ...
        crate::benito::inference::presets::ModelTier::Ram4To8Gb => format!(
...
mod tests {
    ...
    fn bootstrap_honors_opt_in_on_low_ram_device() {
        ...
        crate::benito::inference::presets::apply_preset_to_config(
            &mut config.local_ai,
            crate::benito::inference::presets::ModelTier::Ram2To4Gb,
        );
        let device = test_device(4);
