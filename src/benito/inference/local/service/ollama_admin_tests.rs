...
use crate::benito::inference::local::service::LocalAiService;
...
async fn has_model_detects_exact_and_prefixed_tag() {
    let _guard = crate::benito::inference::inference_test_guard();
    ...
    unsafe {
        std::env::set_var("BENITO_OLLAMA_BASE_URL", &base);
    }
    ...
    unsafe {
        std::env::remove_var("BENITO_OLLAMA_BASE_URL");
    }
}
...
async fn ollama_healthy_returns_false_on_unreachable_url() {
    ...
    unsafe {
        std::env::set_var("BENITO_OLLAMA_BASE_URL", "http://127.0.0.1:1");
    }
    let config = Config::default();
    let service = LocalAiService::new(&config);
    assert!(!service.ollama_healthy().await);
...
async fn diagnostics_reports_server_unreachable_when_url_unbound() {
    ...
    assert!(
        ...
        "Benito should not suggest app-managed repair actions anymore"
    );
    unsafe {
        std::env::remove_var("BENITO_OLLAMA_BASE_URL");
    }
...
async fn diagnostics_ok_when_expected_models_are_present() {
    ...
    let config = Config::default();
    let chat = crate::benito::inference::model_ids::effective_chat_model_id(&config);
    let embedding = crate::benito::inference::model_ids::effective_embedding_model_id(&config);
...
// daemons Benito spawned itself, and that the kill path actually
...
// user started outside Benito — the issue #1622 friendly-fire bug).
...
async fn kill_ollama_server_kills_owned_child() {
    ...
    *service.owned_ollama.lock() = Some(child);
    ...
    assert!(
        crate::benito::inference::local::service::spawn_marker::pid_is_alive(pid),
    ...
    for _ in 0..40 {
        if !crate::benito::inference::local::service::spawn_marker::pid_is_alive(pid) {
            still_alive = false;
            break;
        }
...
async fn shutdown_owned_ollama_clears_marker_and_kills_child() {
    ...
    // touch the real `~/.Benito/`. Per `paths::shared_root_dir`, when
    // `default_root_BENITO_dir()` errors, it falls back to
    ...
    *service.owned_ollama.lock() = Some(child);
    ...
    // assert the marker survives the `default_root_BENITO_dir()`
    ...
    // where `default_root_BENITO_dir()` succeeds against the real home
    // dir, we skip the marker assertion to avoid touching `~/.Benito/`.
    let marker_path = crate::benito::inference::paths::ollama_spawn_marker_path(&config);
    let marker_writable = marker_path.starts_with(tmp.path());
    if marker_writable {
        crate::benito::inference::local::service::spawn_marker::write_marker_at(
            &marker_path,
            &crate::benito::inference::local::service::spawn_marker::OllamaSpawnMarker::new(
                pid,
