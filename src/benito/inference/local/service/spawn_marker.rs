//! Spawn marker for Benito-owned `ollama serve` processes.
//!
...
//! recording the PID, the binary we launched, and the Benito process
//! that owned it. On graceful shutdown the marker is cleared. If Benito
...
use crate::benito::inference::paths::ollama_spawn_marker_path;

/// On-disk record of an Benito-spawned `ollama serve` process.
...
impl OllamaSpawnMarker {
    pub(crate) fn new(pid: u32, binary_path: &Path) -> Self {
        ...
        Self {
            ...
            BENITO_pid: std::process::id(),
        }
    }
}

// ---- Path-keyed helpers (testable without touching ~/.Benito/) -------
...
mod tests {
    ...
    fn new_marker_captures_current_process_id() {
        ...
        assert_eq!(m.BENITO_pid, std::process::id());
