...
/// [`crate::benito::voice::factory::DEFAULT_PIPER_VOICE`].
...
pub fn status(config: &Config) -> VoiceInstallStatus {
    ...
    let configured_voice = crate::benito::inference::model_ids::effective_tts_voice_id(config);
...
mod tests {
    ...
    /// Serialise tests that write into the shared `~/.Benito/bin/piper/`
