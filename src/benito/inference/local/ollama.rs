...
/// 1. `BENITO_OLLAMA_BASE_URL` — app-specific override, used in tests.
...
pub(crate) fn ollama_base_url() -> String {
    if let Ok(url) = std::env::var("BENITO_OLLAMA_BASE_URL") {
...
mod tests {
    ...
    //
    // These tests mutate the process-global `BENITO_OLLAMA_BASE_URL`
    ...
    const ENV_VAR: &str = "BENITO_OLLAMA_BASE_URL";
    ...
    #[test]
    fn ollama_base_url_uses_ollama_host_when_BENITO_var_unset() {
        let _lock = test_lock();
    ...
    #[test]
    fn ollama_base_url_BENITO_var_takes_priority_over_ollama_host() {
        let _lock = test_lock();
