...
fn handle_agent_chat(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::inference::local::ops::agent_chat(
                &mut config,
                &p.message,
                p.model_override,
                p.temperature,
...
fn handle_agent_chat_simple(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::inference::local::ops::agent_chat_simple(
                &config,
                &p.message,
                p.model_override,
                p.temperature,
...
fn handle_local_ai_transcribe(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::benito::inference::local::ops::local_ai_transcribe(
                &config,
                p.audio_path.trim(),
            )
            .await?,
...
fn handle_local_ai_transcribe_bytes(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::benito::inference::local::ops::local_ai_transcribe_bytes(
                &config,
                &p.audio_bytes,
                p.extension,
            )
...
fn handle_local_ai_tts(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::benito::inference::local::ops::local_ai_tts(
                &config,
                &p.text,
...
fn handle_local_ai_assets_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::benito::inference::local::ops::local_ai_assets_status(&config).await?)
    })
}
...
fn handle_local_ai_downloads_progress(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::benito::inference::local::ops::local_ai_downloads_progress(&config).await?,
        )
    })
}
...
fn handle_local_ai_download_asset(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::benito::inference::local::ops::local_ai_download_asset(
                &config,
                p.capability.trim(),
            )
            .await?,
...
fn handle_local_ai_install_whisper(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let slot =
            match crate::benito::inference::local::voice_install_common::try_acquire_install_slot(
                crate::benito::inference::local::voice_install_common::ENGINE_WHISPER,
            ) {
                Some(slot) => slot,
                None => {
                    tracing::debug!(
                    ...
                    let current = crate::benito::inference::local::voice_install_common::read_status(
                    crate::benito::inference::local::voice_install_common::ENGINE_WHISPER,
                );
        ...
        crate::benito::inference::local::voice_install_common::write_status(
            crate::benito::inference::local::voice_install_common::VoiceInstallStatus {
                engine: crate::benito::inference::local::voice_install_common::ENGINE_WHISPER
                    .to_string(),
                state:
                    crate::benito::inference::local::voice_install_common::VoiceInstallState::Installing,
                progress: Some(0),
        ...
        tokio::spawn(async move {
            let _slot = slot;
            if let Err(e) = crate::benito::inference::local::install_whisper::install_whisper(
        ...
        let status = crate::benito::inference::local::voice_install_common::read_status(
            crate::benito::inference::local::voice_install_common::ENGINE_WHISPER,
        );
...
fn handle_local_ai_install_piper(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let slot =
            match crate::benito::inference::local::voice_install_common::try_acquire_install_slot(
                crate::benito::inference::local::voice_install_common::ENGINE_PIPER,
            ) {
                Some(slot) => slot,
                None => {
                    tracing::debug!(
                    ...
                    let current =
                        crate::benito::inference::local::voice_install_common::read_status(
                            crate::benito::inference::local::voice_install_common::ENGINE_PIPER,
                        );
                    return serde_json::to_value(current)
        ...
        crate::benito::inference::local::voice_install_common::write_status(
            crate::benito::inference::local::voice_install_common::VoiceInstallStatus {
                engine: crate::benito::inference::local::voice_install_common::ENGINE_PIPER.to_string(),
                state:
                    crate::benito::inference::local::voice_install_common::VoiceInstallState::Installing,
                progress: Some(0),
                downloaded_bytes: None,
        ...
        tokio::spawn(async move {
            let _slot = slot;
            if let Err(e) = crate::benito::inference::local::install_piper::install_piper(
...
fn handle_local_ai_whisper_install_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let status = crate::benito::inference::local::install_whisper::status(&config);
        serde_json::to_value(status).map_err(|e| format!("serialize whisper status: {e}"))
    })
}
...
fn handle_local_ai_piper_install_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let status = crate::benito::inference::local::install_piper::status(&config);
        serde_json::to_value(status).map_err(|e| format!("serialize piper status: {e}"))
    })
}
