...
//! [`crate::benito::inference::local::voice_install_common`] status table so
...
/// [`crate::benito::voice::factory::DEFAULT_WHISPER_MODEL`].
...
pub fn status(config: &Config) -> VoiceInstallStatus {
    ...
    if matches!(snapshot.state, VoiceInstallState::Missing) {
        let configured = crate::benito::inference::model_ids::effective_stt_model_id(config);
...
mod tests {
    use super::*;
    use crate::benito::inference::local::voice_install_common::reset_status;
    ...
    /// Serialise tests that write into the shared `~/.Benito/bin/whisper/`
