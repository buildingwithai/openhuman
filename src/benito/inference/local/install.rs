...
/// process — it survives the parent Benito process dying. If Benito
...
pub(crate) fn resolve_powershell_executable() -> std::ffi::OsString {
    // `Command::new("powershell")` relies on PATH. When Benito.exe is
...
fn build_install_command(install_dir: &Path) -> Result<tokio::process::Command, String> {
    #[cfg(target_os = "windows")]
    {
        ...
        // Benito mid-install. Without this, `cmd.output().await` keeps
        // Benito.exe alive (and port 7788 bound) for the full 60–120s of
        ...
        // up on the next Benito launch and waits.
        cmd.kill_on_drop(true);
        crate::benito::inference::local::process_util::apply_no_window(&mut cmd);
        cmd.env("BENITO_OLLAMA_INSTALL_DIR", install_dir);
        cmd.args([
            "-NoProfile",
            "-ExecutionPolicy",
            "Bypass",
            ...
            $installDir = $env:BENITO_OLLAMA_INSTALL_DIR
            ...
            # appears. The dialog is owned by the OS, not Benito, so it
            # survives the parent process crashing — giving the user a visible
            # signal that an install is in flight even if Benito dies.
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        cmd.arg("-lc")
            .arg(
                ...
                dest="$BENITO_OLLAMA_INSTALL_DIR"
                tmp_dir="$(mktemp -d)"
