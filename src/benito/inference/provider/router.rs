...
/// Maps Benito's abstract tier model names (`reasoning-v1`,
...
fn BENITO_tier_to_hint(model: &str) -> Option<&'static str> {
    match model {
...
impl RouterProvider {
    ...
    /// 2. Benito abstract tier names — `reasoning-v1`, `agentic-v1`,
    ...
    ///    the literal tier name (which is only meaningful to the Benito
    ...
    fn resolve(&self, model: &str) -> (usize, String) {
        ...
        // Benito abstract tier → hint mapping. These names are internal
        // aliases the Benito backend dispatches itself; custom providers
        // need them translated through the user's route table.
        if let Some(hint) = BENITO_tier_to_hint(model) {
            if let Some((idx, resolved_model)) = self.routes.get(hint) {
                log::info!(
            ...
            // these are Benito-internal aliases that only the hosted
