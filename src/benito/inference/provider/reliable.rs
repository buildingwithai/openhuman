...
/// whether to push a per-attempt event to Sentry (see Benito-TAURI-2E /
...
/// the most common cause we see in production (Benito-TAURI-BY / -BZ /
    ...
    if has_configured_fallbacks {
        ...
        format!(
            "The model `{model}` may not be available on your provider. \
             ...
             Benito config, or change your default model in Settings → AI.\n\n{attempts}"
        )
    }
}
