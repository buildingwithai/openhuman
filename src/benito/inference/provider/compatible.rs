...
use crate::benito::inference::provider::traits::{
...
use compatible_types::{
    ...
    NativeMessage, OpenAiStreamOptions, BenitoMeta, ResponseMessage, ResponsesRequest,
...
impl OpenAiCompatibleProvider {
    ...
    /// Opt this provider into emitting the Benito backend extension
    /// `thread_id` on outbound chat completions bodies. Only the
    /// `BenitoBackendProvider` should call this — third-party
    /// OpenAI-compatible providers must leave it off so they don't
    /// receive an unknown field.
    pub fn with_BENITO_thread_id(mut self) -> Self {
        self.emit_BENITO_thread_id = true;
        self
    }
    ...
    fn http_client(&self) -> Client {
        if let Some(ua) = self.user_agent.as_deref() {
            ...
            let builder = crate::benito::config::apply_runtime_proxy_to_builder(
                builder,
                "provider.compatible",
            );
    ...
    fn tool_specs_to_openai_format(
        tools: &[crate::benito::tools::ToolSpec],
    ...
    fn convert_tool_specs(
        tools: Option<&[crate::benito::tools::ToolSpec]>,
    ...
    fn with_prompt_guided_tool_instructions(
        ...
        let instructions =
            crate::benito::inference::provider::traits::build_tool_instructions_text(tools);
    ...
    /// Extract usage info from API response, preferring the Benito
    ...
    fn extract_usage(resp: &ApiChatResponse) -> Option<ProviderUsageInfo> {
        ...
        // Prefer Benito metadata when the fields are actually present;
        ...
        let from_Benito = oh_usage.is_some();
        let from_standard = std_usage.is_some() && !from_Benito;
    ...
    async fn stream_native_chat(
        ...
        delta_tx: &tokio::sync::mpsc::Sender<crate::benito::inference::provider::ProviderDelta>,
        dump_seq: u64,
    ) -> anyhow::Result<ProviderChatResponse> {
        ...
        let mut last_Benito: Option<BenitoMeta> = None;
        ...
        while let Some(item) = bytes_stream.next().await {
            ...
            while let Some(sep_idx) = buffer.find("\n\n") {
                ...
                for line in event.lines() {
                    ...
                    if let Some(meta) = chunk.Benito {
                        last_Benito = Some(meta);
                    }
                    ...
                    for choice in chunk.choices {
                        ...
                        if let Some(content) = choice.delta.content.as_ref() {
                            if !content.is_empty() {
                                ...
                                let _ = delta_tx
                                    .send(crate::benito::inference::provider::ProviderDelta::TextDelta {
                                        delta: content.clone(),
                                    })
                                    .await;
                            }
                        ...
                        if let Some(reasoning) = choice.delta.reasoning_content.as_ref() {
                            if !reasoning.is_empty() {
                                ...
                                let _ = delta_tx
                                    .send(
                                        crate::benito::inference::provider::ProviderDelta::ThinkingDelta {
                            ...
                            for tc in tc_list {
                                ...
                                if !entry.emitted_start {
                                        ...
                                        (entry.id.as_ref(), entry.name.as_ref())
                                    {
                                        ...
                                        let _ = delta_tx
                                            .send(crate::benito::inference::provider::ProviderDelta::ToolCallStart {
                                                call_id: id.clone(),
                                        ...
                                        if !entry.arguments.is_empty() {
                                            ...
                                            let _ = delta_tx
                                                .send(crate::benito::inference::provider::ProviderDelta::ToolCallArgsDelta {
                                                    call_id: id.clone(),
                                                    delta: buffered,
                                                })
                                                .await;
            ...
            .map(|c| ToolCall {
                ...
                function: Some(Function {
                    ...
                    arguments: if c.arguments.is_empty() {
                        ...
                        // discards malformed strings (Benito-TAURI-6F).
                        Some(
        ...
        // carries usage + Benito cache meta from the last chunks).
        // Hand-build a Value here because `ApiChatResponse` is
        // Deserialize-only.
        if std::env::var("BENITO_PROMPT_DUMP_DIR").is_ok() {
            ...
            let aggregated = serde_json::json!({
                ...
                "Benito": api_resp.Benito.as_ref().map(|m| serde_json::json!({
...
impl Provider for OpenAiCompatibleProvider {
    fn capabilities(&self) -> crate::benito::inference::provider::traits::ProviderCapabilities {
        crate::benito::inference::provider::traits::ProviderCapabilities {
    ...
    async fn chat(
        ...
        if let Some(tx) = request.stream {
            let native_request = NativeChatRequest {
                ...
                // accounting (and `Benito.billing.charged_amount_usd`
                // for the Benito backend) makes it back from
