...
//!
//! When `BENITO_PROMPT_DUMP_DIR` is set, both the outbound request payload
...
/// When `BENITO_PROMPT_DUMP_DIR` is set, write `body` (the exact JSON
...
pub(crate) fn dump_prompt_if_enabled<T: Serialize>(
    ...
    let Ok(dir) = std::env::var("BENITO_PROMPT_DUMP_DIR") else {
        return;
    };
