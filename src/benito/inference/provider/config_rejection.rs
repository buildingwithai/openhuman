...
//!
//! When Benito talks to a user-configured custom cloud endpoint
...
//!   Benito abstract tier alias leaked to a provider that only speaks
//!   its own native ids)
...
//! maintainers can act on: the user pointed Benito at a custom
...
//! (Benito-TAURI-WJ / -QW / -HB / -NH — 88 + 146 + 39 events). This is
...
//! (DeepSeek / OpenRouter / Moonshot). The Benito hosted backend
...
//! additionally guards on `provider != BENITO_backend::PROVIDER_LABEL`
...
/// contract and the Benito-TAURI Sentry issues each phrase drops.
pub fn is_provider_config_rejection_message(body: &str) -> bool {
    const PHRASES: &[&str] = &[
        // #2079 — an Benito abstract tier alias (`reasoning-v1`,
...
mod tests {
    ...
    fn detects_real_sentry_bodies() {
        // The exact upstream bodies from Benito-TAURI-WJ / -QW / -HB
