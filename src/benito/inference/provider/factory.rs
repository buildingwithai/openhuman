...
//!
//! ```text
//! "Benito"                    → BenitoBackendProvider; model = config.default_model
//! "cloud" / missing              → primary_cloud; legacy custom inference_url wins when
//!                                  primary still points at Benito after migration
...
use crate::benito::config::schema::cloud_providers::AuthStyle;
use crate::benito::config::Config;
use crate::benito::credentials::AuthService;
use crate::benito::inference::provider::compatible::{
    AuthStyle as CompatAuthStyle, OpenAiCompatibleProvider,
};
use crate::benito::inference::provider::BENITO_backend::BenitoBackendProvider;
use crate::benito::inference::provider::traits::Provider;
use crate::benito::inference::provider::ProviderRuntimeOptions;

/// Sentinel meaning "use the Benito backend session JWT".
pub const PROVIDER_Benito: &str = "Benito";
...
/// `primary_cloud` still points at Benito because migration 1→2 preserved
...
pub fn create_chat_provider_from_string(
    ...
    if p == PROVIDER_Benito {
        return make_BENITO_backend(config);
    }

    // ── Session gate ──────────────────────────────────────────────────
    // Custom providers (Ollama, <slug>:<model>) require an active
    // Benito session.  Without this check an unregistered user can
    ...
    anyhow::bail!(
        "[chat-factory] unrecognised provider string '{}' for role '{}'. \
         Valid forms: Benito, ollama:<model>, <slug>:<model>. \
         Configured slugs: [{}]",
        p,
        role,
        config
...
/// Build the Benito backend provider (session-JWT auth).
fn make_BENITO_backend(config: &Config) -> anyhow::Result<(Box<dyn Provider>, String)> {
    let model = config
        .default_model
        .clone()
    ...
    // leaves `BENITO_dir = None`, the provider falls back to
    // `~/.Benito`, and reads an unrelated (or empty)
    // profile store — surfacing as "No backend session: store a JWT
    // via auth (app-session)" even though login just succeeded in the
    // user's actual workspace (e.g. test workspaces under BENITO_WORKSPACE).
    ...
    log::debug!(
        "[providers][chat-factory] building Benito backend provider model={} state_dir={:?} secrets_encrypt={}",
        model,
        options.BENITO_dir,
        options.secrets_encrypt
    );
    // Translate `hint:<tier>` model strings into the Benito backend's
    // canonical tier names.
    let model = match model.strip_prefix("hint:") {
        Some("reasoning") => crate::benito::config::MODEL_REASONING_V1.to_string(),
        Some("chat") => crate::benito::config::MODEL_REASONING_QUICK_V1.to_string(),
        Some("agentic") => crate::benito::config::MODEL_AGENTIC_V1.to_string(),
        Some("coding") => crate::benito::config::MODEL_CODING_V1.to_string(),
        _ => model,
    };
    let p = Box::new(BenitoBackendProvider::new(
...
/// Verify the user has an active Benito backend session.
///
...
fn verify_session_active(config: &Config) -> anyhow::Result<()> {
    ...
    if crate::benito::scheduler_gate::is_signed_out() {
        anyhow::bail!(
    ...
    let state_dir = config
        ...
        .unwrap_or_else(|| {
            directories::UserDirs::new()
                ...
                .unwrap_or_else(|| std::path::PathBuf::from(".Benito"))
        });
    let auth = AuthService::new(&state_dir, config.secrets.encrypt);
    let has_session = auth
        .get_provider_bearer_token(crate::benito::credentials::APP_SESSION_PROVIDER, None)?
        .filter(|s| !s.trim().is_empty())
        .is_some();
    if !has_session {
        anyhow::bail!("SESSION_EXPIRED: no backend session — sign in to use Benito")
    }
    Ok(())
}
...
fn resolve_primary_cloud_provider_string(config: &Config) -> String {
    ...
    if primary.is_some_and(is_BENITO_cloud_entry) {
    ...
    legacy_custom_inference_provider_string(config)
        .unwrap_or_else(|| PROVIDER_Benito.to_string())
}
...
fn legacy_custom_inference_provider_string(config: &Config) -> Option<String> {
    ...
    if looks_like_BENITO_backend(inference_url) {
        return None;
    }
...
fn cloud_entry_provider_string(
    entry: &crate::benito::config::schema::cloud_providers::CloudProviderCreds,
    config: &Config,
) -> String {
    if is_BENITO_cloud_entry(entry) {
        return PROVIDER_Benito.to_string();
    }
    ...
    let model = entry
        ...
        .unwrap_or(crate::benito::config::DEFAULT_MODEL);

    format!("{}:{model}", entry.slug)
}

fn is_BENITO_cloud_entry(
    entry: &crate::benito::config::schema::cloud_providers::CloudProviderCreds,
) -> bool {
    entry.slug == PROVIDER_Benito
        || matches!(entry.auth_style, AuthStyle::BenitoJwt)
        || looks_like_BENITO_backend(&entry.endpoint)
}
...
fn looks_like_BENITO_backend(url: &str) -> bool {
...
fn make_cloud_provider_by_slug(
    ...
    if entry.auth_style != AuthStyle::BenitoJwt && is_abstract_tier_model(&effective_model) {
    ...
    match entry.auth_style {
        ...
        AuthStyle::BenitoJwt => {
            // Route to the Benito backend — ignore the entry's endpoint
            // and model; use the backend provider with the configured default.
            log::debug!(
                "[providers][chat-factory] slug='{}' has auth_style=BenitoJwt → routing to Benito backend",
                slug
            );
            make_BENITO_backend(config)
        }
        AuthStyle::None => {
            let p = make_openai_compatible_provider_with_config(
                &entry.endpoint,
