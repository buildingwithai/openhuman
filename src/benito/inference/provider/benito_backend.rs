//! Inference via the Benito backend OpenAI-compatible API (`{api_url}/openai/v1/...`) using the app session JWT.
//! Session material is loaded via [`crate::benito::credentials`] (see also [`crate::api::jwt`] for shared helpers).
...
impl BenitoBackendProvider {
    ...
    fn state_dir(&self) -> PathBuf {
        self.options.BENITO_dir.clone().unwrap_or_else(|| {
    ...
    fn resolve_bearer(&self) -> anyhow::Result<String> {
        ...
        // 5,414 Sentry events on issue Benito-TAURI-1T.
        //
    ...
    fn inner(&self, token: &str) -> anyhow::Result<OpenAiCompatibleProvider> {
        // Hosted Benito API is chat-completions only; skip /v1/responses fallback so transport
        ...
        // never see this field (see `with_BENITO_thread_id`).
        ...
        .with_BENITO_thread_id())
    }
}

#[async_trait]
impl Provider for BenitoBackendProvider {
    ...
    fn stream_chat_with_system(
        ...
        stream::once(async move {
            Ok(StreamChunk::error(
                "streaming is not supported for Benito backend provider",
            ))
        })
        .boxed()
    }
