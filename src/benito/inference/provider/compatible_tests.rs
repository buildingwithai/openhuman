...
fn native_request_emits_thread_id_when_present() {
    ...
    assert!(
        ...
        "absent thread_id must not be serialized so non-Benito backends don't reject the field"
    );
}
...
/// the Benito backend's `Benito.billing` block also never lands,
...
async fn outbound_thread_id_is_gated_per_provider() {
    use crate::benito::inference::provider::thread_context::with_thread_id;

    let third_party = make_provider("Venice", "https://api.venice.ai", None);
    let Benito =
        make_provider("Benito", "https://api.Benito.test", None).with_BENITO_thread_id();

    with_thread_id("thread-xyz", async {
        assert!(
            third_party.outbound_thread_id().is_none(),
            "third-party OpenAI-compatible providers must NOT see the Benito thread_id extension \
             — unknown fields can trip strict input validation on Venice/Moonshot/Groq/etc."
        );
        assert_eq!(
            Benito.outbound_thread_id().as_deref(),
            Some("thread-xyz"),
            "the Benito backend provider opts in via with_BENITO_thread_id() and must \
    ...
    let req = ApiChatRequest {
        ...
        messages: vec![
            Message {
                ...
                content: "You are Benito".to_string(),
            },
            Message {
                role: "user".to_string(),
...
fn prompt_guided_tool_fallback_injects_system_instruction() {
    ...
    let tools = vec![crate::benito::tools::ToolSpec {
...
fn tool_specs_convert_to_openai_format() {
    let specs = vec![crate::benito::tools::ToolSpec {
...
fn normalize_function_arguments_invalid_json_string_falls_back_to_empty_object() {
    // Benito-TAURI-6F: model emitted malformed JSON in `function.arguments`.
...
fn parse_provider_tool_call_from_value_guards_malformed_arguments() {
    // Benito-TAURI-6F: the early-return path in
