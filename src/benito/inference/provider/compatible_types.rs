...
pub(crate) struct NativeChatRequest {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub(crate) tool_choice: Option<String>,
    /// Benito backend extension: stable conversation identifier so the
    ...
    /// set — see `crate::benito::inference::provider::thread_context`).
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// (carrying token counts and `Benito.billing.charged_amount_usd`
    /// when the Benito backend is in front). Without this, streaming
...
/// `crate::benito::inference::provider::traits::StreamOptions`, which is the
...
pub(crate) struct ApiChatResponse {
    ...
    /// Benito backend metadata (usage + billing summary).
    #[serde(default)]
    pub(crate) Benito: Option<BenitoMeta>,
}
...
/// Benito backend metadata appended to the response JSON.
#[derive(Debug, Deserialize, Default)]
pub(crate) struct BenitoMeta {
    #[serde(default)]
    pub(crate) usage: Option<BenitoUsage>,
    #[serde(default)]
    pub(crate) billing: Option<BenitoBilling>,
}

#[derive(Debug, Deserialize, Default)]
pub(crate) struct BenitoUsage {
...
#[derive(Debug, Deserialize, Default)]
pub(crate) struct BenitoBilling {
    #[serde(default)]
