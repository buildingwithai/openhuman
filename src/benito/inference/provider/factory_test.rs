...
fn oh_entry(id: &str) -> CloudProviderCreds {
    CloudProviderCreds {
        id: id.to_string(),
        slug: "Benito".to_string(),
        label: "Benito".to_string(),
        endpoint: "https://api.Benito.ai/v1".to_string(),
        auth_style: AuthStyle::BenitoJwt,
        ..Default::default()
    }
}
...
fn BENITO_literal() {
    ...
    let (_, model) = create_chat_provider_from_string("reasoning", "Benito", &config)
        .expect("Benito literal must build");
    assert!(!model.is_empty(), "model must not be empty");
}

#[test]
fn cloud_no_providers_falls_back_to_Benito() {
    let config = Config::default();
...
#[test]
fn BENITO_slug_routes_to_backend() {
    let config = config_with_providers(vec![oh_entry("p_oh")]);
    let (_, model) =
        create_chat_provider_from_string("reasoning", "Benito:", &config).expect("build");
...
fn orcarouter_legacy_type_seeds_defaults() {
    use crate::benito::config::schema::cloud_providers::migrate_legacy_fields;
...
#[test]
fn all_workloads_default_to_Benito() {
    let config = Config::default();
    for role in &[
        "chat",
        "reasoning",
        ...
        assert_eq!(
            ...
            "Benito",
            "role={role} must default to Benito"
        );
    }
}
...
// routed to the Benito backend regardless of the user's `chat_provider`
...
fn workload_override_respected() {
    ...
    assert_eq!(provider_for_role("reasoning", &config), "Benito");
}
...
#[test]
fn primary_cloud_defaults_to_BENITO_when_no_providers() {
    let config = Config::default();
...
#[test]
fn legacy_inference_url_custom_provider_wins_over_BENITO_primary_for_unset_role() {
...
#[test]
fn legacy_inference_url_without_matching_provider_stays_on_BENITO_primary() {
...
#[test]
fn hosted_endpoint_entry_is_treated_as_BENITO_backend() {
...
#[test]
fn explicit_BENITO_route_ignores_legacy_inference_url() {
    let mut custom = openai_entry("p_custom", "custom");
    custom.endpoint = "https://api.example.com/v1".to_string();
    ...
    config.primary_cloud = Some("p_oh".to_string());
    config.inference_url = Some("https://api.example.com/v1".to_string());
    config.reasoning_provider = Some("Benito".to_string());
...
#[test]
fn summarization_defaults_to_BENITO_like_memory() {
    let config = Config::default();
    assert_eq!(provider_for_role("memory", &config), "Benito");
    assert_eq!(provider_for_role("summarization", &config), "Benito");
}

#[test]
fn unknown_workload_falls_back_to_Benito() {
    let config = Config::default();
    assert_eq!(
        provider_for_role("nope-not-a-workload", &config),
        "Benito"
    );
    assert_eq!(provider_for_role("", &config), "Benito");
}

#[test]
fn BENITO_backend_uses_config_path_parent_as_state_dir() {
    let mut config = Config::default();
    ...
    let (_provider, model) = create_chat_provider("reasoning", &config)
        .expect("Benito backend must build with no cloud_providers");
...
#[test]
fn verify_session_active_called_for_custom_provider_not_for_Benito() {
    // Benito backend must always build (no session gate applied).
    let config = Config::default();
    assert!(create_chat_provider_from_string("reasoning", "Benito", &config).is_ok(),);
