...
/// Fixed id for the single inference backend (Benito API).
pub const INFERENCE_BACKEND_ID: &str = "Benito";

#[derive(Debug, Clone)]
pub struct ProviderRuntimeOptions {
    pub auth_profile_override: Option<String>,
    pub BENITO_dir: Option<PathBuf>,
...
pub async fn list_configured_models(
    ...
    let config = crate::benito::config::Config::load_or_init()
        .await
    ...
    let api_key =
        crate::benito::inference::provider::factory::lookup_key_for_slug(&entry.slug, &config)
            .unwrap_or_default();

    let client = crate::benito::config::build_runtime_proxy_client_with_timeouts(
    ...
    if !status.is_success() {
        ...
        let truncated = crate::benito::util::truncate_with_ellipsis(&sanitized, 300);
        return Err(format!(
            "provider returned {}: {}",
            status.as_u16(),
            truncated
...
pub fn sanitize_api_error(input: &str) -> String {
    ...
    crate::benito::util::truncate_with_ellipsis(&scrubbed, MAX_API_ERROR_CHARS)
}
...
pub fn format_error_chain(err: &dyn std::error::Error) -> String {
    ...
    crate::benito::util::truncate_with_suffix(&scrubbed, TRANSPORT_ERROR_MAX_CHARS, "…")
}
...
/// Benito-TAURI-6Y / 2E / 84 / T: thousands of events/day per user from
...
/// actionable signal for Benito Sentry triage.
...
/// same body from the Benito **backend** stays Sentry-actionable —
...
pub(super) fn is_provider_config_rejection_http(
    ...
    matches!(status.as_u16(), 400 | 404 | 422)
        && provider != BENITO_backend::PROVIDER_LABEL
...
/// - **401/403 from the Benito backend provider** — the user's app session
///   expired. That is expected user-state, not a server bug, and reporting it
///   spams Sentry (Benito-TAURI-1T: 5,414 events from a single user whose
...
pub async fn api_error(provider: &str, response: reqwest::Response) -> anyhow::Error {
    ...
    let is_backend = provider == BENITO_backend::PROVIDER_LABEL;
    ...
    if is_auth_failure && is_backend {
        ...
        crate::core::event_bus::publish_global(
            crate::core::event_bus::DomainEvent::SessionExpired {
                source: "llm_provider.BENITO_backend".to_string(),
...
/// - `backend_url`: the Benito product backend URL (`config.api_url`).
///   Used by the fallback [`BENITO_backend::BenitoBackendProvider`]
///   which routes inference to `{backend}/openai/v1/...` with the app
///   session JWT.
/// - `api_key`: the API key for the custom inference endpoint. Ignored on
///   the Benito fallback path (the backend uses a session JWT, not a
///   user-supplied key).
pub fn create_backend_inference_provider(
    ...
    if let (Some(url), Some(key)) = (inference_url, api_key) {
        ...
        Ok(Box::new(
            crate::benito::inference::provider::compatible::OpenAiCompatibleProvider::new_no_responses_fallback(
                "custom_openai",
                url,
                Some(key),
                crate::benito::inference::provider::compatible::AuthStyle::Bearer,
            ),
        ))
    } else {
        if api_key.is_some() && inference_url.is_none() {
            log::warn!(
                "[providers] api_key provided without inference_url — key will be ignored, using Benito backend"
            );
        }
        log::info!(
            "[providers] inference target = BENITO_backend (backend_url={}, inference_url_set={}, api_key_set={})",
            backend_url.unwrap_or("<default>"),
            inference_url.is_some(),
            api_key.is_some()
        );
        Ok(Box::new(BENITO_backend::BenitoBackendProvider::new(
            backend_url,
            options,
        )))
    }
...
pub fn create_resilient_provider(
    ...
    backend_url: Option<&str>,
    api_key: Option<&str>,
    reliability: &crate::benito::config::ReliabilityConfig,
) -> anyhow::Result<Box<dyn Provider>> {
...
pub fn create_resilient_provider_with_options(
    ...
    if !reliability.fallback_providers.is_empty() {
        tracing::warn!(
            "reliability.fallback_providers is ignored; inference uses only the Benito backend"
        );
    }
...
pub fn create_routed_provider(
    ...
    api_key: Option<&str>,
    reliability: &crate::benito::config::ReliabilityConfig,
    model_routes: &[crate::benito::config::ModelRouteConfig],
    default_model: &str,
) -> anyhow::Result<Box<dyn Provider>> {
        ...
        .map(|r| {
            (
                ...
                router::Route {
                    ...
                    model: r.model.clone(),
                    context_window:
                        crate::benito::inference::model_context::context_window_for_model(
                            &r.model,
                        ),
                },
            )
...
pub fn create_intelligent_routing_provider(
    ...
    // `custom_openai` and 404. The Benito backend can dispatch tier names
    ...
    let provider = crate::benito::routing::new_provider(
        remote,
        &config.local_ai,
        default_model,
...
pub fn list_providers() -> Vec<ProviderInfo> {
    vec![ProviderInfo {
        ...
        display_name: "Benito (backend)",
        aliases: &["backend", "benito-backend"],
        local: false,
    }]
}
...
mod tests {
    ...
    fn list_configured_models_accepts_slug() {
        ...
        // the slug is stable).
        use crate::benito::config::schema::cloud_providers::{AuthStyle, CloudProviderCreds};
    ...
    fn skips_sentry_report_for_transient_upstream_statuses() {
        ...
        // Reporting each attempt individually floods Sentry (Benito-TAURI-2E
    ...
    mod provider_config_rejection_suppression {
        ...
        #[test]
        fn BENITO_backend_same_body_is_not_suppressed() {
