...
- `src/Benito/config/` — `config_rpc::*` for `Config` reads and the workspace dir resolver.
- `src/Benito/autocomplete/` — `AutocompleteStatus` snapshot.
- `src/Benito/local_ai/` — `LocalAiStatus` snapshot.
- `src/Benito/screen_intelligence/` — `AccessibilityStatus` snapshot.
- `src/Benito/service/` — `ServiceState` / `ServiceStatus` runtime info.
- `src/Benito/credentials/` — `session_support::build_session_state` for the auth slice.
- `src/api/{config,jwt}` — backend base URL + bearer token used by the cached current-user fetch.

## Called by

- `src/Benito/agent/harness/session/builder.rs` — agent builder reads cached app state when resolving identity.
