...
use crate::benito::autocomplete::AutocompleteStatus;
...
use crate::benito::inference::LocalAiStatus;
use crate::benito::screen_intelligence::AccessibilityStatus;
use crate::benito::service::{ServiceState, ServiceStatus};
...
pub struct AppStateSnapshot {
    pub auth: crate::benito::credentials::responses::AuthStateResponse,
...
pub fn peek_cached_current_user_identity() -> Option<crate::benito::agent::prompts::UserIdentity>
{
    let cache = CURRENT_USER_CACHE.lock();
    ...
    let identity = crate::benito::agent::prompts::UserIdentity { id, name, email };
...
async fn build_runtime_snapshot(config: &Config) -> RuntimeSnapshot {
    let screen_intelligence = {
        let _ = crate::benito::screen_intelligence::global_engine()
            .apply_config(config.screen_intelligence.clone())
            .await;
        crate::benito::screen_intelligence::global_engine()
            .status()
            .await
    };

    let local_ai = match crate::benito::inference::rpc::inference_status(config).await {
        Ok(outcome) => outcome.value,
        Err(error) => {
            warn!("{LOG_PREFIX} local_ai status failed during snapshot: {error}");
            crate::benito::inference::LocalAiStatus::disabled(config)
        }
    };

    let autocomplete = crate::benito::autocomplete::global_engine()
        .status()
        .await;

    let service = match crate::benito::service::status(config) {
        Ok(status) => status,
        Err(error) => {
...
pub async fn snapshot() -> Result<RpcOutcome<AppStateSnapshot>, String> {
    ...
    // Sentry against `Benito.app_state_snapshot`.
