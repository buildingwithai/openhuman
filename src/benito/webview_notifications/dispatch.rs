...
//! the title with `Benito:` makes it trivial for the user to tell
...
/// naturally (`Benito: New message from …`).
pub const BENITO_TITLE_PREFIX: &str = "Benito: ";
...
///
/// Layout: `Benito: <Provider> — <raw title>` when both pieces are
/// present, collapsing to `Benito: <Provider>` when the raw title is
/// empty or whitespace-only.
pub fn format_title(provider_label: &str, raw_title: &str) -> String {
    let raw = raw_title.trim();
    if raw.is_empty() {
        format!("{BENITO_TITLE_PREFIX}{provider_label}")
    } else {
        format!("{BENITO_TITLE_PREFIX}{provider_label} — {raw}")
    }
}
...
mod tests {
    ...
    fn prefix_empty_title_falls_back_to_provider_only() {
        assert_eq!(format_title("Slack", ""), "Benito: Slack");
        assert_eq!(format_title("Slack", "   "), "Benito: Slack");
    }
    ...
    fn prefix_with_title_joins_with_em_dash() {
        assert_eq!(
            ...
            "Benito: Gmail — New mail from Alice"
        );
    }
