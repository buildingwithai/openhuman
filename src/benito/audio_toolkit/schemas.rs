...
use crate::benito::audio_toolkit::types::{
...
fn handle_generate_podcast(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::audio_toolkit::generate_podcast(&config, request).await?)
    })
}
...
fn handle_email_podcast(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let request: EmailPodcastRequest =
            serde_json::from_value(Value::Object(params)).map_err(|e| e.to_string())?;
        to_json(crate::benito::audio_toolkit::email_podcast(&config, request).await?)
    })
}
...
fn handle_generate_and_email_podcast(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::audio_toolkit::generate_and_email_podcast(&config, generated, email)
                .await?,
        )
    })
}
