...
const DERIVED_TO_BACKEND: Option<CapabilityPrivacy> = Some(CapabilityPrivacy {
    ...
    destinations: &["Benito backend", "TinyHumans Neocortex"],
});
...
// Self-update flows talk to GitHub Releases directly, not the Benito
...
// device — they are sent to backend.composio.dev, not the Benito backend.
...
const CAPABILITIES: &[Capability] = &[
    ...
    Capability {
        ...
        how_to: "Run `Benito-core mcp` and configure the local MCP client to launch that command.",
        status: CapabilityStatus::Beta,
        privacy: LOCAL_RAW,
    },
    Capability {
        ...
        how_to: "Set `[searxng] enabled = true` and `base_url` in config.toml, or use BENITO_SEARXNG_* environment variables.",
        status: CapabilityStatus::Beta,
    ...
    Capability {
        ...
        domain: "intelligence",
        category: CapabilityCategory::Intelligence,
        description: "Discover Benito's MCP stdio tools and controller-backed tools from one local registry, including versions, routes, input/output schemas, allowed agents, and health state.",
        how_to: "Call Benito.tool_registry_list over core JSON-RPC, or Benito.tool_registry_get with a tool_id such as memory.search.",
        status: CapabilityStatus::Beta,
        privacy: LOCAL_RAW,
    },
    Capability {
    ...
    Capability {
        ...
        description:
            "Route Composio tool calls directly to backend.composio.dev with your own API key, \
             bypassing the Benito backend proxy. Tool execution only — trigger webhooks still \
    ...
    Capability {
        ...
        category: CapabilityCategory::LocalAI,
        description:
            "Download and reuse an Benito-managed CPython runtime for Python-backed local integrations such as MCP servers, with a system-Python override reserved for development.",
    ...
    Capability {
        ...
        domain: "auth",
        category: CapabilityCategory::Auth,
        description: "Choose which built-in tools Benito can use on your behalf during setup.",
    ...
    Capability {
        ...
        domain: "channels",
        category: CapabilityCategory::Channels,
        description: "Read and search WhatsApp Web conversations and messages after connecting WhatsApp in Benito. Data is stored locally only and never transmitted.",
