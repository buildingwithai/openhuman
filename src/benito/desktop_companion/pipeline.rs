...
async fn stt(samples: &[i16], sample_rate: u32) -> Result<String, String> {
    ...
    let wav_bytes = crate::benito::meet_agent::wav::pack_pcm16le_mono_wav(samples, sample_rate);
...
You are Benito, a helpful desktop AI companion. The user is talking to you \
...
async fn gather_screen_context() -> Option<String> {
    #[cfg(target_os = "macos")]
    {
        let context = crate::benito::accessibility::foreground_context();
        context.map(|ctx| {
            format!(
                "App: {} | Window: {}",
