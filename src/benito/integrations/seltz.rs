...
use crate::benito::tools::traits::{Tool, ToolCallOptions, ToolResult};
...
///
/// Requires a `SELTZ_API_KEY` (or `BENITO_SELTZ_API_KEY`) environment
...
impl SeltzSearchTool {
    ...
    fn render_results_plain(&self, docs: &[SeltzDocument], query: &str) -> String {
        ...
        for (i, doc) in docs.iter().take(self.max_results).enumerate() {
            ...
            if !content.is_empty() {
                let truncated = crate::benito::util::truncate_with_ellipsis(content, 500);
    ...
    fn render_results_markdown(&self, docs: &[SeltzDocument], query: &str) -> String {
        ...
        for doc in docs.iter().take(self.max_results) {
            ...
            let content = doc.content.trim();
            if !content.is_empty() {
                let truncated = crate::benito::util::truncate_with_suffix(content, 500, "…");
        ...
        let api_key = self.api_key.as_deref().ok_or_else(|| {
            anyhow::anyhow!(
                "Seltz search unavailable: no API key configured. \
                 Set SELTZ_API_KEY or BENITO_SELTZ_API_KEY, \
