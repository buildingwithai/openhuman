...
async fn post_400_propagates_backend_error_envelope_message() {
    ...
    // `backend-Benito/src/middlewares/errorHandler.ts` — the 400
...
// ── Benito-TAURI-BC regression: wire format pins to classifier ─

/// Regression guard for Benito-TAURI-BC: the exact bail message
...
async fn post_400_user_input_failure_classifies_as_backend_user_error() {
    ...
    assert_eq!(
        ...
        "Benito-TAURI-BC: propagated 400 must classify as ProviderUserState (more \
