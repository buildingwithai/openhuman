...
/// `backend-Benito/src/middlewares/errorHandler.ts`). When the body parses
...
pub(crate) fn extract_error_detail(body: &str, max_bytes: usize) -> String {
    ...
    if let Ok(v) = serde_json::from_str::<serde_json::Value>(body) {
        if let Some(msg) = v.get("error").and_then(|e| e.as_str()) {
            ...
            if !trimmed.is_empty() {
                return crate::benito::util::truncate_at_byte_boundary(trimmed, max_bytes);
            }
        }
    }
    crate::benito::util::truncate_at_byte_boundary(body, max_bytes)
}
...
///
/// See issue #2075 / Sentry `Benito-TAURI-H6`, `-HN`: a misconfigured
...
impl IntegrationClient {
    pub fn new(backend_url: String, auth_token: String) -> Self {
        // Defense-in-depth (issue #2075 / Sentry Benito-TAURI-H6, -HN):
    ...
    pub async fn post<T: serde::de::DeserializeOwned>(
        ...
        let resp = self
            ...
            .map_err(|e| {
                ...
                // that no retry on our side can resolve (Benito-TAURI-2G).
        ...
        if !status.is_success() {
            ...
            // auth-state failures (e.g. Benito-TAURI-BC: SharePoint
        ...
        if !envelope.success {
            ...
            // "Missing required fields: …" — Benito-TAURI-3R / -3S / -34 /
    ...
    pub async fn get<T: serde::de::DeserializeOwned>(&self, path: &str) -> anyhow::Result<T> {
        ...
        let resp = self
            ...
            .map_err(|e| {
                ...
                // skip Sentry. Benito-TAURI-2G: TLS handshake EOF
        ...
        if !status.is_success() {
            ...
            let detail = extract_error_detail(&body_text, MAX_ERROR_BODY_LEN);
            let status_str = status.as_u16().to_string();
            // Mirrors the post() site — see Benito-TAURI-BC. 4xx
        ...
        if !envelope.success {
            ...
            // for Benito-TAURI-3R/-3S/-34/-97 rationale. User-state
...
pub async fn pricing_for_config(
    ...
    use crate::benito::config::schema::COMPOSIO_MODE_DIRECT;
...
pub fn build_client(config: &crate::benito::config::Config) -> Option<Arc<IntegrationClient>> {
    ...
    // (Benito-TAURI-51 / -80 / -7Z). The helper falls through to env /
