...
/// Get the path to the Benito data directory.
/// If an active user is set, returns the user-scoped directory under the
/// env-aware root returned by `default_root_BENITO_dir()`
/// (for example `~/.Benito/users/{user_id}` in production or
/// `~/.Benito-staging/users/{user_id}` when `BENITO_APP_ENV=staging`);
/// otherwise it falls back to that root directory itself.
pub fn get_data_dir() -> Result<PathBuf, String> {
    let root_dir = crate::benito::config::default_root_BENITO_dir()
        .map_err(|e| format!("Cannot determine app data directory: {e}"))?;
    std::fs::create_dir_all(&root_dir)
        .map_err(|e| format!("Failed to create data directory: {e}"))?;

    let data_dir = if let Some(user_id) = crate::benito::config::read_active_user_id(&root_dir) {
        let user_dir = crate::benito::config::user_BENITO_dir(&root_dir, &user_id);
...
/// Get the path to the encryption key file under the env-aware Benito root
/// (for example `~/.Benito/encryption.key` or `~/.Benito-staging/encryption.key`).
