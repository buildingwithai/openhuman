...
//!
//! Sampled on a 30s cadence by [`crate::benito::scheduler_gate::gate`]; this
...
fn sample_power() -> (bool, Option<f32>) {
    ...
    let env_on_ac = std::env::var("BENITO_ON_AC_POWER").ok().and_then(|v| {
    ...
    let env_charge = std::env::var("BENITO_BATTERY_CHARGE")
        .ok()
...
fn detect_server_mode(no_battery: bool) -> bool {
    if let Ok(v) = std::env::var("BENITO_DEPLOYMENT") {
