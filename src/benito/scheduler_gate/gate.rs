...
use crate::benito::config::{Config, SchedulerGateConfig};
use crate::benito::scheduler_gate::policy::{decide, PauseReason, Policy};
use crate::benito::scheduler_gate::signals::Signals;
...
pub async fn wait_for_capacity() -> Option<LlmPermit> {
    loop {
        ...
        // `benito::agent::triage::evaluator::tests::*` hangs reported
        // after #1516.
        if STATE.get().is_some() && is_signed_out() {
            let paused_ms = STATE
                .get()
...
mod tests {
    ...
    async fn wait_for_capacity_acquires_immediately_when_signed_out_and_uninit() {
        ...
        // `benito::agent::triage::evaluator::tests::*` hangs that surfaced
