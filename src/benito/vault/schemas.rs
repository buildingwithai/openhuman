...
fn handle_create(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::vault::ops::vault_create(
                &config,
                &name,
                &root_path,
                include_globs,
...
fn handle_list(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::benito::vault::ops::vault_list(&config).await?)
    })
}
...
fn handle_get(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::vault::ops::vault_get(&config, vault_id.trim()).await?)
    })
}
...
fn handle_files(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let vault_id = read_required::<String>(&params, "vault_id")?;
        to_json(crate::benito::vault::ops::vault_files(&config, vault_id.trim()).await?)
    })
}
...
fn handle_remove(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::vault::ops::vault_remove(&config, vault_id.trim(), purge_memory)
                .await?,
        )
    })
}
...
fn handle_sync(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        let vault_id = read_required::<String>(&params, "vault_id")?;
        to_json(crate::benito::vault::ops::vault_sync(&config, vault_id.trim()).await?)
    })
}
