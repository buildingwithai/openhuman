...
//! <https://github.com/tinyhumansai/Benito/issues/133>.
...
pub async fn accessibility_status() -> Result<RpcOutcome<AccessibilityStatus>, String> {
    if let Ok(config) = crate::benito::config::Config::load_or_init().await {
...
pub async fn accessibility_globe_listener_start() -> Result<RpcOutcome<GlobeHotkeyStatus>, String> {
    ...
    let result = crate::benito::accessibility::globe_listener_start()?;
    Ok(RpcOutcome::single_log(
        result,
...
{
    let result = crate::benito::accessibility::globe_listener_poll()?;
    Ok(RpcOutcome::single_log(
        result,
...
pub async fn accessibility_globe_listener_stop() -> Result<RpcOutcome<GlobeHotkeyStatus>, String> {
    ...
    let result = crate::benito::accessibility::globe_listener_stop()?;
    Ok(RpcOutcome::single_log(
        result,
