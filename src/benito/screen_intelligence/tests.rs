...
use crate::benito::accessibility::{parse_foreground_output, AppContext};
use crate::benito::config::{Config, ScreenIntelligenceConfig};
use crate::benito::embeddings::NoopEmbedding;
use crate::benito::memory::store::UnifiedMemory;
...
fn screen_intelligence_env_lock() -> std::sync::MutexGuard<'static, ()> {
    match crate::benito::config::TEST_ENV_LOCK.lock() {
        Ok(guard) => guard,
...
async fn analyze_and_persist_frame_writes_unified_memory_document() {
    ...
    let _mock = EnvVarGuard::set(
        "BENITO_SCREEN_INTELLIGENCE_MOCK_VISION_JSON",
