...
pub(crate) async fn persist_vision_summary(
    ...
    // override BENITO_WORKSPACE per-test and the global may point elsewhere.
    let config = crate::benito::config::Config::load_or_init()
        .await
        .map_err(|err| format!("config load failed: {err}"))?;
    let client = crate::benito::memory::MemoryClient::from_workspace_dir(config.workspace_dir)
