//! `Benito screen-intelligence doctor` — diagnostic readiness check.
...
pub(super) fn run_doctor(args: &[String]) -> Result<()> {
    if args.iter().any(|a| is_help(a)) {
        println!("Usage: Benito screen-intelligence doctor [-v]");
        println!();
    ...
    rt.block_on(async {
        ...
        let doctor_json =
            crate::benito::screen_intelligence::rpc::accessibility_doctor_cli_json()
                .await
        ...
        // Vision config check.
        let config = crate::benito::config::Config::load_or_init().await.ok();
