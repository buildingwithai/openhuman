...
/// `Benito screen-intelligence status` — print current engine status as JSON.
pub(super) fn run_status(args: &[String]) -> Result<()> {
    if args.iter().any(|a| is_help(a)) {
        println!("Usage: Benito screen-intelligence status [-v]");
        println!();
...
/// `Benito screen-intelligence start` — start a capture + vision session.
pub(super) fn run_start_session(args: &[String]) -> Result<()> {
    if args.iter().any(|a| is_help(a)) {
        println!(
            "Usage: Benito screen-intelligence start [--ttl <secs>] [--no-vision-model] [-v]"
        );
        println!();
    ...
    rt.block_on(async {
        ...
        let params = crate::benito::screen_intelligence::StartSessionParams {
            consent: true,
...
/// `Benito screen-intelligence stop` — stop an active session.
pub(super) fn run_stop_session(args: &[String]) -> Result<()> {
    if args.iter().any(|a| is_help(a)) {
        println!("Usage: Benito screen-intelligence stop [-v]");
        println!();
