//! `Benito screen-intelligence run` — start the standalone capture + vision loop.

use anyhow::Result;

use super::{is_help, parse_opts};

/// Delegates to [`crate::benito::screen_intelligence::server::run_standalone`],
...
pub(super) fn run_server(args: &[String]) -> Result<()> {
    ...
    if rest.iter().any(|a| is_help(a)) {
        println!("Usage: Benito screen-intelligence run [--ttl <secs>] [--keep] [--no-vision-model] [-v]");
        println!();
    ...
    rt.block_on(async {
        ...
        let server_config = crate::benito::screen_intelligence::server::SiServerConfig {
        ...
        crate::benito::screen_intelligence::server::run_standalone(config, server_config)
            .await
