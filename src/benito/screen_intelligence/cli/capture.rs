...
/// `Benito screen-intelligence capture` — take a single screenshot and print info.
pub(super) fn run_capture(args: &[String]) -> Result<()> {
    if args.iter().any(|a| is_help(a)) {
        println!("Usage: Benito screen-intelligence capture [--keep] [-v]");
        println!();
    ...
    rt.block_on(async {
        ...
        if result.ok {
            ...
            if opts.keep {
                ...
                match crate::benito::screen_intelligence::AccessibilityEngine::save_capture_test_result(
...
/// `Benito screen-intelligence vision` — inspect recent vision summaries.
pub(super) fn run_vision(args: &[String]) -> Result<()> {
    if args.iter().any(|a| is_help(a)) {
        println!("Usage: Benito screen-intelligence vision [--limit <n>] [-v]");
        println!();
    ...
    rt.block_on(async {
        ...
        if result.summaries.is_empty() {
            ...
            eprintln!("  Start a session first: Benito screen-intelligence start");
        } else {
