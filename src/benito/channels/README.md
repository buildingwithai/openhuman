...
- `pub fn build_system_prompt` — re-exported from `crate::benito::context::channels_prompt`.
...
- `src/Benito/agent/` — inbound messages spawn or resume agent runs through `runtime/dispatch.rs`.
- `src/Benito/credentials/` — per-channel auth tokens, refresh flow.
- `src/Benito/config/schema/channels.rs` — runtime channel configuration.
- `src/Benito/threads/` — thread state for platforms with native threading (Slack `thread_ts`).
- `src/Benito/notifications/` — surface inbound deliveries to the UI.
- `src/Benito/encryption/` — at-rest secret protection.
- `src/core/event_bus/` — emits `DomainEvent::Channel(*)`; `channels/bus.rs` registers `ChannelInboundSubscriber`.

## Called by

- `src/Benito/threads/ops.rs` — thread lifecycle uses channel send paths.
- `src/Benito/memory/conversations/bus.rs` — persists incoming channel messages as conversation memories.
- `src/Benito/cron/bus.rs` — scheduled triggers can post via channels.
- `src/Benito/config/schema/channels.rs` — config layer references channel types for validation.
