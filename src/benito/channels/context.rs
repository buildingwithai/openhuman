...
use crate::benito::inference::provider::{ChatMessage, Provider};
...
pub(crate) struct ChannelRuntimeContext {
    ...
    pub(crate) reliability: Arc<crate::benito::config::ReliabilityConfig>,
    ...
    pub(crate) multimodal: crate::benito::config::MultimodalConfig,
}
...
pub(crate) async fn build_memory_context(
    ...
    if let Ok(entries) = mem
        .recall(user_msg, 5, crate::benito::memory::RecallOpts::default())
        .await
    {
...
mod tests {
    ...
    use crate::benito::inference::provider::Provider;
    use crate::benito::memory::{Memory, MemoryCategory, MemoryEntry};
    use crate::benito::tools::{Tool, ToolResult};
    ...
    fn runtime_context() -> ChannelRuntimeContext {
        ChannelRuntimeContext {
            ...
            api_url: None,
            inference_url: None,
            reliability: Arc::new(crate::benito::config::ReliabilityConfig::default()),
            provider_runtime_options:
                crate::benito::inference::provider::ProviderRuntimeOptions::default(),
            workspace_dir: Arc::new(PathBuf::from("/tmp")),
            message_timeout_secs: CHANNEL_MESSAGE_TIMEOUT_SECS,
            multimodal: crate::benito::config::MultimodalConfig::default(),
        }
    }
    ...
    fn clear_and_compact_sender_history_update_cached_messages() {
        ...
        history.push(crate::benito::inference::provider::ChatMessage::user(
            "short",
        ));
        history.extend((0..20).map(|idx| {
            crate::benito::inference::provider::ChatMessage::assistant("x".repeat(700 + idx))
        }));
        ctx.conversation_histories
            .lock()
            .unwrap()
