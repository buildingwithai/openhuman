...
use crate::benito::agent::harness::build_tool_instructions_filtered;
use crate::benito::agent::host_runtime;
use crate::benito::channels::context::{
...
use crate::benito::channels::dingtalk::DingTalkChannel;
use crate::benito::channels::discord::DiscordChannel;
use crate::benito::channels::email_channel::EmailChannel;
use crate::benito::channels::imessage::IMessageChannel;
use crate::benito::channels::irc;
use crate::benito::channels::irc::IrcChannel;
use crate::benito::channels::lark::LarkChannel;
use crate::benito::channels::linq::LinqChannel;
#[cfg(feature = "channel-matrix")]
use crate::benito::channels::matrix::MatrixChannel;
use crate::benito::channels::mattermost::MattermostChannel;
use crate::benito::channels::qq::QQChannel;
use crate::benito::channels::signal::SignalChannel;
use crate::benito::channels::slack::SlackChannel;
use crate::benito::channels::telegram::TelegramChannel;
use crate::benito::channels::traits;
use crate::benito::channels::whatsapp::WhatsAppChannel;
#[cfg(feature = "whatsapp-web")]
use crate::benito::channels::whatsapp_web::WhatsAppWebChannel;
use crate::benito::channels::Channel;
use crate::benito::config::Config;
use crate::benito::context::channels_prompt::build_system_prompt;
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    crate::benito::skills::bus::register_skill_cleanup_subscriber();
    ...
    // alive for the lifetime of the process (same pattern as TracingSubscriber).
    {
        ...
        EMAIL_SIG_HANDLE.get_or_init(|| {
            crate::benito::learning::extract::signature::register_email_signature_subscriber()
        });
    }
    ...
    // client is initialised later in `start_channels`).
    {
        ...
        REBUILD_TRIGGER_HANDLE.get_or_init(|| {
            if let Some(client) = crate::benito::memory::global::client_if_ready() {
                use crate::benito::learning::cache::FacetCache;
                use crate::benito::learning::scheduler::register_event_trigger;
                use crate::benito::learning::StabilityDetector;
                use std::sync::Arc;
                ...
                crate::benito::learning::scheduler::spawn_rebuild_loop(
                    Arc::clone(&detector),
                    crate::benito::learning::scheduler::DEFAULT_REBUILD_INTERVAL,
                    shutdown_rx,
                );
    ...
    // PROFILE.md blocks (style, identity, tooling, vetoes, goals).
    {
        ...
        PROFILE_MD_RENDERER_HANDLE.get_or_init(|| {
            if let Some(client) = crate::benito::memory::global::client_if_ready() {
                use crate::benito::learning::cache::FacetCache;
                use crate::benito::learning::ProfileMdRenderer;
                use std::sync::Arc;
    ...
    if let Err(err) = crate::benito::agent::harness::AgentDefinitionRegistry::init_global(
    ...
    let skills = crate::benito::skills::load_skills(&workspace);
    ...
    // subagent via category_filter = "skill".
    let non_skill_tools: Vec<&Box<dyn crate::benito::tools::Tool>> = tools_registry
        .iter()
        .filter(|t| t.category() != crate::benito::tools::traits::ToolCategory::Skill)
        .collect();
    let non_skill_refs: Vec<&dyn crate::benito::tools::Tool> =
    ...
    let _cron_delivery_handle = bus.subscribe(Arc::new(
        crate::benito::cron::bus::CronDeliverySubscriber::new(Arc::clone(&channels_by_name)),
    ));
    ...
    let _proactive_handle = bus.subscribe(Arc::new(
        crate::benito::channels::proactive::ProactiveMessageSubscriber::new(
    ...
    let _tree_summarizer_handle = bus.subscribe(Arc::new(
        crate::benito::tree_summarizer::bus::TreeSummarizerEventSubscriber::new(),
    ));
