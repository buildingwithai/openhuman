...
use crate::benito::channels::routes::{
    get_or_create_provider, get_route_selection, handle_runtime_command_if_needed,
};
use crate::benito::channels::traits;
use crate::benito::channels::{Channel, SendMessage};
use crate::benito::composio::fetch_connected_integrations;
use crate::benito::config::Config;
use crate::benito::inference::provider::{self, ChatMessage};
use crate::benito::tools::{orchestrator_tools, Tool};
...
pub(crate) async fn process_channel_message(
    ...
    if ctx.auto_save_memory {
        ...
        let _ = ctx
            ...
            .store(
                ...
                &autosave_key,
                &msg.content,
                crate::benito::memory::MemoryCategory::Conversation,
                None,
            )
            .await;
    }
    ...
    // `crate::benito::agent::bus::register_agent_handlers`); this keeps
    ...
    let (success, response_text) = match llm_result {
        ...
        Ok(Err(e)) => {
            ...
            // Sentry funnel (Benito-TAURI-98) and emit `log::info!`
            // instead — `Err` propagation through the surrounding match
            // arm is unchanged.
            if crate::benito::agent::error::is_max_iterations_error(&e.to_string()) {
                log::info!(
                    target: "channels",
                ...
                // up via `agent.run_single` (`Benito API error
                ...
                // layers (Benito-TAURI-4F ~157ev / -1C ~87ev / -8F
