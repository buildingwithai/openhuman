...
use crate::benito::context::channels_prompt::{build_system_prompt, BOOTSTRAP_MAX_CHARS};
...
fn prompt_skills_compact_list() {
    let ws = make_workspace();
    let skills = vec![crate::benito::skills::Skill {
...
fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
    let msg = "Hello from Benito 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";

    // Reproduces the production crash path where channel logs truncate at 80 chars.
    let result =
        std::panic::catch_unwind(|| crate::benito::util::truncate_with_ellipsis(msg, 80));
    assert!(
        result.is_ok(),
