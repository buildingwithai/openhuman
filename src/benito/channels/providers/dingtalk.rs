...
impl DingTalkChannel {
    ...
    fn http_client(&self) -> reqwest::Client {
        crate::benito::config::build_runtime_proxy_client("channel.dingtalk")
    }
...
impl Channel for DingTalkChannel {
    ...
    async fn send(&self, message: &SendMessage) -> anyhow::Result<()> {
        ...
        let title = message.subject.as_deref().unwrap_or("Benito");
        let body = serde_json::json!({
            "msgtype": "markdown",
            "markdown": {
                "title": title,
...
allowed_users = ["user1", "*"]
"#;
        let config: crate::benito::config::schema::DingTalkConfig =
