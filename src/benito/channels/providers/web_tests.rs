...
fn detects_backend_budget_exhaustion_error() {
    assert!(is_inference_budget_exceeded_error(
        "Benito API error (402 Payment Required): Budget exceeded — add credits to continue."
    ));
    assert!(is_inference_budget_exceeded_error(
        "provider error: budget exceeded, please add credits"
    ));
    assert!(!is_inference_budget_exceeded_error(
        "Benito API error (500): Internal server error"
    ));
}
...
fn generic_error_copy_is_sanitized_and_has_discord_report_action() {
    ...
    assert!(message
        .contains("<Benito-link path=\"community/discord\">Report on Discord</Benito-link>"));
}
...
fn fingerprint_provider_binding_variants_differ() {
    let unset = fp(None, None, "orchestrator", "Benito");
