...
use crate::benito::config::{Config, StreamMode};
use crate::benito::security::pairing::PairingGuard;
use anyhow::Context;
...
impl TelegramChannel {
    ...
    pub(crate) fn http_client(&self) -> reqwest::Client {
        crate::benito::config::build_runtime_proxy_client("channel.telegram")
    }
    ...
    async fn load_config_without_env() -> anyhow::Result<Config> {
        ...
        let BENITO_dir = home.join(".Benito");
        ...
        config.workspace_dir = BENITO_dir.join("workspace");
        Ok(config)
    }
