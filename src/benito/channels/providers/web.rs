...
use crate::benito::agent::profiles::{AgentProfile, AgentProfileStore, DEFAULT_PROFILE_ID};
use crate::benito::agent::Agent;
use crate::benito::config::rpc as config_rpc;
use crate::benito::config::Config;
use crate::benito::prompt_injection::{
    enforce_prompt_input, PromptEnforcementAction, PromptEnforcementContext,
};
use crate::benito::threads::turn_state::{TurnStateMirror, TurnStateStore};
...
struct WebChatTaskResult {
    ...
    citations: Vec<crate::benito::agent::memory_loader::MemoryCitation>,
}
...
fn generic_inference_error_user_message() -> &'static str {
    "Something went wrong. Please try again.\nThis error has been reported. You can also report it on Discord.\n<Benito-link path=\"community/discord\">Report on Discord</Benito-link>"
}
    ...
    while let Some(c) = chars.next() {
        match c {
            '"' => {
                ...
                let sanitized = crate::benito::inference::provider::sanitize_api_error(trimmed);
                return Some(crate::benito::util::truncate_with_ellipsis(
                    &sanitized,
                    MAX_DETAIL_CHARS,
                ));
            }
...
fn classify_inference_error(err: &str) -> (&'static str, String) {
    ...
    } else if crate::benito::inference::provider::is_provider_config_rejection_message(err) {
        // #2079 / #2076 / #2202: an Benito abstract tier alias leaked to
    ...
    let handle = tokio::spawn(async move {
        ...
        match result {
            ...
            Err(err) => {
                ...
                // (Benito-TAURI-98). Substring match is required here
                ...
                // handshake, ISP blocks — Benito-TAURI-32 for the RU
                ...
                if crate::benito::agent::error::is_max_iterations_error(&detailed) {
                    log::info!(
                        target: "web_channel",
...
async fn run_chat_task(
    ...
    let current_fp = SessionCacheFingerprint {
        ...
        provider_binding: crate::benito::inference::provider::provider_for_role(
            provider_role,
            &config,
        ),
    };
    ...
    if was_built_fresh {
        match crate::benito::memory::conversations::get_messages(
    ...
    let result = match crate::benito::inference::provider::thread_context::with_thread_id(
        thread_id.to_string(),
...
fn spawn_progress_bridge(
    mut rx: tokio::sync::mpsc::Receiver<crate::benito::agent::progress::AgentProgress>,
    client_id: String,
    thread_id: String,
    request_id: String,
...
fn load_reflection_chunks_for_thread(
    ...
    thread_id: &str,
) -> Option<Vec<crate::benito::subconscious::SourceChunk>> {
    let messages = crate::benito::memory::conversations::get_messages(
    ...
    let reflection_id = first
        ...
        .to_string();
    let reflection =
        crate::benito::subconscious::store::with_connection(workspace_dir, |conn| {
            crate::benito::subconscious::reflection_store::get_reflection(conn, &reflection_id)
        })
        .ok()
        .flatten()?;
