...
use crate::benito::config::{Config, DiscordConfig, IMessageConfig, TelegramConfig};
use crate::benito::credentials;
    ...
    if channel_id == "telegram" && auth_mode == ChannelAuthMode::BotToken {
        ...
        let (stream_mode, draft_update_interval_ms, silent_streaming, mention_only) =
            if let Some(existing) = persisted.channels_config.telegram.as_ref() {
                ...
                (
                    crate::benito::config::StreamMode::default(),
                    1000,
                    true,
                    false,
                )
...
/// merging the two storage layers Benito uses for connection state.
///
...
/// Default managed Telegram bot when `BENITO_APP_ENV` is staging and no username override is set.
...
const DEFAULT_TELEGRAM_BOT_USERNAME_PRODUCTION: &str = "Benitoaibot";

/// Resolve the managed Telegram bot username from env, or from staging vs production defaults using
/// `BENITO_APP_ENV` / `VITE_BENITO_APP_ENV` (via `app_env_from_env`).
fn telegram_bot_username() -> String {
    if let Ok(v) = std::env::var("BENITO_TELEGRAM_BOT_USERNAME") {
        return v;
    }
...
pub async fn discord_link_start(
    ...
    let instructions =
        format!("In Discord, send this message to the Benito bot: !start {link_token}");
...
pub async fn discord_list_guilds(
    ...
    RpcOutcome<Vec<crate::benito::channels::providers::discord::api::DiscordGuild>>,
    String,
> {
    use crate::benito::channels::providers::discord::api;
...
pub async fn discord_list_channels(
    ...
    RpcOutcome<Vec<crate::benito::channels::providers::discord::api::DiscordTextChannel>>,
    String,
> {
    use crate::benito::channels::providers::discord::api;
...
pub async fn discord_check_permissions(
    ...
    RpcOutcome<crate::benito::channels::providers::discord::api::BotPermissionCheck>,
    String,
> {
    use crate::benito::channels::providers::discord::api;
