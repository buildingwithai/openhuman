...
use super::*;
use crate::benito::threads::title::collapse_whitespace;
...
async fn message_append_returns_typed_not_found_for_stale_thread() {
    ...
    let _workspace_guard = EnvVarGuard::set_to_path("BENITO_WORKSPACE", workspace.path());
...
async fn create_thread_with_title(_workspace: &tempfile::TempDir, thread_id: &str, title: &str) {
    let dir = crate::benito::config::Config::load_or_init()
        .await
        .expect("load config")
        .workspace_dir;
