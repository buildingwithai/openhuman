...
//! paths, no Benito user_ids) and `feedback_pr_no_chat_content.md`
...
mod tests {
    ...
    fn home_path_in_unredacted_string_is_scrubbed() {
        let args = json!({ "action": "list", "cwd": "/Users/oxoxdev/work/Benito" });
        let red = redact_args(&args);
        let cwd = red["cwd"].as_str().unwrap();
        assert!(!cwd.contains("oxoxdev"), "got {cwd}");
        assert!(cwd.contains("<HOME>"));
        assert!(cwd.ends_with("/work/Benito"));
    }

    #[test]
    fn windows_home_path_is_scrubbed() {
        let args = json!({ "action": "list", "cwd": "C:\\Users\\oxoxdev\\work\\Benito" });
        let red = redact_args(&args);
        let cwd = red["cwd"].as_str().unwrap();
        assert!(!cwd.contains("oxoxdev"), "got {cwd}");
        assert!(cwd.contains("<HOME>"));
