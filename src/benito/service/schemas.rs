...
fn handle_install(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::service::rpc::service_install(&config).await?)
    })
}
...
fn handle_start(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::benito::service::rpc::service_start(&config).await?)
    })
}
...
fn handle_restart(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::service::rpc::service_restart(payload.source, payload.reason).await?,
        )
    })
}
...
fn handle_shutdown(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let payload: ServiceRestartParams =
            serde_json::from_value(Value::Object(params)).map_err(|e| e.to_string())?;
        to_json(
            crate::benito::service::rpc::service_shutdown(payload.source, payload.reason)
                .await?,
        )
    })
}
...
fn handle_stop(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::benito::service::rpc::service_stop(&config).await?)
    })
}
...
fn handle_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::benito::service::rpc::service_status(&config).await?)
    })
}
...
fn handle_uninstall(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::benito::service::rpc::service_uninstall(&config).await?)
    })
}
...
fn handle_daemon_host_get(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::benito::service::rpc::daemon_host_get(&config).await?)
    })
}
...
fn handle_daemon_host_set(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let payload: DaemonHostSetParams =
            serde_json::from_value(Value::Object(params)).map_err(|e| e.to_string())?;
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::benito::service::rpc::daemon_host_set(&config, payload.show_tray).await?)
    })
}

/// Formats the RpcOutcome as an Benito-standard JSON result.
