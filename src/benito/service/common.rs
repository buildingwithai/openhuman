...
pub(crate) const SERVICE_LABEL: &str = "com.Benito.core";
pub(crate) const LEGACY_SERVICE_LABEL: &str = "com.Benito.daemon";
pub(crate) const LEGACY_APP_LABEL: &str = "com.Benito.app";

pub(crate) fn resolve_daemon_executable() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("benito_core_BIN") {
    ...
    for dir in search_dirs.drain(..) {
        ...
        for entry in entries.flatten() {
            ...
            #[cfg(windows)]
            let matches = name.starts_with("Benito-core-")
                || name.eq_ignore_ascii_case("Benito-core.exe");
            #[cfg(not(windows))]
            let matches = name.starts_with("Benito-core-") || name == "Benito-core";
