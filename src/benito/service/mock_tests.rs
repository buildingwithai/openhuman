...
//! manager activated by `BENITO_SERVICE_MOCK`.
//!
//! These tests cover:
...
//!   delegate to mock when `BENITO_SERVICE_MOCK` is set.
...
/// Serialise tests that touch `BENITO_SERVICE_MOCK` and
/// `BENITO_SERVICE_MOCK_STATE_FILE` so they don't race.
...
fn is_enabled_returns_false_when_var_absent() {
    ...
    std::env::remove_var("BENITO_SERVICE_MOCK");
    assert!(!is_enabled());
}
...
fn is_enabled_returns_true_for_truthy_values() {
    ...
    for v in ["1", "true", "yes", "on", "TRUE", "YES", "ON"] {
        std::env::set_var("BENITO_SERVICE_MOCK", v);
        assert!(
            is_enabled(),
            "is_enabled should be true for BENITO_SERVICE_MOCK={v}"
        );
    }
    std::env::remove_var("BENITO_SERVICE_MOCK");
}
...
fn is_enabled_returns_false_for_falsy_values() {
    ...
    for v in ["0", "false", "no", "off", "FALSE"] {
        ...
        assert!(
            !is_enabled(),
            "is_enabled should be false for BENITO_SERVICE_MOCK={v}"
        );
    }
    std::env::remove_var("BENITO_SERVICE_MOCK");
}
...
impl StateFileGuard {
    fn new() -> Self {
        ...
        std::env::set_var("BENITO_SERVICE_MOCK_STATE_FILE", &path);
...
impl Drop for StateFileGuard {
    fn drop(&mut self) {
        std::env::remove_var("BENITO_SERVICE_MOCK_STATE_FILE");
    }
}
...
fn forced_install_failure_returns_error() {
    ...
    std::env::set_var("BENITO_SERVICE_MOCK_STATE_FILE", &state_path);
...
fn core_dispatch_routes_install_to_mock_when_env_set() {
    ...
    // Enable the mock.
    std::env::set_var("BENITO_SERVICE_MOCK", "1");
    let status = crate::benito::service::install(&cfg).expect("core::install via mock");
    std::env::remove_var("BENITO_SERVICE_MOCK");
...
fn core_dispatch_routes_status_to_mock_when_env_set() {
    ...
    std::env::set_var("BENITO_SERVICE_MOCK", "1");
    let status = crate::benito::service::status(&cfg).expect("core::status via mock");
    std::env::remove_var("BENITO_SERVICE_MOCK");
