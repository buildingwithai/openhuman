...
//! Stored next to the main Benito config file.
...
fn config_file_path(BENITO_base: &Path) -> PathBuf {
    BENITO_base.join("daemon_host_config.json")
}

pub async fn load_for_config_dir(BENITO_base: &Path) -> DaemonHostConfig {
    let path = config_file_path(BENITO_base);
