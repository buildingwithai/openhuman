...
pub(crate) fn install(config: &Config) -> Result<()> {
    ...
    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "Benito.service"]));
    Ok(())
}
...
pub(crate) fn start(config: &Config) -> Result<ServiceStatus> {
    if !is_service_enabled_linux()? {
        ...
        let _ =
            run_checked(Command::new("systemctl").args(["--user", "enable", "Benito.service"]));
    } else {
    ...
    let start_result =
        run_checked(Command::new("systemctl").args(["--user", "start", "Benito.service"]));
...
pub(crate) fn stop(_config: &Config) -> Result<()> {
    let _ = run_checked(Command::new("systemctl").args(["--user", "stop", "Benito.service"]));
    Ok(())
}

pub(crate) fn status(config: &Config) -> Result<ServiceStatus> {
    let out =
        run_capture(Command::new("systemctl").args(["--user", "is-active", "Benito.service"]))
    ...
    Ok(ServiceStatus {
        ...
        label: "Benito.service".to_string(),
        details: None,
    })
}
...
fn is_service_enabled_linux() -> Result<bool> {
    let result = Command::new("systemctl")
        .args(["--user", "is-enabled", "Benito.service"])
        .output();
...
mod tests {
    ...
    fn linux_service_file_uses_config_dir() {
        ...
        assert!(path.ends_with(".config/systemd/user/Benito.service"));
    }
}
