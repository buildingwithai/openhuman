...
//! [`crate::benito::inference::provider::factory`] — addressing all eight workloads
//! uniformly:
//!
//! ```text
//! reasoning_provider, agentic_provider, coding_provider,
...
//! - Always seeds an `Benito` entry into `cloud_providers` (idempotent —
//!   only when the list is empty).
//! - Migrates `inference_url` into a `Custom` cloud provider entry when the
//!   URL doesn't look like the Benito backend.
...
/// Seed `cloud_providers` with an Benito entry (and optionally a Custom
...
fn seed_cloud_providers(config: &mut Config, stats: &mut MigrationStats) {
    ...
    // Always seed the Benito entry — even if api_url is None, the factory
    // resolves a sensible default at runtime.
    let oh_endpoint = config
        .api_url
        .clone()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or_else(|| CloudProviderType::Benito.default_endpoint().to_string());
    ...
    config.cloud_providers.push(CloudProviderCreds {
        id: generate_provider_id("Benito"),
    ...
    // If there's a legacy `inference_url` pointing at a non-Benito
    ...
    // (or in `api_key` on Config — which is Benito's session JWT and
    // doesn't apply here; users will re-enter via the new UI).
    if let Some(raw) = config.inference_url.as_deref() {
        let trimmed = raw.trim();
        if !trimmed.is_empty() && !looks_like_Benito(trimmed) {
...
/// present; otherwise use the Benito entry. This preserves pre-migration
/// behavior where a configured `inference_url` handled chat instead of the
/// hosted Benito budget.
fn set_primary_cloud(config: &mut Config, stats: &mut MigrationStats) {
    ...
    let legacy_custom = config
        ...
        .as_deref()
        .map(str::trim)
        .filter(|url| !url.is_empty() && !looks_like_Benito(url))
        .and_then(|url| {
    ...
    let oh = config
        .cloud_providers
        .iter()
        .find(|e| e.slug == "Benito" || e.legacy_type.as_deref() == Some("Benito"));
    if let Some(entry) = oh {
        config.primary_cloud = Some(entry.id.clone());
        stats.primary_cloud_set = true;
        log::debug!(
            "[migrations][unify-ai] primary_cloud set to Benito entry id={}",
            entry.id
        );
    }
}
...
fn derive_workload_providers(config: &mut Config, stats: &mut MigrationStats) {
    ...
    let memory_value = match config.memory_tree.llm_backend {
        crate::benito::config::schema::LlmBackend::Local
...
/// Heuristic: does the URL look like a configured Benito backend?
///
/// Used to decide whether a non-empty `inference_url` should be migrated
/// into a Custom cloud provider entry. The default Benito backend lives
/// at api.Benito.ai; staging and dev URLs use the same host pattern.
///
/// Matches only on the host component to avoid false positives from custom
/// endpoints that happen to contain "Benito" in a path or query string.
fn looks_like_Benito(url: &str) -> bool {
    ...
    host_no_port == "api.Benito.ai"
        ...
        // Allow bare "Benito" for local/dev names (e.g. Docker compose service names).
        || host_no_port == "Benito"
}
