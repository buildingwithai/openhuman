...
use super::*;
use crate::benito::config::schema::{LocalAiConfig, LocalAiUsage};
...
fn make_legacy_config_local_on() -> Config {
    ...
    c.memory_tree.llm_backend = crate::benito::config::schema::LlmBackend::Local;
    c
}

#[test]
fn empty_config_seeds_BENITO_entry() {
    let mut c = Config::default();
    ...
    assert_eq!(c.cloud_providers[0].slug, "Benito");
    assert!(c.cloud_providers[0].id.starts_with("p_BENITO_"));
}

#[test]
fn primary_cloud_defaults_to_BENITO_id() {
    let mut c = Config::default();
    let stats = run(&mut c).expect("migration must succeed");
...
fn legacy_inference_url_becomes_custom_entry() {
    ...
    c.model_routes
        .push(crate::benito::config::schema::ModelRouteConfig {
            hint: "reasoning".into(),
            model: "gpt-4o".into(),
        });
...
#[test]
fn BENITO_inference_url_does_not_seed_custom() {
    let mut c = Config::default();
    c.inference_url = Some("https://api.Benito.ai/v1".into());
    let _ = run(&mut c).expect("migration must succeed");
    // Only the Benito entry should be seeded — no Custom entry.
    assert_eq!(c.cloud_providers.len(), 1);
    assert_eq!(c.cloud_providers[0].slug, "Benito");
}
...
fn chat_workload_providers_left_unset() {
    ...
    // and the factory defaults them to "Benito" at runtime.
