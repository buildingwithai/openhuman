...
use crate::benito::mcp_server::McpToolSpec;
...
pub fn registry_entries() -> Vec<ToolRegistryEntry> {
    ...
    for spec in crate::benito::mcp_server::tool_specs() {
        let entry = mcp_tool_entry(spec);
        insert_registry_entry(&mut entries, entry, "mcp_stdio");
    }

    for schema in crate::benito::tools::all_tools_controller_schemas() {
