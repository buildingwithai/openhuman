...
fn handle_list_registrations(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async { to_json(crate::benito::webhooks::ops::list_registrations().await?) })
}

fn handle_list_logs(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let payload = deserialize_params::<WebhookListLogsParams>(params)?;
        to_json(crate::benito::webhooks::ops::list_logs(payload.limit).await?)
    })
}

fn handle_clear_logs(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async { to_json(crate::benito::webhooks::ops::clear_logs().await?) })
}
...
fn handle_register_echo(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::webhooks::ops::register_echo(
                &payload.tunnel_uuid,
                payload.tunnel_name,
...
fn handle_unregister_echo(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::webhooks::ops::unregister_echo(&payload.tunnel_uuid).await?)
    })
}
...
fn handle_register_agent(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::webhooks::ops::register_agent(
                &payload.tunnel_uuid,
                payload.agent_id,
                payload.tunnel_name,
                payload.backend_tunnel_id,
...
fn handle_trigger_agent(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::webhooks::ops::trigger_agent(
                source,
                &payload.caller_id,
                reason,
                trigger_payload,
...
fn handle_list_tunnels(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = crate::benito::config::rpc::load_config_with_timeout().await?;
        to_json(crate::benito::webhooks::ops::list_tunnels(&config).await?)
    })
}
...
fn handle_create_tunnel(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::webhooks::ops::create_tunnel(
                &config,
                payload.name.trim(),
                payload.description,
            )
...
fn handle_delete_tunnel(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::benito::webhooks::ops::delete_tunnel(&config, payload.id.trim()).await?)
    })
}
...
fn handle_get_tunnel(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = crate::benito::config::rpc::load_config_with_timeout().await?;
        let payload = deserialize_params::<WebhookTunnelIdParams>(params)?;
        to_json(crate::benito::webhooks::ops::get_tunnel(&config, payload.id.trim()).await?)
    })
}
...
fn handle_update_tunnel(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::webhooks::ops::update_tunnel(&config, payload.id.trim(), body)
                .await?,
        )
    })
}
...
fn handle_get_bandwidth(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let config = crate::benito::config::rpc::load_config_with_timeout().await?;
        to_json(crate::benito::webhooks::ops::get_bandwidth(&config).await?)
    })
}
