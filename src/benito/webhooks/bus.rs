...
use crate::benito::socket::global_socket_manager;
use crate::benito::webhooks::WebhookResponseData;
    ...
    async fn handle(&self, event: &DomainEvent) {
        ...
        let (response, resolved_skill_id, response_error) = match registration {
            Some(ref reg) if reg.target_kind == "echo" => {
                ...
                let resp = crate::benito::webhooks::ops::build_echo_response(request);
            ...
            Some(ref reg) if reg.target_kind == "agent" => {
                ...
                if let Err(e) = &decoded {
                    ...
                    let envelope = crate::benito::agent::triage::TriggerEnvelope::from_webhook(
                        &tunnel_uuid,
                        &method,
                        &path,
                        payload,
...
async fn run_agent_trigger(
    envelope: &crate::benito::agent::triage::TriggerEnvelope,
...
mod tests {
    use super::*;
    use crate::benito::webhooks::WebhookRequest;
    use base64::Engine;
    use std::collections::HashMap;
