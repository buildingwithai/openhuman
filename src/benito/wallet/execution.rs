...
mod tests {
    ...
    use super::*;
    use crate::benito::wallet::ops::{setup, WalletSetupParams, WalletSetupSource};
    ...
    async fn setup_wallet(temp: &TempDir) -> Result<(), String> {
        std::env::set_var("BENITO_WORKSPACE", temp.path());
        let config = config_rpc::load_config_with_timeout().await?;
        let encrypted = crate::benito::encryption::rpc::encrypt_secret(
            &config,
    ...
    async fn execute_prepared_broadcasts_native_evm_transaction() {
        ...
        std::env::set_var("BENITO_WALLET_RPC_EVM", format!("http://{addr}"));
