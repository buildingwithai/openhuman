...
use crate::benito::runtime_node::types::{ExecuteToolOutcome, RuntimeToolSummary};
use crate::benito::security::SecurityPolicy;
use crate::benito::tools::{self, Tool, ToolCallOptions, ToolScope};
...
fn build_runtime_tools(config: &Config) -> Result<Vec<Box<dyn Tool>>, String> {
    ...
    let memory: Arc<dyn Memory> = Arc::from(
        crate::benito::memory::create_memory_with_local_ai(
            &config.memory,
...
mod tests {
    ...
    impl Tool for DummyTool {
        ...
        ) -> anyhow::Result<crate::benito::skills::types::ToolResult> {
            Ok(crate::benito::skills::types::ToolResult::success("ok"))
        }
    }
