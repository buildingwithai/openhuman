...
use crate::benito::tree_summarizer::store;
use crate::benito::tree_summarizer::types::{
...
async fn summarize_to_limit(
    ...
    let response = if response.len() > char_limit {
        ...
        let truncated =
            &response[..crate::benito::util::floor_char_boundary(&response, char_limit)];
        truncated.to_string()
    } else {
        response
    };
    ...
    for entry in std::fs::read_dir(dir)? {
        ...
        } else if ft.is_file() && name.ends_with(".md") && name != "summary.md" && name != "root.md"
        {
            ...
            if level == NodeLevel::Hour {
                ...
                let node = crate::benito::tree_summarizer::store::parse_node_markdown_pub(
