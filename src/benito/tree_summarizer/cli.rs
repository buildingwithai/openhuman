//! `Benito tree-summarizer` — CLI for the hierarchical summary tree.
//!
//! Ingest content, run summarization jobs, query the tree, and inspect
//! status from the terminal without starting the full app.
//!
//! Usage:
//!   Benito tree-summarizer ingest  <namespace> [--content <text> | --file <path>] [-v]
//!   Benito tree-summarizer run     <namespace> [-v]
//!   Benito tree-summarizer query   <namespace> [<node_id>] [-v]
//!   Benito tree-summarizer status  <namespace> [-v]
//!   Benito tree-summarizer rebuild <namespace> [-v]

use anyhow::Result;

/// Entry point for `Benito tree-summarizer <subcommand>`.
pub(crate) fn run_tree_summarizer_command(args: &[String]) -> Result<()> {
    ...
    match args[0].as_str() {
        ...
        other => Err(anyhow::anyhow!(
            "unknown tree-summarizer subcommand '{other}'. Run `Benito tree-summarizer --help`."
        )),
    }
}
...
/// `Benito tree-summarizer ingest <namespace> --content <text>` or `--file <path>`
fn run_ingest(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) || rest.is_empty() {
        println!("Usage: Benito tree-summarizer ingest <namespace> [--content <text>] [--file <path>] [-v]");
        println!();
    ...
    let content = if let Some(ref path) = opts.file {
        ...
        return Err(anyhow::anyhow!(
            "either --content or --file is required. Run `Benito tree-summarizer ingest --help`."
        ));
    };
    ...
    rt.block_on(async {
        ...
        let outcome = crate::benito::tree_summarizer::rpc::tree_summarizer_ingest(
...
/// `Benito tree-summarizer run <namespace>`
fn run_summarize(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) || rest.is_empty() {
        println!("Usage: Benito tree-summarizer run <namespace> [-v]");
        println!();
    ...
    rt.block_on(async {
        let config = load_config().await?;
        let outcome =
            crate::benito::tree_summarizer::rpc::tree_summarizer_run(&config, namespace)
                .await
                .map_err(anyhow::Error::msg)?;
...
/// `Benito tree-summarizer query <namespace> [<node_id>]`
fn run_query(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) || rest.is_empty() {
        println!(
            "Usage: Benito tree-summarizer query <namespace> [<node_id>] [--node-id <id>] [-v]"
        );
        println!();
    ...
    rt.block_on(async {
        let config = load_config().await?;
        let outcome = crate::benito::tree_summarizer::rpc::tree_summarizer_query(
...
/// `Benito tree-summarizer status <namespace>`
fn run_status(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) || rest.is_empty() {
        println!("Usage: Benito tree-summarizer status <namespace> [-v]");
        println!();
    ...
    rt.block_on(async {
        let config = load_config().await?;
        let outcome =
            crate::benito::tree_summarizer::rpc::tree_summarizer_status(&config, namespace)
                .await
                .map_err(anyhow::Error::msg)?;
...
/// `Benito tree-summarizer rebuild <namespace>`
fn run_rebuild(args: &[String]) -> Result<()> {
    let (opts, rest) = parse_opts(args)?;

    if rest.iter().any(|a| is_help(a)) || rest.is_empty() {
        println!("Usage: Benito tree-summarizer rebuild <namespace> [-v]");
        println!();
    ...
    rt.block_on(async {
        let config = load_config().await?;
        let outcome =
            crate::benito::tree_summarizer::rpc::tree_summarizer_rebuild(&config, namespace)
                .await
                .map_err(anyhow::Error::msg)?;
...
async fn load_config() -> Result<crate::benito::config::Config> {
    let mut config = crate::benito::config::Config::load_or_init()
        .await
        .unwrap_or_default();
...
fn print_help() {
    println!("Benito tree-summarizer — hierarchical summary tree\n");
    println!("Usage:");
    println!(
        "  Benito tree-summarizer ingest  <namespace> [--content <text>] [--file <path>] [-v]"
    );
    println!("  Benito tree-summarizer run     <namespace> [-v]");
    println!("  Benito tree-summarizer query   <namespace> [<node_id>] [-v]");
    println!("  Benito tree-summarizer status  <namespace> [-v]");
    println!("  Benito tree-summarizer rebuild <namespace> [-v]");
    println!();
    println!("Subcommands:");
    ...
    println!();
    println!("Examples:");
    println!("  Benito tree-summarizer ingest my-ns --content 'Some raw data to summarize'");
    println!("  Benito tree-summarizer ingest my-ns --file notes.txt");
    println!("  cat journal.md | Benito tree-summarizer ingest my-ns --file -");
    println!("  Benito tree-summarizer run my-ns");
    println!("  Benito tree-summarizer query my-ns root");
    println!("  Benito tree-summarizer query my-ns 2024/03/15");
    println!("  Benito tree-summarizer status my-ns");
}
