...
use crate::benito::util::provenance_tag;
...
use crate::benito::learning::transcript_ingest::CONVERSATION_MEMORY_NAMESPACE;
use crate::benito::memory::conversations::ConversationStore;
...
pub async fn collect_recall_citations(
    ...
    let entries = memory
        .recall(
            user_message,
            limit.max(1),
            crate::benito::memory::RecallOpts::default(),
        )
        .await?;
    ...
    let citations = entries
        ...
        .map(|entry| {
            let snippet = if entry.content.chars().count() > 280 {
                crate::benito::util::truncate_with_ellipsis(&entry.content, 280)
            } else {
                entry.content
            };
            MemoryCitation {
    ...
    async fn load_context(
        ...
        let prior_entries = memory
            .recall(
                ...
                crate::benito::memory::RecallOpts {
        ...
        let cross_hits: Vec<(String, String)> = if let Some(workspace_dir) = &self.workspace_dir {
            ...
            let cross_session_opts = crate::benito::memory::RecallOpts {
        ...
        for (sid, content) in cross_hits {
            let snippet = if content.chars().count() > CROSS_CHAT_SNIPPET_CHARS {
                crate::benito::util::truncate_with_ellipsis(&content, CROSS_CHAT_SNIPPET_CHARS)
            } else {
                content
            };
...
mod tests {
    ...
    impl Memory for MockMemory {
        ...
        async fn recall(
            ...
            _query: &str,
            _limit: usize,
            opts: crate::benito::memory::RecallOpts<'_>,
