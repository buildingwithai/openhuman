...
impl SystemPromptBuilder {
    ...
    /// the `omit_*` flags from the [`crate::benito::agent::harness::definition::AgentDefinition`]:
    ...
    pub fn from_dynamic(
        builder: crate::benito::agent::harness::definition::PromptBuilder,
    ) -> Self {
        Self {
    ...
    /// [`crate::benito::memory::ToolMemoryStore::rules_for_prompt`]
    ...
    pub fn with_tool_memory_rules(
        mut self,
        rules: Vec<crate::benito::memory::ToolMemoryRule>,
    ) -> Self {
        if rules.is_empty() {
            return self;
        }
        ...
        let section: Box<dyn PromptSection> =
            Box::new(crate::benito::memory::ToolMemoryRulesSection::new(rules));
        let tools_idx = self
            .sections
            .iter()
    ...
    pub fn with_reflection_context(
        mut self,
        chunks: Vec<crate::benito::subconscious::SourceChunk>,
    ) -> Self {
        if chunks.is_empty() {
            return self;
        }
...
impl ReflectionMemoryContextSection {
    pub fn new(chunks: Vec<crate::benito::subconscious::SourceChunk>) -> Self {
        Self { chunks }
    }
}
...
impl PromptSection for ReflectionMemoryContextSection {
    ...
    fn build(&self, _ctx: &PromptContext<'_>) -> Result<String> {
        ...
        // it'll quote.
        let usable: Vec<&crate::benito::subconscious::SourceChunk> = self
            .chunks
            .iter()
        ...
        out.push_str(
            "This thread was spawned from a subconscious reflection. The chunks below \
             are what Benito was looking at when it surfaced the observation — \
...
/// [`crate::benito::agent::harness::definition::AgentDefinition`]'s
...
/// Section that defers to a [`crate::benito::agent::harness::definition::PromptBuilder`]
...
impl DynamicPromptSection {
    pub fn new(builder: crate::benito::agent::harness::definition::PromptBuilder) -> Self {
        Self { builder }
    }
}
...
fn render_pformat_signature_for_box_tool(tool: &dyn crate::benito::tools::Tool) -> String {
...
fn default_workspace_file_content(filename: &str) -> &'static str {
    // The bundled identity files live at `src/Benito/agent/prompts/`
    // (owned by the `agent/` tree because they describe agent identity).
    // This module is under `src/Benito/context/`, so the relative path
