...
impl Tool for TestTool {
    ...
    async fn execute(
        ...
        Ok(crate::benito::tools::ToolResult::success("ok"))
    }
}
...
fn identity_section_creates_missing_workspace_files() {
    let workspace =
        std::env::temp_dir().join(format!("BENITO_prompt_create_{}", uuid::Uuid::new_v4()));
    ...
    assert!(
        soul.starts_with("# Benito"),
        "SOUL.md should be seeded from src/Benito/agent/prompts/SOUL.md"
    );
...
fn render_subagent_system_prompt_honors_identity_safety_and_skills_flags() {
    let workspace =
        std::env::temp_dir().join(format!("BENITO_prompt_opts_{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&workspace).unwrap();
...
fn render_subagent_system_prompt_skips_profile_md_when_include_profile_false() {
    ...
    let workspace = std::env::temp_dir().join(format!(
        "BENITO_prompt_profile_opt_out_{}",
        uuid::Uuid::new_v4()
    ));
    std::fs::create_dir_all(&workspace).unwrap();
    std::fs::write(
...
fn render_subagent_system_prompt_injects_profile_md_when_identity_included() {
    ...
    let workspace = std::env::temp_dir().join(format!(
        "BENITO_prompt_profile_with_identity_{}",
        uuid::Uuid::new_v4()
    ));
    std::fs::create_dir_all(&workspace).unwrap();
    std::fs::write(workspace.join("SOUL.md"), "# Soul\nctx").unwrap();
...
fn render_subagent_system_prompt_silently_skips_missing_profile_md() {
    ...
    // focused on tools.
    let workspace = std::env::temp_dir().join(format!(
        "BENITO_prompt_profile_missing_{}",
        uuid::Uuid::new_v4()
    ));
    std::fs::create_dir_all(&workspace).unwrap();
...
fn welcome_agent_definition_flags_still_load_profile_md() {
    ...
    // Match `src/Benito/agent/agents/welcome/agent.toml` exactly.
...
fn for_subagent_builder_injects_user_files_even_when_identity_omitted() {
    ...
    // contains both files.
    let workspace = std::env::temp_dir().join(format!(
        "BENITO_prompt_for_subagent_user_files_{}",
        uuid::Uuid::new_v4()
    ));
    std::fs::create_dir_all(&workspace).unwrap();
    std::fs::write(
