...
- `src/Benito/providers/` — `ChatMessage`, `ChatResponse` send/receive against LLMs.
- `src/Benito/tools/` — `Tool` / `ToolSpec` execution surface invoked from the tool loop.
- `src/Benito/memory/` — episodic indexing + memory-loader context injection.
- `src/Benito/context/` — prompt sections, tool-call format selection.
- `src/Benito/local_ai/` — `agent_chat` / `agent_chat_simple` execution backend.
- `src/Benito/config/` — runtime config load via `config::rpc::load_config_with_timeout`.
- `src/core/event_bus/` — emits `DomainEvent::Agent(*)` and `Trigger*` events; subscribers in `agent/bus.rs`.

## Called by

- `src/Benito/channels/runtime/dispatch.rs` and `channels/providers/web.rs` — drive chat turns from inbound channel messages.
- `src/Benito/cron/scheduler.rs` — fire scheduled triggers through `triage::run_triage` + `apply_decision`.
- `src/Benito/webhooks/ops.rs` — webhook ingestion routes through triage.
- `src/Benito/composio/bus.rs` — Composio trigger envelopes go through `agent::triage`.
- `src/Benito/notifications/rpc.rs` — surfaces agent runs to the UI.
- `src/Benito/learning/{reflection,tool_tracker,user_profile}.rs` — read transcripts + tool outcomes.
- `src/Benito/tools/impl/agent/{dispatch,spawn_subagent}.rs` — `spawn_subagent` tool delegates here.
