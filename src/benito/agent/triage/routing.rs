...
/// The concrete provider + metadata that [`crate::benito::agent::triage::evaluator::run_triage`]
/// should use for this particular triage turn.
pub struct ResolvedProvider {
    /// Ready-to-use provider, already constructed.
    pub provider: Arc<dyn Provider>,
    /// Provider name token — always `"Benito"` (remote backend).
...
/// overridable via `BENITO_LOCAL_INFERENCE_URL`). It mirrors the
...
pub fn build_local_provider_with_config(config: &Config) -> Option<ResolvedProvider> {
    ...
    let (label, base) = if use_openai_compat {
        ...
        let ollama_base = crate::benito::inference::local::ollama_base_url();
