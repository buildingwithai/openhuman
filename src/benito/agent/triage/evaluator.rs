...
//! `src/Benito/agent/harness/tool_loop.rs` handles an empty registry
...
use crate::benito::agent::bus::{AgentTurnRequest, AgentTurnResponse, AGENT_RUN_TURN_METHOD};
...
use crate::benito::config::MultimodalConfig;
use crate::benito::inference::provider::reliable::{
    is_rate_limited, is_upstream_unhealthy, parse_retry_after_ms,
};
use crate::benito::inference::provider::ChatMessage;
use crate::benito::scheduler_gate::LlmPermit;
...
pub async fn run_triage(envelope: &TriggerEnvelope) -> anyhow::Result<TriageOutcome> {
    ...
    let outcome = run_triage_with_arms_inner(cloud, local, envelope, || {
        crate::benito::scheduler_gate::wait_for_capacity()
    })
    .await;
    ...
    Fut: Future<Output = Option<LlmPermit>>,
{
    ...
    // flagged the content (Benito-TAURI-X). The guard runs before
...
enum ArmError {
    ...
    /// its job (Benito-TAURI-X regression: an adversarial Gmail
...
///
/// Canonical sources:
/// - `src/Benito/agent/bus.rs` — `Blocked` / `ReviewBlocked` arms of the
///   `enforce_prompt_input` decision (the path the triage evaluator hits via
///   `agent.run_turn`).
/// - `src/Benito/agent/harness/session/runtime.rs` — same strings in the
///   tool-call loop, kept identical so this classifier covers both.
/// - `src/Benito/local_ai/ops.rs` — user-facing variants with the
...
///
/// The vocabulary matches the Benito backend's error copy and common
...
fn truncate_payload(payload: &serde_json::Value, max_bytes: usize) -> String {
    ...
    let end = crate::benito::util::floor_char_boundary(&pretty, max_bytes);
