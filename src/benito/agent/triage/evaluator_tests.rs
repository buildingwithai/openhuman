use super::*;
use crate::benito::agent::agents::BUILTINS;
use crate::benito::agent::bus::{mock_agent_run_turn, AgentTurnResponse};
...
fn classify_string_recognises_budget_exceeded_as_budget_exhausted() {
    // Matches the real payload that fired Benito-TAURI-X in Sentry.
    let err = classify_error(
        "Benito API error (400 Bad Request): {\"success\":false,\
...
fn classify_string_recognises_prompt_guard_rejection_as_safety_flagged() {
    // Benito-TAURI-X regression: the exact phrase our prompt-injection
...
async fn cloud_budget_exhausted_skips_retry_and_falls_to_local() {
    // Regression for Benito-TAURI-X: when the cloud arm returns
    ...
    let _guard = mock_agent_run_turn(move |req| {
        ...
        async move {
            ...
            if n == 0 {
                ...
                Err("Benito API error (400 Bad Request): \
...
async fn cloud_budget_exhausted_on_retry_falls_through_to_local() {
    // Variant of Benito-TAURI-X: cloud arm trips a transient first
...
async fn cloud_budget_exhausted_without_local_returns_deferred_not_err() {
    ...
    let _guard = mock_agent_run_turn(move |_req| {
        ...
        async move {
            counter.fetch_add(1, Ordering::SeqCst);
            Err(
                "Benito API error (400 Bad Request): Budget exceeded — add credits to continue"
                    .to_string(),
            )
        }
    })
...
async fn cloud_safety_flagged_then_local_flagged_defers_not_errs() {
    // Regression for Benito-TAURI-X (regressed): the prompt-injection
