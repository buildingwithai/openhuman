...
use crate::benito::agent::harness::subagent_runner::{self, SubagentRunOptions};
...
async fn dispatch_target_agent(agent_id: &str, prompt: &str) -> anyhow::Result<String> {
    ...
    let integrations = crate::benito::composio::fetch_connected_integrations(&config).await;
