...
use crate::benito::agent::harness::session::Agent;
use crate::benito::composio::ComposioActionTool;
...
async fn render_integrations_agent(config: &Config, toolkit: &str) -> Result<DumpedPrompt> {
    ...
    match &client_kind {
        ComposioClientKind::Backend(composio_client) => {
            match crate::benito::composio::fetch_toolkit_actions(
                composio_client,
                &integration.toolkit,
            )
            .await
    ...
    let base_tools: Vec<Box<dyn Tool>> = match &definition_snapshot.tools {
        crate::benito::agent::harness::definition::ToolScope::Named(names) => {
        ...
        crate::benito::agent::harness::definition::ToolScope::Wildcard => agent
            .tools()
            .iter()
            .map(|t| clone_tool_as_prompt_proxy(t.as_ref()))
            .collect(),
    ...
    if !rendered_tools.is_empty() {
        ...
        text.push_str("\n\n");
        text.push_str(
            &crate::benito::agent::harness::subagent_runner::build_text_mode_tool_instructions(
                &tool_specs,
            ),
        );
    }
...
impl Tool for PromptProxyTool {
    ...
    fn permission_level(&self) -> crate::benito::tools::PermissionLevel {
        crate::benito::tools::PermissionLevel::None
    }
    async fn execute(
        &self,
        _args: serde_json::Value,
    ) -> anyhow::Result<crate::benito::tools::ToolResult> {
        Err(anyhow!(
