...
use crate::benito::agent::dispatcher::{ParsedToolCall, ToolExecutionResult};
use crate::benito::agent::harness;
use crate::benito::agent::hooks::{self, ToolCallRecord, TurnContext};
use crate::benito::agent::memory_loader::collect_recall_citations;
use crate::benito::agent::progress::AgentProgress;
use crate::benito::agent::tool_policy::{ToolPolicyDecision, ToolPolicyRequest};
use crate::benito::agent_experience::{
    prepend_experience_block, render_experience_hits, AgentExperienceStore, ExperienceQuery,
};
use crate::benito::context::prompt::{LearnedContextData, PromptContext, PromptTool};
use crate::benito::context::{ReductionOutcome, ARCHIVIST_EXTRACTION_PROMPT};
use crate::benito::inference::model_context::context_window_for_model;
use crate::benito::inference::provider::{
    ChatMessage, ChatRequest, ConversationMessage, ProviderDelta,
};
use crate::benito::memory::MemoryCategory;
use crate::benito::tools::traits::ToolCallOptions;
use crate::benito::tools::Tool;
use crate::benito::util::truncate_with_ellipsis;

use crate::benito::agent::harness::token_budget::{
...
impl Agent {
    ...
    pub async fn turn(&mut self, user_message: &str) -> Result<String> {
        ...
        if self.history.is_empty() {
            ...
            self.last_seen_integrations_hash =
                crate::benito::composio::connected_set_hash(&self.connected_integrations);
        } else {
            ...
            // [`crate::benito::composio::INTEGRATIONS_CACHE`], kept
            ...
            // [`crate::benito::composio::cached_active_integrations`]
            ...
            if let Ok(cfg) = crate::benito::config::Config::load_or_init().await {
                if let Some(cache_view) =
                    crate::benito::composio::cached_active_integrations(&cfg)
                {
                    let new_hash = crate::benito::composio::connected_set_hash(&cache_view);
        ...
        let context = if crate::benito::agent::tree_loader::should_prefetch(
            self.last_tree_prefetch_at,
            now,
            crate::benito::agent::tree_loader::REFRESH_INTERVAL,
        ) {
            match crate::benito::config::rpc::load_config_with_timeout().await {
                Ok(cfg) => {
                    match crate::benito::agent::tree_loader::TreeContextLoader::load(&cfg).await
                    {
                        Ok(tree_ctx) => {
        ...
        let context = if is_first_turn_for_stm {
            ...
            let stm_enabled = crate::benito::config::rpc::load_config_with_timeout()
                .await
            ...
            if stm_enabled {
                if let Some(conn) = self.memory.sqlite_conn() {
                    use crate::benito::memory::stm_recall::recall::{stm_recall, StmRecallOpts};
        ...
        // docs on [`crate::benito::skills::inject`] for the matching
        // heuristic and size cap rationale.
        let enriched = {
            use crate::benito::skills::inject;
        ...
        let turn_body = async {
            ...
            // a deterministic agent-state outcome, not a bug (Benito-
            ...
            Err(anyhow::Error::new(
                crate::benito::agent::error::AgentError::MaxIterationsExceeded {
    ...
    pub(super) async fn execute_tool_call(
        ...
        let (result, budget_outcome) =
            crate::benito::context::apply_tool_result_budget(raw_result, budget_bytes);
    ...
    pub(super) async fn fetch_learned_context(&self) -> LearnedContextData {
        ...
        let reflection_entries = self
            .memory
            .list(
                Some(crate::benito::learning::reflection::REFLECTIONS_NAMESPACE),
                Some(&MemoryCategory::Custom(
                    crate::benito::learning::reflection::REFLECTIONS_NAMESPACE.into(),
                )),
                None,
            )
            .await
    ...
    ///
    /// Delegates to the shared [`crate::benito::composio::fetch_connected_integrations`]
    ...
    /// [`crate::benito::composio::client::create_composio_client`]
    ...
    pub async fn fetch_connected_integrations(&mut self) {
        ...
        self.connected_integrations =
            crate::benito::composio::fetch_connected_integrations(&config).await;
    }
    ...
    pub fn refresh_delegation_tools(&mut self) -> bool {
        ...
        use crate::benito::tools::orchestrator_tools::collect_orchestrator_tools;
        ...
        let synthed_specs: Vec<crate::benito::tools::ToolSpec> =
        ...
        // tool names where Benito's backend silently accepts.
        let visible_specs: Vec<crate::benito::tools::ToolSpec> =
            if self.visible_tool_names.is_empty() {
    ...
    pub fn build_system_prompt(&self, learned: LearnedContextData) -> Result<String> {
        ...
        let ctx = PromptContext {
            ...
            connected_identities_md: crate::benito::agent::prompts::render_connected_identities(
            ),
            include_profile: !self.omit_profile,
            include_memory_md: !self.omit_memory_md,
            curated_snapshot: None,
            user_identity: crate::benito::app_state::peek_cached_current_user_identity(),
        };
    ...
    pub(super) fn persist_session_transcript(
        ...
        let meta = transcript::TranscriptMeta {
            ...
            cached_input_tokens,
            charged_amount_usd,
            thread_id: crate::benito::inference::provider::thread_context::current_thread_id(),
        };
    ...
    /// heuristic [`crate::benito::learning::transcript_ingest`]
    ...
    pub(super) fn spawn_transcript_ingestion(&self) {
        ...
        tokio::spawn(async move {
            match crate::benito::learning::transcript_ingest::ingest_transcript_path(
                memory.as_ref(),
                &path,
            )
            .await
...
/// Wrapper around
/// [`crate::benito::tree_summarizer::store::collect_root_summaries_with_caps`]
...
/// preset by [`crate::benito::config::schema::agent::AgentConfig::resolved_memory_limits`].
