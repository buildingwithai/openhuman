...
use crate::benito::agent::dispatcher::XmlToolDispatcher;
use crate::benito::agent::hooks::{PostTurnHook, TurnContext};
use crate::benito::agent::memory_loader::MemoryLoader;
use crate::benito::agent::tool_policy::{ToolPolicy, ToolPolicyDecision, ToolPolicyRequest};
use crate::benito::inference::provider::{ChatRequest, ChatResponse, Provider};
use crate::benito::memory::Memory;
use crate::benito::tools::Tool;
use crate::benito::tools::ToolResult;
...
fn make_agent(visible_tool_names: Option<HashSet<String>>) -> Agent {
    ...
    let mut builder = Agent::builder()
        ...
        .config(crate::benito::config::AgentConfig {
            max_history_messages: 3,
            ..crate::benito::config::AgentConfig::default()
        });
...
fn make_agent_with_builder(
    ...
    config: crate::benito::config::AgentConfig,
    context_config: crate::benito::config::ContextConfig,
) -> Agent {
    let workspace = tempfile::TempDir::new().expect("temp workspace");
    let workspace_path = workspace.path().to_path_buf();
    std::mem::forget(workspace);
...
fn build_parent_context_and_sanitize_helpers_cover_snapshot_paths() {
    ...
    agent.skills = vec![crate::benito::skills::Skill {
        name: "demo".into(),
        ..Default::default()
    }];
...
async fn turn_runs_full_tool_cycle_with_context_and_hooks() {
    ...
    let mut agent = make_agent_with_builder(
        ...
        hooks,
        crate::benito::config::AgentConfig {
...
async fn execute_tool_call_applies_inline_result_budget() {
    ...
    let agent = make_agent_with_builder(
        ...
        vec![],
        crate::benito::config::AgentConfig::default(),
        crate::benito::config::ContextConfig {
...
fn make_real_memory(workspace: &std::path::Path) -> Arc<dyn Memory> {
    use crate::benito::embeddings::NoopEmbedding;
    use crate::benito::memory::UnifiedMemory;
...
async fn fetch_learned_context_returns_empty_when_both_flags_off() {
    ...
    mem.store(
        ...
        crate::benito::memory::MemoryCategory::Core,
        None,
    )
    .await
    .unwrap();
