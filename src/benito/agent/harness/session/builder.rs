...
use crate::benito::agent::dispatcher::{
    NativeToolDispatcher, PFormatToolDispatcher, ToolDispatcher, XmlToolDispatcher,
};
use crate::benito::agent::harness::definition::{
    AgentDefinitionRegistry, PromptSource, ToolScope,
};
use crate::benito::agent::host_runtime;
use crate::benito::agent::memory_loader::{DefaultMemoryLoader, MemoryLoader};
use crate::benito::config::{Config, ContextConfig};
use crate::benito::context::prompt::SystemPromptBuilder;
use crate::benito::context::{ContextManager, ProviderSummarizer, SegmentRecapSummarizer};
use crate::benito::inference::provider::{self, Provider};
use crate::benito::memory::{self, Memory};
use crate::benito::security::SecurityPolicy;
use crate::benito::tools::{self, Tool, ToolSpec};
use anyhow::Result;
use std::sync::Arc;
...
/// request that lists two tools with the same name — Benito's own
...
impl AgentBuilder {
    ...
    pub fn config(mut self, config: crate::benito::config::AgentConfig) -> Self {
    ...
    pub fn skills(mut self, skills: Vec<crate::benito::skills::Skill>) -> Self {
    ...
    pub fn post_turn_hooks(
        mut self,
        hooks: Vec<Arc<dyn crate::benito::agent::hooks::PostTurnHook>>,
    ) -> Self {
    ...
    /// [`crate::benito::agent::harness::payload_summarizer::PayloadSummarizer::maybe_summarize`]
    ...
    pub fn payload_summarizer(
        mut self,
        summarizer: Arc<
            dyn crate::benito::agent::harness::payload_summarizer::PayloadSummarizer,
        >,
    ) -> Self {
    ...
    pub fn tool_policy(
        mut self,
        policy: Arc<dyn crate::benito::agent::tool_policy::ToolPolicy>,
    ) -> Self {
    ...
    pub fn archivist_hook(
        mut self,
        hook: Option<Arc<crate::benito::agent::harness::archivist::ArchivistHook>>,
    ) -> Self {
    ...
    pub fn build(self) -> Result<Agent> {
        ...
        // the same name — Benito's own backend and OpenAI silently
        ...
        let model_name = self
            .model_name
            .unwrap_or_else(|| crate::benito::config::DEFAULT_MODEL.into());
        ...
        let inner_summarizer: Arc<dyn crate::benito::context::Summarizer> =
            Arc::new(ProviderSummarizer::new(provider.clone()));
        let session_id_for_recap = self
            .event_session_id
            .clone()
            .unwrap_or_else(|| "standalone".to_string());
        let summarizer: Arc<dyn crate::benito::context::Summarizer> =
        ...
        Ok(Agent {
            ...
            tool_policy: self.tool_policy.unwrap_or_else(|| {
                Arc::new(crate::benito::agent::tool_policy::AllowAllToolPolicy)
            }),
...
impl Agent {
    ...
    pub fn from_config_for_agent(config: &Config, agent_id: &str) -> Result<Self> {
        ...
        let target_def: Option<crate::benito::agent::harness::definition::AgentDefinition> =
    ...
    pub fn from_config_for_agent_with_reflection_chunks(
        config: &Config,
        agent_id: &str,
        reflection_chunks: Vec<crate::benito::subconscious::SourceChunk>,
    ) -> Result<Self> {
    ...
    pub fn from_config_for_agent_with_profile(
        config: &Config,
        agent_id: &str,
        reflection_chunks: Option<Vec<crate::benito::subconscious::SourceChunk>>,
    ...
    fn build_session_agent_inner(
        config: &Config,
        agent_id: &str,
        target_def: Option<&crate::benito::agent::harness::definition::AgentDefinition>,
        reflection_chunks: Option<Vec<crate::benito::subconscious::SourceChunk>>,
        profile_prompt_suffix: Option<String>,
    ) -> Result<Self> {
        ...
        if agent_id != "welcome" {
            tools.retain(|t| {
                !crate::benito::agent::harness::subagent_runner::is_welcome_only_tool(t.name())
            });
        }

        // Filter tools by user preference stored in app state.
        {
            use crate::benito::app_state::load_stored_app_state;
            match load_stored_app_state(config) {
                Ok(stored) => {
                    if let Some(ref tasks) = stored.onboarding_tasks {
                        if !tasks.enabled_tools.is_empty() {
                            crate::benito::tools::filter_tools_by_user_preference(
                                &mut tools,
                                &tasks.enabled_tools,
                            );
                        }
        ...
        // resolves to the primary cloud (Benito by default), so the
        ...
        let _ = provider::ProviderRuntimeOptions {
            ...
            BENITO_dir: config.config_path.parent().map(std::path::PathBuf::from),
        ...
        let (provider, mut model_name): (Box<dyn Provider>, String) =
            crate::benito::inference::provider::create_chat_provider(provider_role, config)?;
            ...
            Some(def) => match &def.system_prompt {
                ...
                PromptSource::File { path } => {
                    ...
                    let body_text = if workspace_path.is_file() {
                        match crate::benito::security::validate_path_within_root(&workspace_path, &prompt_root) {
                            Ok(resolved) => {
        ...
        if config.learning.enabled {
            ...
            prompt_builder = prompt_builder
                .insert_section_before(
                    "user_memory",
                    Box::new(crate::benito::context::prompt::UserReflectionsSection),
                )
                .add_section(Box::new(
                    crate::benito::learning::LearnedContextSection::new(memory.clone()),
                ))
                .add_section(Box::new(
                    crate::benito::learning::UserProfileSection::new(memory.clone()),
                ));
            ...
            .filter(|s| !s.is_empty())
        {
            ...
            prompt_builder = prompt_builder.add_section(Box::new(
                crate::benito::agent::profiles::AgentProfilePromptSection::new(suffix),
            ));
        }

        // Build post-turn hooks when learning is enabled
        let mut post_turn_hooks: Vec<Arc<dyn crate::benito::agent::hooks::PostTurnHook>> =
            Vec::new();
        if config.learning.enabled {
            if config.learning.reflection_enabled {
                ...
                let reflection_provider: Option<
                    Arc<dyn crate::benito::inference::provider::Provider>,
                ...
                post_turn_hooks.push(Arc::new(crate::benito::learning::ReflectionHook::new(
            ...
            if config.learning.user_profile_enabled {
                post_turn_hooks.push(Arc::new(crate::benito::learning::UserProfileHook::new(
                    config.learning.clone(),
                    memory.clone(),
                )));
            ...
            if config.learning.tool_tracking_enabled {
                post_turn_hooks.push(Arc::new(crate::benito::learning::ToolTrackerHook::new(
                    config.learning.clone(),
                    memory.clone(),
                )));
            ...
            if config.learning.tool_memory_capture_enabled {
                post_turn_hooks.push(Arc::new(
                    crate::benito::memory::ToolMemoryCaptureHook::new(memory.clone(), true),
                ));
            ...
            if config.learning.tool_memory_capture_enabled {
                post_turn_hooks.push(Arc::new(
                    crate::benito::agent_experience::AgentExperienceCaptureHook::new(
                        memory.clone(),
                        true,
                    ),
                ));
        ...
        let archivist_hook_arc: Option<
            Arc<crate::benito::agent::harness::archivist::ArchivistHook>,
        > = if config.learning.episodic_capture_enabled {
            match memory.sqlite_conn() {
                Some(conn) => {
                    let hook = Arc::new(
                        crate::benito::agent::harness::archivist::ArchivistHook::new(conn, true)
                            .with_config(config.clone()),
                    );
                    post_turn_hooks
                        .push(Arc::clone(&hook)
                            as Arc<dyn crate::benito::agent::hooks::PostTurnHook>);
                    log::info!(
        ...
        let (delegation_tools, filter_from_scope): (
            ...
            target_def,
            crate::benito::agent::harness::definition::AgentDefinitionRegistry::global(),
        ) {
            (Some(def), Some(reg)) => {
        ...
        if config.learning.enabled {
            ...
            if has_retrieval {
                prompt_builder = prompt_builder
                    .add_section(Box::new(crate::benito::learning::MemoryAccessSection));
        ...
        let pformat_registry = crate::benito::agent::pformat::build_registry(&tools);
        ...
        {
            match crate::benito::agent::harness::definition::AgentDefinitionRegistry::global() {
                Some(reg) => match reg.get("summarizer") {
                    Some(summarizer_def) => {
                        ...
                        Some(std::sync::Arc::new(
                            crate::benito::agent::harness::payload_summarizer::SubagentPayloadSummarizer::new(
        ...
        let mut builder = Agent::builder()
            ...
            .skills(crate::benito::skills::load_skills(&config.workspace_dir))
...
fn prefetch_tool_memory_rules_blocking(
    ...
    tool_names: &[String],
) -> Vec<crate::benito::memory::ToolMemoryRule> {
    ...
    tokio::task::block_in_place(|| {
        handle.block_on(async move {
            let store = crate::benito::memory::ToolMemoryStore::new(memory);
...
mod dedup_tests {
    ...
    use crate::benito::tools::ToolSpec;
    use serde_json::json;
