...
use crate::benito::agent::dispatcher::{NativeToolDispatcher, XmlToolDispatcher};
use crate::benito::inference::provider::{ChatRequest, ConversationMessage, Provider};
...
struct MockProvider {
    responses: Mutex<Vec<crate::benito::inference::provider::ChatResponse>>,
}
...
impl Provider for MockProvider {
    ...
    async fn chat(
        ...
        _model: &str,
        _temperature: f64,
    ) -> Result<crate::benito::inference::provider::ChatResponse> {
        let mut guard = self.responses.lock();
        if guard.is_empty() {
            return Ok(crate::benito::inference::provider::ChatResponse {
                text: Some("done".into()),
                tool_calls: vec![],
                usage: None,
            });
...
impl Tool for MockTool {
    ...
    ) -> Result<crate::benito::tools::ToolResult> {
        Ok(crate::benito::tools::ToolResult::success("tool-out"))
    }
}
...
fn build_minimal_agent_with_definition_name(definition_name: Option<&str>) -> Agent {
    ...
    let memory_cfg = crate::benito::config::MemoryConfig {
        backend: "none".into(),
        ..crate::benito::config::MemoryConfig::default()
    };
    let mem: Arc<dyn Memory> =
        Arc::from(crate::benito::memory::create_memory(&memory_cfg, &workspace_path).unwrap());
...
async fn turn_without_tools_returns_text() {
    ...
    let provider = Box::new(MockProvider {
        responses: Mutex::new(vec![crate::benito::inference::provider::ChatResponse {
...
async fn turn_with_native_dispatcher_handles_tool_results_variant() {
    ...
    let provider = Box::new(MockProvider {
        responses: Mutex::new(vec![
            crate::benito::inference::provider::ChatResponse {
                text: Some(String::new()),
                tool_calls: vec![crate::benito::inference::provider::ToolCall {
                    id: "tc1".into(),
                    name: "echo".into(),
                    arguments: "{}".into(),
                }],
...
async fn turn_dispatches_spawn_subagent_through_full_path() {
    ...
    use crate::benito::tools::SpawnSubagentTool;
...
fn seed_resume_from_messages_is_noop_on_warm_agent() {
    ...
    agent.cached_transcript_messages = Some(vec![
        crate::benito::inference::provider::ChatMessage::system("warm prefix"),
        crate::benito::inference::provider::ChatMessage::user("hi"),
    ]);
    agent
...
fn bound_cached_transcript_messages_without_system_prefix_keeps_tail() {
    ...
    let messages = vec![
        crate::benito::inference::provider::ChatMessage::user("u1"),
        crate::benito::inference::provider::ChatMessage::assistant("a1"),
        crate::benito::inference::provider::ChatMessage::user("u2"),
        crate::benito::inference::provider::ChatMessage::assistant("a2"),
        crate::benito::inference::provider::ChatMessage::user("u3"),
    ];
