...
fn sanitizers_and_tool_call_helpers_cover_fallback_paths() {
    ...
    let calls = vec![
        crate::benito::agent::dispatcher::ParsedToolCall {
            name: "a".into(),
    ...
    let response = crate::benito::inference::provider::ChatResponse {
...
async fn run_single_preserves_typed_max_iterations_error_for_sentry_skip() {
    // Benito-TAURI-99 regression guard: when the agent hits its tool
...
fn helper_paths_cover_no_overlap_native_calls_and_truncation() {
    ...
    let native_calls = vec![crate::benito::inference::provider::ToolCall {
        id: "native-1".into(),
        name: "echo".into(),
        arguments: "{}".into(),
    }];
