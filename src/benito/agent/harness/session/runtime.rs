...
use crate::benito::agent::dispatcher::ParsedToolCall;
use crate::benito::agent::error::AgentError;
use crate::benito::inference::provider::{self, ConversationMessage, Provider, ToolCall};
...
impl Agent {
    ...
    pub fn skills(&self) -> &[crate::benito::skills::Skill] {
        &self.skills
    }
    ...
    pub fn connected_integrations(
        &self,
    ) -> &[crate::benito::context::prompt::ConnectedIntegration] {
    ...
    pub fn set_connected_integrations(
        &mut self,
        integrations: Vec<crate::benito::context::prompt::ConnectedIntegration>,
    ) {
        self.connected_integrations = integrations;
    }

    /// The agent's runtime config snapshot.
    pub fn agent_config(&self) -> &crate::benito::config::AgentConfig {
        &self.config
    }
    ...
    pub fn set_on_progress(
        &mut self,
        tx: Option<tokio::sync::mpsc::Sender<crate::benito::agent::progress::AgentProgress>>,
    ) {
        self.on_progress = tx;
    }
    ...
    pub fn seed_resume_from_messages(
        ...
        let learned = crate::benito::agent::prompts::LearnedContextData::default();
        let system_prompt = self.build_system_prompt(learned)?;

        let mut cached: Vec<crate::benito::inference::provider::ChatMessage> =
            Vec::with_capacity(prior.len() + 1);
        cached.push(crate::benito::inference::provider::ChatMessage::system(
            system_prompt,
        ));
        for (role, content) in prior {
            let chat = match role.as_str() {
                "user" => crate::benito::inference::provider::ChatMessage::user(content),
                "agent" | "assistant" => {
                    crate::benito::inference::provider::ChatMessage::assistant(content)
                }
                ...
                _ => crate::benito::inference::provider::ChatMessage::user(content),
            };
            cached.push(chat);
        }
    ...
    pub fn take_last_turn_citations(
        &mut self,
    ) -> Vec<crate::benito::agent::memory_loader::MemoryCitation> {
    ...
    pub(super) fn persisted_tool_calls_for_history(
        response: &crate::benito::inference::provider::ChatResponse,
    ...
    pub async fn run_single(&mut self, message: &str) -> Result<String> {
        ...
        match self.turn(message).await {
            ...
            Err(err) => {
                ...
                // `log::info!` (Benito-TAURI-99 / -98).
                //
                // Other agent errors go through `report_error_or_expected`
                // so Benito-TAURI-5Z and the budget-noise cluster —
    ...
    pub async fn run_interactive(&mut self) -> Result<()> {
        println!("🦀 Benito Interactive Mode");
        println!("Type /quit to exit.\n");

        let (tx, mut rx) = tokio::sync::mpsc::channel(32);
        let cli = crate::benito::channels::CliChannel::new();

        let listen_handle = tokio::spawn(async move {
            let _ = crate::benito::channels::Channel::listen(&cli, tx).await;
        });
