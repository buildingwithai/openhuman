...
use crate::benito::inference::provider::{ChatMessage, ChatRequest, ChatResponse, Provider};
use crate::benito::tools::traits::{PermissionLevel, Tool, ToolCategory, ToolResult, ToolScope};
...
fn mm() -> crate::benito::config::MultimodalConfig {
...
async fn harness_invokes_composio_action_tool_against_fake_backend() {
    ...
    // Hold `TEST_ENV_LOCK` and point `BENITO_WORKSPACE` at the
    ...
    unsafe {
        std::env::set_var("BENITO_WORKSPACE", &workspace_root);
    }
...
async fn orchestrator_prompt_drives_composio_call_via_delegation_chain() {
    use crate::benito::agent::agents::orchestrator::prompt as orch_prompt;
    use crate::benito::agent::prompts::types::ConnectedIntegration;
    use crate::benito::composio::ComposioActionTool;
    use crate::benito::config::TEST_ENV_LOCK;

    // Post-#1710-Wave-4, `ComposioActionTool::execute` reloads config via
    // `load_config_with_timeout()` per call. Hold `TEST_ENV_LOCK` and
    // point `BENITO_WORKSPACE` at the persisted fake-backend
    ...
    let ctx = {
        use crate::benito::context::prompt::{LearnedContextData, ToolCallFormat};
        use std::collections::HashSet;
        use std::sync::OnceLock;
        static EMPTY: OnceLock<HashSet<String>> = OnceLock::new();
        crate::benito::context::prompt::PromptContext {
