...
//! [`crate::benito::agent::debug`] can mirror the live runner
...
use crate::benito::context::prompt::PromptContext;
...
/// [`crate::benito::agent::dispatcher::XmlToolDispatcher::prompt_instructions`]
...
/// agent builder ([`crate::benito::agent::harness::session::builder`])
...
///   ([`crate::benito::tools::orchestrator_tools::collect_orchestrator_tools`]
...
/// [`crate::benito::agent::debug`] shares the exact same
...
///
/// Exposed `pub(crate)` so the debug dump path in
/// [`crate::benito::agent::debug`] loads prompts through the
    ...
    match source {
        ...
        PromptSource::File { path } => {
            ...
            if workspace_path.is_file() {
                if let Ok(resolved) = crate::benito::security::validate_path_within_root(
                    &workspace_path,
                    &prompt_root,
                ) {
            ...
            // already shipped at `src/Benito/agent/prompts/` and
