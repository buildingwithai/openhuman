use super::*;
use crate::benito::agent::harness::definition::{ModelSpec, ToolScope};
...
fn make_def_named_tools(names: &[&str]) -> AgentDefinition {
    AgentDefinition {
        ...
        timeout_secs: None,
        sandbox_mode: crate::benito::agent::harness::definition::SandboxMode::None,
        background: false,
        subagents: vec![],
        delegate_name: None,
        agent_tier: crate::benito::agent::harness::definition::AgentTier::Worker,
        source: crate::benito::agent::harness::definition::DefinitionSource::Builtin,
    }
}
...
use crate::benito::tools::{PermissionLevel, ToolResult};
...
fn append_subagent_role_contract_adds_role_and_brevity_rules() {
    ...
    assert!(rendered.contains("You are a sub-agent working for a parent Benito agent"));
...
use crate::benito::agent::harness::fork_context::with_parent_context;
...
struct CapturedRequest {
    messages: Vec<crate::benito::inference::provider::ChatMessage>,
    tool_count: usize,
    model: String,
}
...
fn make_parent(provider: Arc<dyn Provider>, tools: Vec<Box<dyn Tool>>) -> ParentExecutionContext {
    let tool_specs: Vec<crate::benito::tools::ToolSpec> =
    ...
    ParentExecutionContext {
        ...
        memory: noop_memory(),
        agent_config: crate::benito::config::AgentConfig::default(),
        skills: Arc::new(vec![]),
        memory_context: Arc::new(None),
        session_id: "test-session".into(),
        channel: "test".into(),
        connected_integrations: vec![],
        tool_call_format: crate::benito::context::prompt::ToolCallFormat::PFormat,
...
fn noop_memory() -> Arc<dyn crate::benito::memory::Memory> {
    struct NoopMemory;
    #[async_trait]
    impl crate::benito::memory::Memory for NoopMemory {
        async fn store(
            &self,
            _namespace: &str,
            _key: &str,
            _content: &str,
            _category: crate::benito::memory::MemoryCategory,
        ...
        async fn recall(
            ...
            _query: &str,
            _limit: usize,
            _opts: crate::benito::memory::RecallOpts<'_>,
        ) -> anyhow::Result<Vec<crate::benito::memory::MemoryEntry>> {
            Ok(vec![])
        }
        async fn get(
            &self,
            _namespace: &str,
            _key: &str,
        ) -> anyhow::Result<Option<crate::benito::memory::MemoryEntry>> {
            Ok(None)
        }
        async fn list(
            &self,
            _namespace: Option<&str>,
            _category: Option<&crate::benito::memory::MemoryCategory>,
            _session_id: Option<&str>,
        ) -> anyhow::Result<Vec<crate::benito::memory::MemoryEntry>> {
            Ok(vec![])
        }
        ...
        async fn namespace_summaries(
            &self,
        ) -> anyhow::Result<Vec<crate::benito::memory::NamespaceSummary>> {
            Ok(vec![])
        }
...
async fn typed_mode_system_prompt_includes_subagent_role_contract() {
    ...
    assert!(system_msg
        .content
        .contains("You are a sub-agent working for a parent Benito agent"));
    assert!(system_msg
        .content
...
fn resolve_subagent_provider_config_model_wins_over_definition_model() {
    use crate::benito::config::{Config, TeamModelConfig};
...
fn resolve_subagent_provider_hint_with_no_config_falls_back() {
    ...
    // `{workload}-v1` — that's the Benito-only naming that breaks
...
fn resolve_subagent_provider_hint_with_config_routes_via_factory() {
    ...
    // factory may return a fresh Benito backend or whatever
    ...
    let mut config = Config::default();
    // Route `agentic` to Benito backend explicitly. The backend
    // returns the configured default_model, which we set to a known
    // string so the assertion is meaningful.
    config.agentic_provider = Some("Benito".to_string());
...
fn direct_mode_user_with_stored_key_passes_signed_in_check() {
    ...
    let mut config = crate::benito::config::Config::default();
    ...
    config.composio.mode = crate::benito::config::schema::COMPOSIO_MODE_DIRECT.to_string();
