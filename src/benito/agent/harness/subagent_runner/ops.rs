...
use crate::benito::agent::harness::definition::{AgentDefinition, PromptSource};
...
use crate::benito::context::prompt::{
    render_subagent_system_prompt, PromptContext, PromptTool, SubagentRenderOptions,
};
use crate::benito::inference::provider::{ChatMessage, ChatRequest, Provider, ToolCall};
use crate::benito::memory::conversations::ConversationMessage;
use crate::benito::tools::{Tool, ToolCategory, ToolSpec};
...
You are a sub-agent working for a parent Benito agent, not a direct end-user assistant.\n\
...
///     workload — the Benito backend and every third-party provider
...
pub(super) fn resolve_subagent_provider(
    spec: &crate::benito::agent::harness::definition::ModelSpec,
    agent_id: &str,
    config: Option<&crate::benito::config::Config>,
    parent_provider: std::sync::Arc<dyn Provider>,
    parent_model: String,
    is_team_lead: bool,
    model_override: Option<&str>,
) -> (std::sync::Arc<dyn Provider>, String) {
    use crate::benito::agent::harness::definition::ModelSpec;
    ...
    match spec {
        ModelSpec::Hint(workload) => match config {
            Some(cfg) => {
                match crate::benito::inference::provider::create_chat_provider(workload, cfg) {
                    Ok((p, m)) => {
                        log::info!(
...
/// [`crate::benito::composio::ComposioClient`] so the live
/// `composio.mode` toggle is honoured per execute — see
/// [`crate::benito::composio::ComposioActionTool`] and issue #1710.
struct LazyToolkitResolver {
    config: std::sync::Arc<crate::benito::config::Config>,
    actions: Vec<crate::benito::context::prompt::ConnectedIntegrationTool>,
}
...
impl LazyToolkitResolver {
    fn resolve(&self, name: &str) -> Option<Box<dyn Tool>> {
        ...
        Some(Box::new(
            crate::benito::composio::ComposioActionTool::new(
                self.config.clone(),
                action.name.clone(),
...
async fn run_typed_mode(
    ...
    let config_loaded = crate::benito::config::Config::load_or_init().await;
    ...
    let live_integrations: Vec<crate::benito::context::prompt::ConnectedIntegration> = {
        ...
        let probe_config = crate::benito::config::Config::load_or_init().await.ok();
        ...
        if !signed_in {
            ...
            match crate::benito::config::Config::load_or_init().await {
                Ok(config) => {
                    use crate::benito::composio::FetchConnectedIntegrationsStatus;
                    ...
                    // can't answer.
                    match crate::benito::composio::fetch_connected_integrations_status(&config)
                        .await
                    {
    ...
    if is_integrations_agent_with_toolkit {
        ...
        if let Some(tk) = toolkit_filter {
            ...
            let arc_config = match crate::benito::config::Config::load_or_init().await {
            ...
            // surface (#1710 Wave 2).
            use crate::benito::composio::client::{create_composio_client, ComposioClientKind};
                ...
                .find(|ci| ci.connected && ci.toolkit.eq_ignore_ascii_case(tk))
            {
                ...
                let fresh_actions = match &client_kind {
                    Some(ComposioClientKind::Backend(client)) => {
                        match crate::benito::composio::fetch_toolkit_actions(client, tk).await {
                ...
                let integration = crate::benito::context::prompt::ConnectedIntegration {
                ...
                let selected: Vec<&crate::benito::context::prompt::ConnectedIntegrationTool> =
    ...
    let narrowed_integrations: Vec<crate::benito::context::prompt::ConnectedIntegration> =
    ...
    // [`crate::benito::context::prompt`]. `Inline` / `File` sources
    ...
    let dispatcher_instructions = {
        ...
        use crate::benito::agent::pformat::PFormatRegistry;
        use crate::benito::context::prompt::ToolCallFormat;
    ...
    let prompt_ctx = PromptContext {
        ...
        learned: crate::benito::context::prompt::LearnedContextData::default(),
    ...
    let append_worker_message =
        |content: String, sender: String, extra_metadata: serde_json::Value| {
            if let Some(ref thread_id) = worker_thread_id {
                ...
                if let Err(err) = crate::benito::memory::conversations::append_message(
                    parent.workspace_dir.clone(),
                    thread_id,
                    message,
                ) {
    ...
    for iteration in 0..max_iterations {
        ...
        for call in &native_calls {
                ...
                .or_else(|| parent_tools.iter().find(|t| t.name() == call.name))
            {
                ...
                let timeout = crate::benito::tool_timeout::tool_execution_timeout_duration();
                ...
                let gate_denial: Option<String> = if tool.external_effect_with_args(&args) {
                    if let Some(gate) = crate::benito::approval::ApprovalGate::try_global() {
                        let summary =
                            crate::benito::approval::summarize_action(&call.name, &args);
                        let redacted = crate::benito::approval::redact_args(&args);
                        match gate.intercept(&call.name, &summary, redacted).await {
                            crate::benito::approval::GateOutcome::Allow => None,
                            crate::benito::approval::GateOutcome::Deny { reason } => {
                                tracing::warn!(
...
pub(crate) fn user_is_signed_in_to_composio(config: &crate::benito::config::Config) -> bool {
    crate::benito::composio::client::create_composio_client(config).is_ok()
}
