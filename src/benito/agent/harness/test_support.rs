...
//!    [`crate::benito::composio::ComposioClient`] to exercise the
...
use crate::benito::composio::ComposioClient;
        ...
        Self {
            ...
            execute_responses: [
                ...
                (
                    ...
                    json!({
                        "messages": [
                            {"id": "m1", "subject": "Welcome", "from": "team@Benito.com"},
...
impl FakeComposioBackend {
    ...
    /// ([`crate::benito::composio::ComposioActionTool`],
    ...
    /// resolves. Callers must hold `crate::benito::config::TEST_ENV_LOCK`
    /// and `std::env::set_var("BENITO_WORKSPACE", &workspace_root)`
    ...
    /// `BENITO_WORKSPACE` to). The tempdir is leaked so it stays
    /// valid for the test's lifetime.
    pub async fn config_persisted(
        &self,
    ) -> (
        std::sync::Arc<crate::benito::config::Config>,
        std::path::PathBuf,
    ) {
        ...
        config.composio.mode = crate::benito::config::schema::COMPOSIO_MODE_BACKEND.to_string();
        ...
        // from `BENITO_WORKSPACE`) reads exactly this config.
        config
            .save()
            .await
