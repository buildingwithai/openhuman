...
//! [`crate::benito::agent::agents`] — each agent is a subfolder
...
/// [`crate::benito::agent::agents`] verify in CI that every entry in
/// [`crate::benito::agent::agents::BUILTINS`] still parses cleanly.
///
...
pub fn all() -> Vec<AgentDefinition> {
    #[allow(unused_mut)]
    let mut defs = crate::benito::agent::agents::load_builtins()
...
mod tests {
    ...
    fn all_definitions_present() {
        ...
        assert_eq!(
            defs.len(),
            crate::benito::agent::agents::BUILTINS.len() + 2
        );
    }
