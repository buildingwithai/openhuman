...
//! [`crate::benito::agent::agents`] — one subfolder per agent, each
...
//! TOML files under `$BENITO_WORKSPACE/agents/*.toml` (with a fallback
//! to `~/.Benito/agents/*.toml` for user-global specialists) which
...
pub struct AgentDefinition {
    ...
    /// [`crate::benito::context::prompt::render_subagent_system_prompt`].
    #[serde(default = "defaults::true_")]
    ...
    ///
    /// [`ArchetypeDelegationTool`]: crate::benito::tools::impl::agent::ArchetypeDelegationTool
    /// [`SkillDelegationTool`]: crate::benito::tools::impl::agent::SkillDelegationTool
    #[serde(default)]
...
/// full runtime [`crate::benito::context::prompt::PromptContext`]
...
/// [`crate::benito::context::prompt`] in the order the builder
/// wants.
pub type PromptBuilder =
    fn(&crate::benito::context::prompt::PromptContext<'_>) -> anyhow::Result<String>;
...
pub enum PromptSource {
    ...
    /// `src/Benito/agent/prompts/` for built-ins. Resolved by the runner
    /// at spawn time.
    File { path: String },
...
impl AgentDefinitionRegistry {
    ...
    /// `~/.Benito/agents/*.toml` fallback). Custom definitions
    ...
    /// [`crate::benito::config::Config::load_or_init`] and load from
    /// it. Built for sync CLI call sites (`Benito agent list`,
    ...
    pub async fn load_for_default_workspace() -> Result<Self> {
        let config = crate::benito::config::Config::load_or_init().await?;
