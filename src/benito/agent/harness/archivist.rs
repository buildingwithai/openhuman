...
use crate::benito::memory::store::events::{self, EventRecord, EventType};
use crate::benito::memory::store::fts5::{self, EpisodicEntry};
use crate::benito::memory::store::profile::{self, FacetType};
use crate::benito::memory::store::segments::{
...
use crate::benito::memory::tree::chat::{ChatConsumer, ChatProvider};
use crate::benito::memory::tree::ingest;
use crate::benito::memory::tree::score::embed::{build_embedder_from_config, Embedder};
use crate::benito::memory::tree::tree_source::summariser::llm::{
...
impl ArchivistHook {
    ...
    pub fn with_config(mut self, config: Config) -> Self {
        ...
        let chat_provider: Option<Arc<dyn ChatProvider>> =
            match crate::benito::memory::tree::chat::build_chat_provider(
                &config,
...
impl ArchivistHook {
    ...
    async fn summarize_entries(
        ...
        let corpus_inputs: Vec<SummaryInput> = entries
            ...
            .map(|e| {
                use crate::benito::memory::tree::types::approx_token_count;
    ...
    async fn pipe_segment_to_tree(
        &self,
        config: &Config,
        segment: &crate::benito::memory::store::segments::ConversationSegment,
        session_id: &str,
...
fn extract_lesson_from_tools(
    tool_calls: &[crate::benito::agent::hooks::ToolCallRecord],
) -> Option<String> {
