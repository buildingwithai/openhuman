...
// `Benito.composio_list_tools` call. Fixtures live in
...
fn real_data_github_create_pr() {
    ...
    let hits = filter_actions_by_prompt(
        "Create a pull request from feature/auth-fix to main in the Benito repo",
        &actions,
        15,
    );
...
fn real_data_github_list_prs() {
    let actions = load_real_toolkit("github");
    let hits = filter_actions_by_prompt(
        "Find all open pull requests assigned to the current user in the Benito repo",
        &actions,
        15,
    );
    assert!(hits.len() >= MIN_CONFIDENT_HITS);
