use crate::benito::agent::cost::TurnCost;
use crate::benito::agent::multimodal;
use crate::benito::agent::progress::AgentProgress;
use crate::benito::agent::stop_hooks::{current_stop_hooks, StopDecision, TurnState};
use crate::benito::approval::{ApprovalManager, ApprovalRequest, ApprovalResponse};
use crate::benito::inference::provider::{
    ChatMessage, ChatRequest, Provider, ProviderCapabilityError, ProviderDelta,
};
use crate::benito::tools::traits::ToolScope;
...
use crate::benito::context::guard::{ContextCheckResult, ContextGuard};
...
pub(crate) async fn agent_turn(
    ...
    temperature: f64,
    silent: bool,
    multimodal_config: &crate::benito::config::MultimodalConfig,
...
pub(crate) async fn run_tool_call_loop(
    ...
    let tool_specs: Vec<crate::benito::tools::ToolSpec> = tools_registry
        .iter()
    ...
    for iteration in 0..max_iterations {
        ...
        match context_guard.check() {
            ...
            ContextCheckResult::CompactionNeeded => {
                tracing::warn!(
                    ...
                    crate::benito::context::guard::COMPACTION_TRIGGER_THRESHOLD * 100.0
                );
        ...
        let (response_text, parsed_text, tool_calls, assistant_history_content, native_tool_calls) =
            match chat_result {
                ...
                Err(e) => {
                    ...
                    // signal and floods Sentry — see Benito-TAURI-3Y/3Z
                    // (~46 events combined) and the underlying TAURI-2E/84/T
                    // (~3300 events from raw per-attempt 429/503/504 reports).
                    let transient = crate::benito::inference::provider::reliable::is_rate_limited(
                        &e,
                    )
                        || crate::benito::inference::provider::reliable::is_upstream_unhealthy(
                            &e,
                        );
                    if transient {
                        tracing::warn!(
        ...
        for (call_idx, call) in tool_calls.iter().enumerate() {
            ...
            if let Some(tool) = tool_opt {
                if tool.external_effect_with_args(&call.arguments) {
                    if let Some(gate) = crate::benito::approval::ApprovalGate::try_global() {
                        let summary = crate::benito::approval::summarize_action(
                            &call.name,
                            &call.arguments,
                        );
                        let redacted = crate::benito::approval::redact_args(&call.arguments);
                        match gate.intercept(&call.name, &summary, redacted).await {
                            crate::benito::approval::GateOutcome::Allow => {}
            ...
            let result = if let Some(tool) = tool_opt {
                let tool_deadline =
                    crate::benito::tool_timeout::tool_execution_timeout_duration();
                let timeout_secs = crate::benito::tool_timeout::tool_execution_timeout_secs();
                    ...
                    Ok(Ok(r)) => {
                        ...
                        if success {
                            ...
                            let (compacted, tj_stats) =
                                crate::benito::tokenjuice::compact_tool_output(
                                    &call.name,
                                    Some(&call.arguments),
                                    &scrubbed,
                                    Some(0),
                            ...
                            let (compacted, _) = crate::benito::tokenjuice::compact_tool_output(
                                &call.name,
                                Some(&call.arguments),
                                &scrubbed,
                                Some(1),
    ...
    // (Benito-TAURI-99 / -98). The `Display` text is preserved verbatim so
