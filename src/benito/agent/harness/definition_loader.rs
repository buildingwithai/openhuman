...
//! with a fallback to `~/.Benito/agents/*.toml` for user-global
//! specialists. Each file defines exactly one definition.
//!
//! TOML (rather than YAML) is used for consistency with the rest of
//! Benito's config system, which already depends on the `toml` crate
...
/// `~/.Benito/agents/` fallback. Returns an empty Vec when neither
/// directory exists.
...
/// injected by [`crate::benito::agent::agents::load_builtins`], so a
...
fn user_home_agents_dir() -> Option<PathBuf> {
    // Honour BENITO_HOME first if set; otherwise ~/.Benito.
    if let Ok(custom) = std::env::var("BENITO_HOME") {
        return Some(PathBuf::from(custom).join("agents"));
    }
    match crate::benito::config::default_root_BENITO_dir() {
        Ok(dir) => Some(dir.join("agents")),
        Err(error) => {
            tracing::debug!(
                error = %error,
                "[agent-definition-loader] resolving root Benito dir failed"
            );
            None
        }
    }
