...
use crate::benito::agent::profiles::{AgentProfile, AgentProfileStore};
...
fn handle_chat(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::inference::local::rpc::agent_chat(
                &mut config,
                &p.message,
                p.model_override,
                p.temperature,
...
fn handle_chat_simple(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::benito::inference::local::rpc::agent_chat_simple(
                &config,
                &p.message,
                p.model_override,
                p.temperature,
...
fn handle_list_definitions(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async {
        let registry = crate::benito::agent::harness::AgentDefinitionRegistry::global()
            .ok_or_else(|| "AgentDefinitionRegistry not initialised".to_string())?;
        let defs: Vec<&crate::benito::agent::harness::AgentDefinition> = registry.list();
...
fn handle_reload_definitions(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async {
        ...
        let already_loaded =
            crate::benito::agent::harness::AgentDefinitionRegistry::global().is_some();
        Ok(serde_json::json!({
            "status": "noop",
    ...
    Box::pin(async move {
        ...
        let envelope = match p.source.as_str() {
            "composio" => {
                ...
                crate::benito::agent::triage::TriggerEnvelope::from_composio(
            ...
            "webhook" => {
                ...
                crate::benito::agent::triage::TriggerEnvelope::from_webhook(
            ...
            "cron" => {
                ...
                let output = p
                    ...
                    .unwrap_or(job_name);
                crate::benito::agent::triage::TriggerEnvelope::from_cron(
            ...
            "external" => {
                ...
                crate::benito::agent::triage::TriggerEnvelope::from_external(
        ...
        let outcome = crate::benito::agent::triage::run_triage(&envelope)
            .await
            .map_err(|e| format!("triage evaluation failed: {e}"))?;

        let dry_run = p.dry_run.unwrap_or(false);
        match outcome {
            crate::benito::agent::triage::TriageOutcome::Decision(run) => {
                if !dry_run {
                    crate::benito::agent::triage::apply_decision(run.clone(), &envelope)
                        .await
            ...
            crate::benito::agent::triage::TriageOutcome::Deferred {
                defer_until_ms,
                reason,
            } => {
...
fn handle_profile_upsert(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        if let Some(registry) = crate::benito::agent::harness::AgentDefinitionRegistry::global()
        {
...
mod tests {
    ...
    use crate::benito::agent::profiles::DEFAULT_PROFILE_ID;
    ...
    impl Drop for WorkspaceEnvGuard {
        fn drop(&mut self) {
            match self.previous.take() {
                Some(value) => unsafe {
                    std::env::set_var("BENITO_WORKSPACE", value);
                },
                None => unsafe {
    ...
    async fn profile_upsert_rejects_unknown_registered_agent_id() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let _ = crate::benito::agent::harness::AgentDefinitionRegistry::init_global_builtins();
