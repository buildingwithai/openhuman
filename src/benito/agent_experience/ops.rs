...
use crate::benito::agent_experience::store::{AgentExperienceStore, ExperienceQuery};
use crate::benito::agent_experience::types::{AgentExperience, ExperienceHit};
...
async fn open_store() -> Result<AgentExperienceStore, String> {
    let client = match crate::benito::memory::global::client_if_ready() {
        Some(client) => client,
        None => {
            let config = Config::load_or_init()
                .await
                .map_err(|e| format!("load config: {e}"))?;
            crate::benito::memory::global::init(config.workspace_dir)?
        }
    };
